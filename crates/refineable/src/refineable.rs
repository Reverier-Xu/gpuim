...
///
/// The `Refineable` trait enables hierarchical configuration patterns where a
/// base configuration can be selectively overridden by refinements. This is
/// particularly useful for styling and settings, and theme hierarchies.
///
/// # Derive Macro
///
/// The `#[derive(Refineable)]` macro automatically generates a companion
/// refinement type and implements this trait. For a struct `Style`, it creates
/// `StyleRefinement` where each field is wrapped appropriately:
///
/// - **Refineable fields** (marked with `#[refineable]`): Become the
///   corresponding refinement type (e.g., `Bar` becomes `BarRefinement`, or
///   `BarRefinement` remains `BarRefinement`)
...
/// - `#[refineable(Serialize)]`: Derives `Serialize` which skips serializing
///   `None`
/// - `#[refineable(OtherTrait)]`: Derives additional traits on the refinement
///   type
///
...
pub trait Refineable: Clone {
    ...
    /// Returns a new instance with the refinement applied, equivalent to
    /// cloning `self` and calling `refine` on it.
    fn refined(self, refinement: Self::Refinement) -> Self;

    /// Creates an instance from a cascade by merging all refinements atop the
    /// default value.
    ...
    /// Returns `true` if this instance would contain all values from the
    /// refinement.
    ///
    /// For refineable fields, this recursively checks `is_superset_of`. For
    /// other fields, this checks if the refinement's `Some` values match
    /// this instance's values.
    fn is_superset_of(&self, refinement: &Self::Refinement) -> bool;

    /// Returns a refinement that represents the difference between this
    /// instance and the given refinement.
    ///
    /// For refineable fields, this recursively calls `subtract`. For other
    /// fields, the field is `None` if the field's value is equal to the
    /// refinement.
...
///
/// This is useful for implementing configuration hierarchies like CSS
/// cascading, where styles from different sources (user agent, user, author)
/// are combined with specific precedence rules.
