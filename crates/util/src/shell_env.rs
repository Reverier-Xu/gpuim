...
pub async fn capture(
    shell_path: impl AsRef<Path>, args: &[String], directory: impl AsRef<Path>,
...
async fn capture_unix(
    shell_path: &Path, args: &[String], directory: &Path,
) -> Result<collections::HashMap<String, String>> {
    ...
    // mode, so file descriptor 0 (stdin) is used instead. This impacts zsh, old
    // bash; perhaps others. See: https://github.com/zed-industries/zed/pull/32136#issuecomment-2999645482
    ...
    match shell_kind {
        ShellKind::Csh | ShellKind::Tcsh => {
            // For csh/tcsh, login shell requires passing `-` as 0th argument (instead of
            // `-l`)
            command.arg0("-");
        }
        ShellKind::Fish => {
    ...
    // Prefix with "./" if the path starts with "-" to prevent cd from interpreting
    // it as a flag
    ...
    // cd into the directory, triggering directory specific side-effects (asdf,
    // direnv, etc)
...
async fn spawn_and_read_fd(
    mut command: std::process::Command, child_fd: std::os::fd::RawFd,
