...
pub struct ShellBuilder {
    ...
    args: Vec<String>,
    interactive: bool,
    /// Whether to redirect stdin to /dev/null for the spawned command as a
    /// subshell.
    redirect_stdin: bool,
    kind: ShellKind,
}
...
impl ShellBuilder {
    ...
    pub fn build(
        mut self, task_command: Option<String>, task_args: &[String],
    ) -> (String, Vec<String>) {
    ...
    /// Builds a `smol::process::Command` with the given task command and
    /// arguments.
    ///
    /// Prefer this over manually constructing a command with the output of
    /// `Self::build`, as this method handles `cmd` weirdness on windows
    /// correctly.
    pub fn build_smol_command(
        self, task_command: Option<String>, task_args: &[String],
    ) -> smol::process::Command {
        smol::process::Command::from(self.build_std_command(task_command, task_args))
    }

    /// Builds a `std::process::Command` with the given task command and
    /// arguments.
    ///
    /// Prefer this over manually constructing a command with the output of
    /// `Self::build`, as this method handles `cmd` weirdness on windows
    ...
    pub fn build_std_command(
        self, mut task_command: Option<String>, task_args: &[String],
    ) -> std::process::Command {
        #[cfg(windows)]
        let kind = self.kind;
