...
/// Generates a URL-friendly slug from heading text (e.g. "Hello World" →
/// "hello-world").
...
/// Escapes markdown special characters in markdown text blocks. Markdown code
/// blocks follow different rules and `MarkdownInlineCode` or
/// `MarkdownCodeBlock` should be used in that case.
///
...
/// * `~` for strikethrough
///
/// Escape of some characters is unnecessary, because while they are involved in
/// markdown syntax, the other characters involved are escaped:
///
/// * `!`, `]`, `(`, and `)` are used in link syntax, but `[` is escaped so
///   these are parsed as plaintext.
///
/// * `;` is used in HTML entity syntax, but `&` is escaped, so they are parsed
///   as plaintext.
///
/// TODO: There is one escape this doesn't do currently. Period after numbers at
/// the start of the line (`[0-9]*\.`) should also be escaped to avoid it being
/// interpreted as a list item.
pub struct MarkdownEscaped<'a>(pub &'a str);

/// Implements `Display` to format markdown inline code (wrapped in backticks),
/// handling code that contains backticks and spaces. All whitespace is treated
/// as a single space character. For text that does not contain whitespace other
/// than ' ', this escaping roundtrips through pulldown-cmark.
///
/// When used in tables, `|` should be escaped like `\|` in the text provided to
/// this function.
pub struct MarkdownInlineCode<'a>(pub &'a str);

/// Implements `Display` to format markdown code blocks, wrapped in 3 or more
/// backticks as needed.
...
impl Display for MarkdownInlineCode<'_> {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        // When inline code has leading and trailing ' ' characters, additional space is
        // needed to escape it, unless all characters are space.
        if all_whitespace {
            write!(formatter, "`{text}`")
        } else {
            // More backticks are needed to delimit the inline code than the maximum number
            // of backticks in a consecutive run.
