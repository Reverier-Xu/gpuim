...
pub async fn extract_zip<R: AsyncRead + Unpin>(destination: &Path, reader: R) -> Result<()> {
    ...
    // This is only possible to do when a reader impls `AsyncSeek` and
    // `seek::ZipFileReader` is used. `stream::ZipFileReader` also has the
    // `unix_permissions` method, but it will always return `Some(0)`.
    //
    // A typical `reader` comes from a streaming network response, so cannot be
    // sought right away, and reading the entire archive into the memory seems
    // wasteful.
    //
    // So, save the stream into a temporary file first and then get it read with a
    // seeking reader.
...
mod tests {
    ...
    fn test_extract_zip_sets_default_permissions() {
        ...
        smol::block_on(async {
            ...
            // by default for new files. This should match what we saw when we previously
            // wrote the file.
            let extracted_path = extract_dir.path().join("my_script");
            assert!(extracted_path.exists());
            let extracted_perms = std::fs::metadata(&extracted_path).unwrap().permissions();
