...
/// Removes characters from the end of the string if its length is greater than
/// `max_chars` and appends "..." to the string. Returns string unchanged if its
/// length is smaller than max_chars.
pub fn truncate_and_trailoff(s: &str, max_chars: usize) -> String {
    debug_assert!(max_chars >= 5);

    // If the string's byte length is <= max_chars, walking the string can be
    // skipped since the number of chars is <= the number of bytes.
    if s.len() <= max_chars {
        return s.to_string();
    }
...
/// Removes characters from the front of the string if its length is greater
/// than `max_chars` and prepends the string with "...". Returns string
/// unchanged if its length is smaller than max_chars.
...
/// Takes only `max_lines` from the string and, if there were more than
/// `max_lines-1`, appends a a newline and "..." to the string, so that
/// `max_lines` are returned. Returns string unchanged if its length is smaller
/// than max_lines.
...
/// Truncates the string at a character boundary, such that the result is less
/// than `max_bytes` in length.
...
/// Takes a prefix of complete lines which fit within the byte limit. If the
/// first line is longer than the limit, truncates at a character boundary.
...
/// Extend a sorted vector with a sorted sequence of items, maintaining the
/// vector's sort order and enforcing a maximum length. This also de-duplicates
/// items. Sort the items according to the given callback. Before calling this,
/// both `vec` and `new_items` should already be sorted according to the `cmp`
/// comparator.
...
where
    ...
    // When limit is near to items.len() it may be more efficient to sort the whole
    // list and truncate, rather than always doing selection first as is done
    // below. It's hard to analyze where the threshold for this should be since
    // the quickselect style algorithm used by `select_nth_unstable_by` makes
    // the prefix partially sorted, and so its work is not wasted - the expected
    // number of comparisons needed by `sort_by` is less than it is for some
    // arbitrary unsorted input.
...
/// and terminates the program with an error message unless explicitly allowed
/// via the `ZED_ALLOW_ROOT` environment variable.
#[cfg(unix)]
...
pub fn get_shell_safe_zed_path(shell_kind: shell::ShellKind) -> anyhow::Result<String> {
    ...
    {
        // Might have been deleted during update; let's use the new binary if there is
        // one.
...
pub async fn load_login_shell_environment() -> Result<()> {
        ...
        .with_context(|| format!("capturing environment with {:?}", get_system_shell()))?
    {
        ...
        // The login shell used for env capture increments SHLVL, and if we propagate
        // it, terminals spawned by Zed will inherit it and increment again,
        // causing SHLVL to start at 2 instead of 1 (and increase by 2 on each
        // reload).
        if name == "SHLVL" {
            continue;
        }
...
/// Configures the process to start a new session, to prevent interactive shells
/// from taking control of the terminal.
///
...
pub fn merge_json_lenient_value_into(
    source: serde_json_lenient::Value, target: &mut serde_json_lenient::Value,
) {
    match (source, target) {
...
pub fn expanded_and_wrapped_usize_range(
    range: Range<usize>, additional_before: usize, additional_after: usize, wrap_length: usize,
...
/// Yields `[i, i + 1, i - 1, i + 2, ..]`, each modulo `wrap_length` and bounded
/// by `additional_before` and `additional_after`. If the wrapping causes
/// overlap, duplicates are not emitted. If wrap_length is 0, nothing is
/// yielded.
pub fn wrapped_usize_outward_from(
    start: usize, additional_before: usize, additional_after: usize, wrap_length: usize,
) -> impl Iterator<Item = usize> {
    let mut count = 0;
...
/// A way to sort strings with starting numbers numerically first, falling back
/// to alphanumeric one, case-insensitive.
///
/// This is useful for turning regular alphanumerically sorted sequences as
/// `1-abc, 10, 11-def, .., 2, 21-abc` into `1-abc, 2, 10, 11-def, .., 21-abc`
...
/// When dealing with equality, we need to consider the case of the strings to
/// achieve strict equality to handle cases like "a" < "A" instead of "a" ==
/// "A".
...
/// Similar to `str::split`, but also provides byte-offset ranges of the
/// results. Unlike `str::split`, this is not generic on pattern types and does
/// not return an `Iterator`.
pub fn split_str_with_ranges<'s>(
    s: &'s str, pat: &dyn Fn(char) -> bool,
