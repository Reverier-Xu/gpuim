...
impl ShellKind {
    ...
    /// Returns whether this shell's command chaining syntax can be parsed by
    /// brush-parser.
    ///
    /// This is used to determine if we can safely parse shell commands to
    /// extract sub-commands for security purposes (e.g., preventing shell
    /// injection in "always allow" patterns).
    ///
    /// The brush-parser handles `;` (sequential execution) and `|` (piping),
    /// which are supported by all common shells. It also handles `&&` and
    /// `||` for conditional execution, `$()` and backticks for command
    /// substitution, and process substitution.
    ///
    /// # Shell Notes
    ///
    /// - **Nushell**: Uses `;` for sequential execution. The `and`/`or`
    ///   keywords are boolean operators on values (e.g., `$true and $false`),
    ///   not command chaining operators.
    /// - **Elvish**: Uses `;` to separate pipelines, which brush-parser
    ///   handles. Elvish does not have `&&` or `||` operators. Its `and`/`or`
    ///   are special commands that operate on values, not command chaining
    ///   (e.g., `and $true $false`).
    /// - **Rc (Plan 9)**: Uses `;` for sequential execution and `|` for piping.
    ///   Does not have `&&`/`||` operators for conditional chaining.
    /// All current shell variants are listed here because brush-parser can
    /// handle their syntax. If a new `ShellKind` variant is added, evaluate
    /// whether brush-parser can safely parse its command chaining syntax
    /// before including it. Omitting a variant will cause
    /// `tool_permissions::from_input` to deny terminal commands that have
    /// `always_allow` patterns configured.
    ...
    /// Quotes the given argument if necessary, taking into account the command
    /// prefix.
    ///
    /// In other words, this will consider quoting arg without its command
    /// prefix to not break the command. You should use this over
    /// `try_quote` when you want to quote a shell command.
    ...
    /// If we do we end up with too many quotes/escaped quotes for CMD to
    /// handle.
...
mod tests {
    ...
    // wsl.exe --distribution NixOS --cd /home/user -- /usr/bin/zsh -c "\"echo
    // hello\"" | grep hello" wsl.exe --distribution NixOS --cd ~ env
    // RUST_LOG=info,remote=debug .zed_wsl_server/zed-remote-server-dev-build proxy
    // --identifier dev-workspace-53 PowerShell from Nushell
    // nu -c overlay use
    // "C:\Users\kubko\dev\python\39007\tests\.venv\Scripts\activate.nu";
    // ^"C:\Program Files\PowerShell\7\pwsh.exe" -C
    // "C:\Users\kubko\dev\python\39007\tests\.venv\Scripts\python.exe -m pytest
    // \"test_foo.py::test_foo\"" PowerShell from CMD
    // cmd /C \" \"C:\\\\Users\\\\kubko\\\\dev\\\\python\\\\39007\\\\tests\\\\.venv\
    // \\\Scripts\\\\activate.bat\"& \"C:\\\\Program
    // Files\\\\PowerShell\\\\7\\\\pwsh.exe\" -C
    // \"C:\\\\Users\\\\kubko\\\\dev\\\\python\\\\39007\\\\tests\\\\.venv\\\\
    // Scripts\\\\python.exe -m pytest \\\"test_foo.py::test_foo\\\"\"\"
    ...
    fn test_try_quote_cmd_edge_cases() {
        ...
        // String with backslashes not before quotes (path without spaces doesn't need
        // quoting)
        assert_eq!(
            shell_kind.try_quote("C:\\path\\to\\file").unwrap(),
            "C:\\path\\to\\file"
