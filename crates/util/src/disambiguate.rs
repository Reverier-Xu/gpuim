...
/// stay at 0; items that collide get their detail level incremented until
/// either the collision is resolved or increasing the level no longer changes
/// the description (preventing infinite loops for truly identical items).
///
...
pub fn compute_disambiguation_details<T, D>(
    items: &[T], get_description: impl Fn(&T, usize) -> D,
) -> Vec<usize>
where
    D: Eq + Hash + Clone, {
...
mod tests {
    ...
    fn test_deeper_conflict() {
        ...
        // At detail 1, items 0 and 1 both show "src/file.rs", item 2 shows
        // "lib/file.rs". At detail 2, item 0 shows "a/src/file.rs", item 1
        // shows "b/src/file.rs".
        let items = vec![
    ...
    fn test_duplicate_paths_from_multiple_groups() {
        ...
        //   /Users/rtfeldman/code/zed                             (group 3, same path
        // as group 2)   /Users/rtfeldman/code/roc
        // (group 3)
        //
