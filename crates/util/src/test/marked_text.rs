...
pub fn marked_text_offsets_by(
    marked_text: &str, markers: Vec<char>,
...
pub fn marked_text_ranges_by(
    marked_text: &str, markers: Vec<TextRangeMarker>,
...
///    which can be typed on a US keyboard with the `alt-|` and `alt-shift-|`
///    keys.
///
///    ```text
...
///    ```
///
/// 2. To mark a single position in the text, use the `ˇ` caron, which can be
///    typed on a US keyboard with the `alt-shift-t` key.
///
///    ```text
///    the cursors are hereˇ and hereˇ.
///    ```
///
/// 3. To mark a range whose direction is meaningful (like a selection), put a
///    caron character beside one of its bounds, on the inside:
///
///    ```text
///    one «ˇreversed» selection and one «forwardˇ» selection
///    ```
///
/// Any • characters in the input string will be replaced with spaces. This
/// makes it easier to test cases with trailing spaces, which tend to get
/// trimmed from the source code.
#[track_caller]
pub fn marked_text_ranges(
    marked_text: &str, ranges_are_directed: bool,
...
pub fn generate_marked_text(
    unmarked_text: &str, ranges: &[Range<usize>], indicate_cursors: bool,
) -> String {
