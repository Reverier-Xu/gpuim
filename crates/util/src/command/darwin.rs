use std::{
    ...
    io,
    os::unix::{ffi::OsStrExt, io::FromRawFd, process::ExitStatusExt},
    path::{Path, PathBuf},
...
use mach2::{
    exception_types::{EXC_MASK_ALL, EXCEPTION_DEFAULT, exception_behavior_t, exception_mask_t},
    port::{MACH_PORT_NULL, mach_port_t},
    thread_status::{THREAD_STATE_NONE, thread_state_flavor_t},
};
use smol::Unblock;
...
unsafe extern "C" {
    fn posix_spawnattr_setexceptionports_np(
        attr: *mut libc::posix_spawnattr_t, mask: exception_mask_t, new_port: mach_port_t,
        behavior: exception_behavior_t, new_flavor: thread_state_flavor_t,
    ) -> libc::c_int;

    fn posix_spawn_file_actions_addchdir_np(
        file_actions: *mut libc::posix_spawn_file_actions_t, path: *const libc::c_char,
    ) -> libc::c_int;
...
fn spawn_posix_spawn(
    program: &OsStr, args: &[OsString], current_dir: &Path, envs: Option<&[(OsString, OsString)]>,
    stdin_cfg: Stdio, stdout_cfg: Stdio, stderr_cfg: Stdio, kill_on_drop: bool,
) -> io::Result<Child> {
...
/// Zero means `Ok()`, all other values are treated as raw OS errors. Does not
/// look at `errno`. Mirrored after Rust's std `cvt_nz` function.
