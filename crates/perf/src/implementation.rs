...
pub mod consts {
    //! Preset identifiers and constants so that the profiler and proc macro
    //! agree on their communication protocol.

    /// The suffix on the actual test function.
    pub const SUF_NORMAL: &str = "__ZED_PERF_FN";
    /// The suffix on an extra function which prints metadata about a test to
    /// stdout.
    ...
    /// The prefix printed on all benchmark test metadata lines, to distinguish
    /// it from possible output by the test harness itself.
    pub const MDATA_LINE_PREF: &str = "ZED_MDATA_";
    /// The version number for the data returned from the test metadata
    /// function. Increment on non-backwards-compatible changes.
...
pub enum Importance {
    ...
    #[default]
    Average   = 2,
    /// Unclear if regressions are likely to be meaningful, but still worth
    /// keeping an eye on. Lowest level that's checked by default by the
    /// profiler.
    Iffy      = 1,
    /// Regressions are likely to be spurious or don't affect core
    /// functionality. Only relevant if a lot of them happen, or as
    /// supplemental evidence for a higher-importance benchmark regressing.
...
pub struct TestMdata {
    ...
    /// - The handler exits with an error message indicating the version
    ///   mismatch or inability to parse the metadata.
    ///
...
impl Output {
    ...
    pub fn success(
        &mut self, name: impl AsRef<str>, mut mdata: TestMdata, iters: NonZero<usize>,
        timings: Timings,
    ) {
    ...
    /// Reports a failure and adds it to this run's `Output`. If this test was
    /// tried with some number of iterations (i.e. this was not a version
    /// mismatch or skipped test), it should be reported also.
    ///
    /// Using the `fail!()` macro is usually more convenient.
    pub fn failure(
        &mut self, name: impl AsRef<str>, mut mdata: Option<TestMdata>,
        attempted_iters: Option<NonZero<usize>>, kind: FailKind,
    ) {
    ...
    /// Merges the output of two runs, appending a prefix to the results of the
    /// new run. To be used in conjunction with `Output::blank()`, or else
    /// only some tests will have a prefix set.
    ...
    /// Evaluates the performance of `self` against `baseline`. The latter is
    /// taken as the comparison point, i.e. a positive resulting
    /// `PerfReport` means that `self` performed better.
    ///
    /// # Panics
