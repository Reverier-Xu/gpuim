//! Perf profiler for Zed tests. Outputs timings of tests marked with the
//! `#[perf]` attribute to stdout in Markdown. See the documentation of
//! `util_macros::perf` for usage details on the actual attribute.
//!
//! # Setup
//! Make sure `hyperfine` is installed and in the shell path.
//!
//! # Usage
//! Calling this tool rebuilds the targeted crate(s) with some cfg flags set for
//! the perf proc macro *and* enables optimisations (`release-fast` profile), so
//! expect it to take a little while.
//!
...
//! ```
//!
//! Similarly, to skip outputting progress to the command line, pass `--
//! --quiet`. These flags can be combined.
//!
//! ## Comparing runs
//! Passing `--json=ident` will save per-crate run files in `.perf-runs`, e.g.
//! `cargo perf-test -p gpui -- --json=blah` will result in
//! `.perf-runs/blah.gpui.json` being created (unless no tests were run). These
//! results can be automatically compared. To do so, run `cargo perf-compare
//! new-ident old-ident`.
//!
//! To save the markdown output to a file instead, run `cargo perf-compare
//! --save=$FILE new-ident old-ident`.
//!
//! NB: All files matching `.perf-runs/ident.*.json` will be considered when
//! doing this comparison, so ensure there aren't leftover files in your
//! `.perf-runs` directory that might match that!
//!
//! # Notes
//! This should probably not be called manually unless you're working on the
//! profiler itself; use the `cargo perf-test` alias (after building this crate)
//! instead.
...
/// Multiplier for the iteration count when a test doesn't pass the noise
/// cutoff.
    ...
    fn log(&self, output: &Output, t_bin: &str) {
        match self {
            ...
            OutputKind::Json(ident) => {
                ...
                // would be reduced to just "gpuim".
...
/// Triage a test to determine the correct number of iterations that it should
/// run. Specifically, repeatedly runs the given test until its execution time
/// exceeds `thresh`, calling `step(iterations)` after every failed run to
/// determine the new iteration count. Returns `None` if the test errored or
/// `step` returned `None`, else `Some(iterations)`.
///
/// # Panics
/// This will panic if `step(usize)` is not monotonically increasing, or if the
/// test binary is invalid.
fn triage_test(
    t_bin: &str, t_name: &str, thresh: Duration,
...
/// Profiles a given test with hyperfine, returning the mean and standard
/// deviation for its runtime. If the test errors, returns `None` instead.
