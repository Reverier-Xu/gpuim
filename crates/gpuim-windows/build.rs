...
mod shader_compilation {
    ...
    /// You can set the `GPUI_FXC_PATH` environment variable to specify the path
    /// to the fxc.exe compiler.
    fn find_fxc_compiler() -> String {
        ...
        // Try to find in PATH
        // NOTE: This has to be `where.exe` on Windows, not `where`, it must be ended
        // with `.exe`
    ...
    fn compile_shader_for_module(
        module: &str, out_dir: &str, fxc_path: &str, shader_path: &str, rust_binding_path: &str,
    ) {
    ...
    fn compile_shader_impl(
        fxc_path: &str, entry_point: &str, output_path: &str, var_name: &str, shader_path: &str,
        target: &str,
    ) {
