...
impl WindowsWindowInner {
    pub(crate) fn handle_msg(
        self: &Rc<Self>, handle: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM,
    ) -> LRESULT {
        let handled = match msg {
            ...
            // Treat double click as a second single click, since we track the double clicks
            // ourselves. If you don't interact with any elements, this will fall
            // through to the windows default behavior of toggling whether the window is
            // maximized.
    ...
    fn handle_size_msg(&self, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
        // Don't resize the renderer when the window is minimized, but record that it
        // was minimized so that on restore the swap chain can be recreated via
        // `update_drawable_size_even_if_unchanged`.
    ...
    fn handle_size_change(
        &self, device_size: Size<DevicePixels>, scale_factor: f32, should_resize_renderer: bool,
    ) {
    ...
    fn handle_syskeyup_msg(&self, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
        ...
        // Always return 0 to indicate that the message was handled, so we could
        // properly handle `ModifiersChanged` event.
        Some(0)
    }
    ...
    fn handle_mouse_down_msg(
        &self, handle: HWND, button: MouseButton, lparam: LPARAM,
    ) -> Option<isize> {
    ...
    fn handle_mouse_up_msg(
        &self, _handle: HWND, button: MouseButton, lparam: LPARAM,
    ) -> Option<isize> {
    ...
    fn handle_xbutton_msg(
        &self, handle: HWND, wparam: WPARAM, lparam: LPARAM,
    ...
    fn handle_activate_msg(self: &Rc<Self>, wparam: WPARAM) -> Option<isize> {
        ...
        // When the window is activated (gains focus), reset the modifier tracking
        // state. This fixes the issue where Alt-Tab away and back leaves stale
        // modifier state (especially the Alt key) because Windows doesn't
        // always send key-up events to windows that have lost focus.
        if activated {
    ...
    fn handle_hit_test_msg(&self, handle: HWND, lparam: LPARAM) -> Option<isize> {
        ...
        // We do not use the OS title bar, so the default `DefWindowProcW` will only
        // register a 1px edge for resizes We need to calculate the frame
        // thickness ourselves and do the hit test manually.
    ...
    fn handle_nc_mouse_down_msg(
        &self, handle: HWND, button: MouseButton, wparam: WPARAM, lparam: LPARAM,
    ) -> Option<isize> {
    ...
    fn handle_system_settings_changed(
        ...
        if wparam.0 != 0 {
            ...
            self.state.border_offset.update(handle).log_err();
            // system settings may emit a window message which wants to take the refcell
            // self.state, so drop it
    ...
    fn handle_system_theme_changed(&self, handle: HWND, lparam: LPARAM) -> Option<isize> {
        // lParam is a pointer to a string that indicates the area containing the system
...
fn handle_key_event<F>(
    wparam: WPARAM, lparam: LPARAM, state: &WindowsWindowState, f: F,
...
fn parse_normal_key(
    vkey: VIRTUAL_KEY, lparam: LPARAM, mut modifiers: Modifiers,
...
fn process_key(vkey: VIRTUAL_KEY, scan_code: u16) -> (Option<String>, bool) {
    ...
    // Workaround for some bug that makes the compiler think keyboard_state is still
    // zeroed out
...
fn should_use_ime_cursor_position(ctx: HIMC, cursor_pos: usize) -> bool {
    ...
    // Keep the cursor adjacent to the inserted text by only using the suggested
    // position if it's adjacent to unconverted text.
...
// - SM_CXPADDEDBORDER: Additional border space that isn't part of the resize
//   handle.
