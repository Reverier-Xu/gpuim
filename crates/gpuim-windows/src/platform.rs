...
pub struct WindowsPlatform {
    ...
    /// Flag to instruct the `VSyncProvider` thread to invalidate the directx
    /// devices as resizing them has failed, causing us to have lost at
    /// least the render target.
...
impl WindowsPlatform {
    pub fn new(headless: bool) -> Result<Self> {
        ...
        let (directx_devices, text_system, direct_write_text_system) = if !headless {
            ...
            (
                None,
                Arc::new(gpuim::NoopTextSystem::new()) as Arc<dyn PlatformTextSystem>,
                None,
            )
        };
...
impl Platform for WindowsPlatform {
    ...
    fn screen_capture_sources(
        ...
        gpuim::scap_screen_capture::scap_screen_sources(&self.foreground_executor)
    }
...
impl WindowsPlatformInner {
    ...
    /// Calls `project` to project to the corresponding callback field, removes
    /// it from callbacks, calls `f` with the callback and then puts the
    /// callback back.
    fn with_callback<T>(
        &self, project: impl Fn(&PlatformCallbacks) -> &Cell<Option<T>>, f: impl FnOnce(&mut T),
    ) {
        ...
        'tasks: loop {
            'timeout_loop: loop {
                if start.elapsed().as_millis() >= MAIN_TASK_TIMEOUT {
                    ...
                    // we spent our budget on gpui tasks, we likely have a lot of work queued so
                    // drain system events first to stay responsive
                    // then quit out of foreground work to allow us to process other gpui events
                    // first before returning back to foreground task work if we
                    // don't we might not for example process window quit events
                    let mut msg = MSG::default();
                    ...
                    // We need to process a paint message here as otherwise we will re-enter
                    // `run_foreground_task` before painting if we have work remaining.
                    // The reason for this is that windows prefers custom application message
                    // processing over system messages.
            ...
            // Someone could enqueue a Runnable here. The flag is still true, so they will
            // not PostMessage. We need to check for those Runnables after we
            // clear the flag.
            self.dispatcher.wake_posted.store(false, Ordering::Release);
            let mut main_receiver = self.main_receiver.clone();
            match main_receiver.try_pop() {
...
fn file_open_dialog(
    options: PathPromptOptions, window: Option<HWND>,
...
fn file_save_dialog(
    directory: PathBuf, suggested_name: Option<String>, window: Option<HWND>,
...
fn handle_gpu_device_lost(
    directx_devices: &mut DirectXDevices, platform_window: HWND, validation_number: usize,
    all_windows: &std::sync::Weak<RwLock<SmallVec<[SafeHwnd; 4]>>>,
    text_system: &std::sync::Weak<DirectWriteTextSystem>,
) -> Result<()> {
    // Here we wait a bit to ensure the system has time to recover from the device
    // lost state. If we don't wait, the final drawing result will be blank.
