...
impl WindowsWindowState {
    fn new(
        ...
        current_cursor: Option<HCURSOR>, display: WindowsDisplay, min_size: Option<Size<Pixels>>,
        appearance: WindowAppearance, disable_direct_composition: bool,
    ...
    ///
    /// Currently, GPUI uses the logical size of the app to handle mouse
    /// interactions (such as whether the mouse collides with other elements
    /// of GPUI).
...
impl WindowsWindow {
    pub(crate) fn new(
        handle: AnyWindowHandle, params: WindowParams, creation_info: WindowCreationInfo,
    ) -> Result<Self> {
...
impl PlatformWindow for WindowsWindow {
    ...
    fn resize(&mut self, size: Size<Pixels>) {
        ...
        let bounds =
            gpuim::bounds(self.bounds().origin, size).to_device_pixels(self.scale_factor());
...
impl IDropTarget_Impl for WindowsDragDropHandler_Impl {
    fn DragEnter(
        &self, pdataobj: windows::core::Ref<IDataObject>, _grfkeystate: MODIFIERKEYS_FLAGS,
        pt: &POINTL, pdweffect: *mut DROPEFFECT,
    ) -> windows::core::Result<()> {
        unsafe {
    ...
    fn DragOver(
        &self, _grfkeystate: MODIFIERKEYS_FLAGS, pt: &POINTL, pdweffect: *mut DROPEFFECT,
    ) -> windows::core::Result<()> {
...
fn calculate_client_rect(
    rect: RECT, border_offset: &WindowBorderOffset, scale_factor: f32,
) -> Bounds<Pixels> {
    let left_offset = border_offset.width_offset.get() / 2;
    let top_offset = border_offset.height_offset.get() / 2;
    let right_offset = border_offset.width_offset.get() - left_offset;
...
fn retrieve_window_placement(
    hwnd: HWND, display: WindowsDisplay, initial_bounds: Bounds<Pixels>, scale_factor: f32,
...
// When the platform title bar is hidden, Windows may think that our application
// is meant to appear 'fullscreen' and will stop the taskbar from appearing on
// top of our window. Prevent this. https://devblogs.microsoft.com/oldnewthing/20250522-00/?p=111211
...
mod tests {
    ...
    use gpuim::{DevicePixels, MouseButton, point};
