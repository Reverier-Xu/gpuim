...
use gpuim::{Action, MenuItem, SharedString};
...
pub(crate) fn update_jump_list(
    recent_workspaces: &[SmallVec<[PathBuf; 2]>], dock_menus: &[(SharedString, SharedString)],
...
fn add_dock_menu(
    list: &ICustomDestinationList, dock_menus: &[(SharedString, SharedString)],
) -> anyhow::Result<()> {
    unsafe {
...
fn add_recent_folders(
    list: &ICustomDestinationList, entries: &[SmallVec<[PathBuf; 2]>],
...
fn create_shell_link(
    argument: HSTRING, description: HSTRING, icon: Option<HSTRING>, display: &str,
