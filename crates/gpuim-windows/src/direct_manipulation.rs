...
impl DirectManipulationEventHandler {
    fn new(
        window: HWND, scale_factor: Rc<Cell<f32>>, pending_events: Rc<RefCell<Vec<PlatformInput>>>,
    ) -> Self {
        Self {
            window,
            scale_factor,
...
impl IDirectManipulationViewportEventHandler_Impl for DirectManipulationEventHandler_Impl {
    fn OnViewportStatusChanged(
        &self, viewport: windows_core::Ref<'_, IDirectManipulationViewport>,
        current: DIRECTMANIPULATION_STATUS, previous: DIRECTMANIPULATION_STATUS,
    ) -> windows_core::Result<()> {
        if current == previous {
            return Ok(());
        }
        ...
        if current == DIRECTMANIPULATION_READY {
            ...
            // ZoomToRect triggers a second RUNNING -> READY cycle, so prevent an infinite
            // loop here.
    ...
    fn OnViewportUpdated(
        &self, _viewport: windows_core::Ref<'_, IDirectManipulationViewport>,
    ) -> windows_core::Result<()> {
        Ok(())
    }
    ...
    fn OnContentUpdated(
        ...
        // Direct Manipulation reports both translation and scale in every content
        // update. Translation values can shift during a pinch due to the zoom
        // center shifting. We classify each gesture as either scroll or pinch
        // and only emit one type of event. We allow Scroll -> Pinch (a pinch
        // can start with a small pan) but not the reverse.
