...
pub use gpuim::Platform;

/// Returns a background executor for the current platform.
pub fn background_executor() -> gpuim::BackgroundExecutor {
    current_platform(true).background_executor()
}

pub fn application() -> gpuim::Application {
    gpuim::Application::with_platform(current_platform(false))
}

pub fn headless() -> gpuim::Application {
    gpuim::Application::with_platform(current_platform(true))
}

/// Unlike `application`, this function returns a single-threaded web
/// application.
#[cfg(target_family = "wasm")]
pub fn single_threaded_web() -> gpuim::Application {
    gpuim::Application::with_platform(Rc::new(gpuim_web::WebPlatform::new(false)))
}
...
pub fn current_platform(headless: bool) -> Rc<dyn Platform> {
    #[cfg(target_os = "macos")]
    {
        Rc::new(gpuim_macos::MacPlatform::new(headless))
    }

    #[cfg(target_os = "windows")]
    {
        Rc::new(
            gpuim_windows::WindowsPlatform::new(headless)
    ...
    {
        gpuim_linux::current_platform(headless)
    }

    #[cfg(target_family = "wasm")]
    {
        let _ = headless;
        Rc::new(gpuim_web::WebPlatform::new(true))
    }
}

/// Returns a new [`HeadlessRenderer`] for the current platform, if available.
#[cfg(feature = "test-support")]
pub fn current_headless_renderer() -> Option<Box<dyn gpuim::PlatformHeadlessRenderer>> {
    #[cfg(target_os = "macos")]
    {
        Some(Box::new(
            gpuim_macos::metal_renderer::MetalHeadlessRenderer::new(),
        ))
    }
...
mod tests {
    ...
    use gpuim::{AppContext, Empty, VisualTestAppContext};

    use super::*;

    // Note: All VisualTestAppContext tests are ignored by default because they
    // require the macOS main thread. Standard Rust tests run on worker threads,
    // which causes SIGABRT when interacting with macOS AppKit/Cocoa APIs.
    //
