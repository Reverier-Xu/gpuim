...
/// A macro used in tests for cross-platform path string literals in tests. On
/// Windows it replaces `/` with `\\` and adds `C:` to the beginning of absolute
/// paths. On other platforms, the path is returned unmodified.
///
/// # Example
/// ```rust
...
struct PerfArgs {
    ...
    /// How much this test's results should be weighed when comparing across
    /// runs. If unspecified, defaults to `WEIGHT_DEFAULT` (50).
...
/// There are 5 levels of importance (`Critical`, `Important`, `Average`,
/// `Iffy`, `Fluff`); see the documentation on `Importance` for details. Add the
/// importance as a parameter to override the default (e.g.
/// `#[perf(important)]`).
///
/// Each test also has a weight factor. This is irrelevant on its own, but is
/// considered when comparing results across different runs. By default, this is
/// set to 50; pass `weight = n` as a parameter to override this. Note that this
/// value is only relevant within its importance category.
///
/// By default, the number of iterations when profiling this test is
/// auto-determined. If this needs to be overwritten, pass the desired iteration
/// count as a parameter (`#[perf(iterations = n)]`). Note that the actual
/// profiler may still run the test an arbitrary number times; this flag just
/// sets the number of executions before the process is restarted and global
/// state is reset.
///
/// This attribute should probably not be applied to tests that do any
/// significant disk IO, as locks on files may not be released in time when
/// repeating a test many times. This might lead to spurious failures.
///
/// # Examples
/// ```rust
...
/// ```
///
/// This also works with `#[gpuim::test]`s, though in most cases it shouldn't
...
/// #[gpuim::test]
/// fn oneshot_test(_cx: &mut gpuim::TestAppContext) {
///     // Test goes here.
/// }
/// ```
