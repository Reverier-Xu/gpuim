...
use std::{cmp::Ordering, fmt, iter::FromIterator, marker::PhantomData, mem, sync::Arc};
...
// Helper for when we cannot use ArrayVec::<T>::push().unwrap() as T doesn't
// impl Debug
...
/// A type that describes the Sum of all [`Item`]s in a subtree of the
/// [`SumTree`]
///
...
/// Catch-all implementation for when you need something that implements
/// [`Summary`] without a specific type. We implement it on a `NoSummary`
/// instead of re-using `()`, as that avoids blanket impl collisions with
/// `impl<T: Summary> Dimension for T` (as we also need unit type to be a
/// fill-in dimension)
...
/// Each [`Summary`] type can have more than one [`Dimension`] type that it
/// measures.
///
/// You can use dimensions to seek to a specific location in the [`SumTree`]
///
/// # Example:
/// Zed's rope has a `TextSummary` type that summarizes lines, characters, and
/// bytes. Each of these are different dimensions we may want to seek to
...
/// Bias is used to settle ambiguities when determining positions in an ordered
/// sequence.
///
...
/// - The left anchor of the selection has [Bias::Right], attaching it to the
///   character `B`
/// - The right anchor of the selection has [Bias::Left], attaching it to the
///   character `C`
///
/// Given the buffer `{ˇ<...>`, where `<...>` is a folded region:
/// - The display offset of the cursor is 1, but the offset in the buffer is
///   determined by the bias
/// - [Bias::Left] would attach the cursor to the character `{`, with a buffer
///   offset of 1
/// - [Bias::Right] would attach the cursor to the first character of the folded
...
/// A B+ tree in which each leaf node contains `Item`s of type `T` and a
/// `Summary`s for each `Item`. Each internal node contains a `Summary` of the
/// items in its subtree.
///
/// The maximum number of items per node is `TREE_BASE * 2`.
///
/// Any [`Dimension`] supported by the [`Summary`] type can be used to seek to a
/// specific location in the tree.
#[derive(Clone)]
...
impl<T: Item> SumTree<T> {
    ...
    /// Useful in cases where the item type has a non-trivial context type, but
    /// the zero value of the summary type doesn't depend on that context.
    ...
    pub fn from_iter<I: IntoIterator<Item = T>>(
        iter: I, cx: <T::Summary as Summary>::Context<'_>,
    ) -> Self {
    ...
    /// A more efficient version of `Cursor::new()` + `Cursor::seek()` +
    /// `Cursor::item()`.
    ///
    /// Only returns the item that exactly has the target match.
    #[instrument(skip_all)]
    pub fn find_exact<'a, 'slf, D, Target>(
        &'slf self, cx: <T::Summary as Summary>::Context<'a>, target: &Target, bias: Bias,
    ) -> (D, D, Option<&'slf T>)
    where
    ...
    fn find_iterate<'tree, 'a, D, Target, const EXACT: bool>(
        cx: <T::Summary as Summary>::Context<'a>, target: &Target, bias: Bias, position: &mut D,
    ...
    pub fn cursor<'a, 'b, D>(
        &'a self, cx: <T::Summary as Summary>::Context<'b>,
    ) -> Cursor<'a, 'b, T, D>
    where
    ...
    /// Note: If the summary type requires a non `()` context, then the filter
    /// cursor that is returned cannot be used with Rust's iterators.
    pub fn filter<'a, 'b, F, U>(
        &'a self, cx: <T::Summary as Summary>::Context<'b>, filter_node: F,
    ...
    pub fn update_last(
        &mut self, f: impl FnOnce(&mut T), cx: <T::Summary as Summary>::Context<'_>,
    ) {
    ...
    pub fn extent<'a, D: Dimension<'a, T::Summary>>(
        &'a self, cx: <T::Summary as Summary>::Context<'_>,
    ) -> D {
    ...
    fn push_tree_recursive(
        &mut self, other: SumTree<T>, cx: <T::Summary as Summary>::Context<'_>,
    ) -> Option<SumTree<T>> {
        match Arc::make_mut(&mut self.0) {
            Node::Internal {
                height,
    ...
    // appends the `large` tree to a `small` tree, assumes small.height() <=
    // large.height()
    fn append_large(
        small: Self, large: &mut Self, cx: <T::Summary as Summary>::Context<'_>,
    ) -> Option<Self> {
    ...
    fn merge_into_right(
        small: Self, large: &mut Self, cx: <<T as Item>::Summary as Summary>::Context<'_>,
    ) -> Option<SumTree<T>> {
    ...
    fn from_child_trees(
        left: SumTree<T>, right: SumTree<T>, cx: <T::Summary as Summary>::Context<'_>,
    ) -> Self {
...
impl<T: KeyedItem> SumTree<T> {
    pub fn insert_or_replace<'a, 'b>(
        &'a mut self, item: T, cx: <T::Summary as Summary>::Context<'b>,
    ) -> Option<T> {
    ...
    pub fn edit(
        &mut self, mut edits: Vec<Edit<T>>, cx: <T::Summary as Summary>::Context<'_>,
    ) -> Vec<T> {
        if edits.is_empty() {
            return Vec::new();
        }
    ...
    pub fn get<'a>(
        &'a self, key: &T::Key, cx: <T::Summary as Summary>::Context<'a>,
    ) -> Option<&'a T> {
