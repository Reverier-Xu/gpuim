...
impl WgpuRenderer {
    ...
    ///
    /// The `gpu_context` is a shared reference that coordinates GPU context
    /// across multiple windows. The first window to create a renderer will
    /// initialize the context; subsequent windows will share it.
    ///
    /// # Safety
    ...
    #[cfg(not(target_family = "wasm"))]
    pub fn new<W>(
        gpu_context: GpuContext, window: &W, config: WgpuSurfaceConfig,
        compositor_gpu: Option<CompositorGpuHint>,
    ) -> anyhow::Result<Self>
    where
    ...
    pub fn new_from_canvas(
        context: &WgpuContext, canvas: &web_sys::HtmlCanvasElement, config: WgpuSurfaceConfig,
    ) -> anyhow::Result<Self> {
        let surface = context
            .instance
    ...
    fn new_internal(
        gpu_context: Option<GpuContext>, context: &WgpuContext, surface: wgpu::Surface<'static>,
        config: WgpuSurfaceConfig, compositor_gpu: Option<CompositorGpuHint>,
        atlas: Arc<WgpuAtlas>,
    ) -> anyhow::Result<Self> {
        ...
        // Configure the surface immediately. The adapter selection process already
        // validated that this adapter can successfully configure this surface.
        ...
        let resources = WgpuResources {
            ...
            instance_buffer,
            // Defer intermediate texture creation to first draw call via
            // ensure_intermediate_textures(). This avoids panics when the
            // device/surface is in an invalid state during initialization.
    ...
    fn create_pipelines(
        device: &wgpu::Device, layouts: &WgpuBindGroupLayouts, surface_format: wgpu::TextureFormat,
        alpha_mode: wgpu::CompositeAlphaMode, path_sample_count: u32, dual_source_blending: bool,
    ) -> WgpuPipelines {
    ...
    fn draw_quads(
        &self, quads: &[Quad], instance_offset: &mut u64, pass: &mut wgpu::RenderPass<'_>,
    ) -> bool {
    ...
    fn draw_shadows(
        &self, shadows: &[Shadow], instance_offset: &mut u64, pass: &mut wgpu::RenderPass<'_>,
    ) -> bool {
    ...
    fn draw_underlines(
        &self, underlines: &[Underline], instance_offset: &mut u64, pass: &mut wgpu::RenderPass<'_>,
    ) -> bool {
    ...
    fn draw_monochrome_sprites(
        &self, sprites: &[MonochromeSprite], texture_id: AtlasTextureId, instance_offset: &mut u64,
    ...
    fn draw_subpixel_sprites(
        &self, sprites: &[SubpixelSprite], texture_id: AtlasTextureId, instance_offset: &mut u64,
        pass: &mut wgpu::RenderPass<'_>,
    ) -> bool {
        let tex_info = self.atlas.get_texture_info(texture_id);
        let data = unsafe { Self::instance_bytes(sprites) };
    ...
    fn draw_polychrome_sprites(
        &self, sprites: &[PolychromeSprite], texture_id: AtlasTextureId, instance_offset: &mut u64,
        pass: &mut wgpu::RenderPass<'_>,
    ) -> bool {
        let tex_info = self.atlas.get_texture_info(texture_id);
        let data = unsafe { Self::instance_bytes(sprites) };
    ...
    fn draw_instances(
        &self, data: &[u8], instance_count: u32, pipeline: &wgpu::RenderPipeline,
        instance_offset: &mut u64, pass: &mut wgpu::RenderPass<'_>,
    ) -> bool {
        if instance_count == 0 {
            return true;
        }
    ...
    fn draw_instances_with_texture(
        &self, data: &[u8], instance_count: u32, texture_view: &wgpu::TextureView,
        pipeline: &wgpu::RenderPipeline, instance_offset: &mut u64,
        pass: &mut wgpu::RenderPass<'_>,
    ) -> bool {
        if instance_count == 0 {
            return true;
    ...
    fn draw_paths_from_intermediate(
        &self, paths: &[Path<ScaledPixels>], instance_offset: &mut u64,
        pass: &mut wgpu::RenderPass<'_>,
    ) -> bool {
    ...
    fn draw_paths_to_intermediate(
        &self, encoder: &mut wgpu::CommandEncoder, paths: &[Path<ScaledPixels>],
    ...
    fn write_to_instance_buffer(
        &self, instance_offset: &mut u64, data: &[u8],
    ...
    pub fn replace_surface<W: HasWindowHandle>(
        &mut self, window: &W, config: WgpuSurfaceConfig, instance: &wgpu::Instance,
    ) -> anyhow::Result<()> {
        let window_handle = window
            .window_handle()
            .map_err(|e| anyhow::anyhow!("Failed to get window handle: {e}"))?;
    ...
    /// Recovers from a lost GPU device by recreating the renderer with a new
    /// context.
    ///
        ...
        W: HasWindowHandle + HasDisplayHandle + std::fmt::Debug + Send + Sync + Clone + 'static,
    {
        ...
        let config = WgpuSurfaceConfig {
            size: gpuim::Size {
                width: gpuim::DevicePixels(self.surface_config.width as i32),
                height: gpuim::DevicePixels(self.surface_config.height as i32),
            },
...
#[cfg(not(target_family = "wasm"))]
fn create_surface(
    instance: &wgpu::Instance, raw_window_handle: raw_window_handle::RawWindowHandle,
