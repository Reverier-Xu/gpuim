...
struct CosmicTextSystemState {
    ...
    /// Contains all already loaded fonts, including all faces. Indexed by
    /// `FontId`.
    loaded_fonts: Vec<LoadedFont>,
    /// Caches the `FontId`s associated with a specific family to avoid
    /// iterating the font database for every font face in a family.
...
impl CosmicTextSystemState {
    ...
    fn load_family(
        ...
        let name = gpuim::font_name_with_fallbacks(name, &self.system_font_fallback);
        ...
        for (font_id, postscript_name) in families {
            ...
            // HACK: To let the storybook run and render Windows caption icons. We should
            // actually do better font fallback.
    ...
    #[profiling::function]
    fn rasterize_glyph(
        &mut self, params: &RenderGlyphParams, glyph_bounds: Bounds<DevicePixels>,
    ) -> Result<(Size<DevicePixels>, Vec<u8>)> {
    ...
    fn render_glyph_image(
        &mut self, params: &RenderGlyphParams,
    ...
    /// This is used when cosmic_text has chosen a fallback font instead of
    /// using the requested font, typically to handle some unicode
    /// characters. When this happens, `loaded_fonts` may not yet have an
    /// entry for this fallback font, and so one is added.
    ///
    /// Note that callers shouldn't use this `FontId` somewhere that will
    /// retrieve the corresponding `LoadedFont.features`, as it will have an
    /// arbitrarily chosen or empty value. The only current use of this
    /// field is for the *input* of `layout_line`, and so it's fine to use
...
fn find_best_match(
    ...
    let target_italic = matches!(
        font.style,
        gpuim::FontStyle::Italic | gpuim::FontStyle::Oblique
    );
...
#[cfg(feature = "font-kit")]
fn font_into_properties(font: &gpuim::Font) -> font_kit::properties::Properties {
    font_kit::properties::Properties {
        style: match font.style {
            gpuim::FontStyle::Normal => font_kit::properties::Style::Normal,
            gpuim::FontStyle::Italic => font_kit::properties::Style::Italic,
            gpuim::FontStyle::Oblique => font_kit::properties::Style::Oblique,
        },
