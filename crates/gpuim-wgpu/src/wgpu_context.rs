...
impl WgpuContext {
    ...
    pub fn new(
        ...
        // Select an adapter by actually testing surface configuration with the real
        // device. This is the only reliable way to determine compatibility on
        // hybrid GPU systems.
    ...
    /// Select an adapter and create a device, testing that the surface can
    /// actually be configured. This is the only reliable way to determine
    /// compatibility on hybrid GPU systems, where adapters may report
    /// surface compatibility via get_capabilities() but fail when actually
    /// configuring (e.g., NVIDIA reporting Vulkan Wayland support but failing
    /// because the Wayland compositor runs on the Intel GPU).
    #[cfg(not(target_family = "wasm"))]
    async fn select_adapter_and_device(
        instance: &wgpu::Instance, device_id_filter: Option<u32>, surface: &wgpu::Surface<'_>,
        compositor_gpu: Option<&CompositorGpuHint>,
    ) -> anyhow::Result<(
        wgpu::Adapter,
        wgpu::Device,
        ...
        // 3. Device type (Discrete > Integrated > Other > Virtual > Cpu). "Other" ranks
        //    above "Virtual" because OpenGL seems to count as "Other".
    ...
    /// Try to use an adapter with a surface by creating a device and testing
    /// configuration. Returns the device and queue if successful, allowing
    /// them to be reused.
    #[cfg(not(target_family = "wasm"))]
    async fn try_adapter_with_surface(
        adapter: &wgpu::Adapter, surface: &wgpu::Surface<'_>,
    ) -> anyhow::Result<(wgpu::Device, wgpu::Queue, bool, TextureFormat)> {
    ...
    /// Returns true if the GPU device was lost (e.g., due to driver crash,
    /// suspend/resume). When this returns true, the context should be
    /// recreated.
