//! Implements `#[derive_inspector_reflection]` macro to provide runtime access
//! to trait methods that have the shape `fn method(self) -> Self`. This code
//! was generated using Zed Agent with Claude Opus 4.
...
fn generate_reflected_trait(trait_item: ItemTrait) -> TokenStream {
    ...
    let inspector_reflection_path = if is_called_from_gpui_crate(call_site) {
        ...
        quote! { ::gpuim::inspector_reflection }
    };

    // Collect method information for methods of form fn name(self) -> Self or fn
    // name(mut self) -> Self
...
fn is_called_from_gpui_crate(_span: Span) -> bool {
    // Check if we're being called from within the gpui crate by examining the call
    // site This is a heuristic approach - we check if the current crate name is
    // "gpuim"
    std::env::var("CARGO_PKG_NAME").is_ok_and(|name| name == "gpuim")
}
...
fn try_expand_macro(macro_item: &syn::TraitItemMacro) -> Option<Vec<TraitItem>> {
    ...
    match macro_name.as_str() {
        "gpuim_macros::style_helpers" | "style_helpers" => {
