...
/// This can be used to register an action with the GPUI runtime when you want
/// to manually implement the `Action` trait. Typically you should use the
/// `Action` derive macro or `actions!` macro instead.
#[proc_macro]
...
/// Note that a `#[app]` attribute is required to identify the variable holding
/// the &mut App.
///
/// Failure to add the attribute causes a compile error:
///
/// ```compile_fail
/// # #[macro_use] extern crate gpuim_macros;
/// # #[macro_use] extern crate gpuim;
/// #[derive(AppContext)]
/// struct MyContext<'a> {
///     app: &'a mut gpuim::App
/// }
...
/// Note that a `#[app]` and a `#[window]` attribute are required to identify
/// the variables holding the &mut App, and &mut Window respectively.
///
...
/// `#[gpuim::test]` can be used to annotate test functions that run with GPUI
/// support.
///
/// It supports both synchronous and asynchronous tests, and can provide you
/// with as many `TestAppContext` instances as you need.
/// The output contains a `#[test]` annotation so this can be used with any
/// existing test harness (`cargo test` or `cargo-nextest`).
///
/// ```
/// #[gpuim::test]
...
/// ```
///
/// In addition to passing a TestAppContext, you can also ask for a `StdRnd`
/// instance. this will be seeded with the `SEED` environment variable and is
/// used internally by the ForegroundExecutor and BackgroundExecutor to run
/// tasks deterministically in tests. Using the same `StdRng` for behavior in
/// your test will allow you to exercise a wide variety of scenarios and
/// interleavings just by changing the seed.
///
/// # Arguments
///
/// - `#[gpuim::test]` with no arguments runs once with the seed `0` or `SEED`
///   env var if set.
/// - `#[gpui::test(seed = 10)]` runs once with the seed `10`.
/// - `#[gpui::test(seeds(10, 20, 30))]` runs three times with seeds `10`, `20`,
///   and `30`.
/// - `#[gpui::test(iterations = 5)]` runs five times, providing as seed the
///   values in the range `0..5`.
/// - `#[gpui::test(retries = 3)]` runs up to four times if it fails to try and
///   make it pass.
/// - `#[gpui::test(on_failure = "crate::test::report_failure")]` will call the
///   specified function after the tests fail so that you can write out more
///   detail about the failure.
///
/// You can combine `iterations = ...` with `seeds(...)`:
/// - `#[gpui::test(iterations = 5, seed = 10)]` is equivalent to
///   `#[gpui::test(seeds(0, 1, 2, 3, 4, 10))]`.
/// - `#[gpui::test(iterations = 5, seeds(10, 20, 30)]` is equivalent to
///   `#[gpui::test(seeds(0, 1, 2, 3, 4, 10, 20, 30))]`.
/// - `#[gpui::test(seeds(10, 20, 30), iterations = 5]` is equivalent to
///   `#[gpui::test(seeds(0, 1, 2, 3, 4, 10, 20, 30))]`.
///
...
/// A variant of `#[gpuim::test]` that supports property-based testing.
///
...
///   file, which can be checked in, and future runs will check these cases
///   before future cases.
///
...
/// some of the same GPUI-specific arguments as `#[gpuim::test]`. Specifically,
...
///
/// Note: much of the following is copied from the proptest docs, specifically
/// the [`#[proptest::property_test]`] macro docs.
///
...
/// `Arbitrary`. Parameters to a `#[gpuim::property_test]`, by default, use a
...
/// ## Scheduler
///
/// Similar to `#[gpuim::test]`, this macro will choose random seeds for the
/// test scheduler. It uses `.no_shrink()` to tell proptest that all seeds are
...
/// When added to a trait, `#[derive_inspector_reflection]` generates a module
/// which provides enumeration and lookup by name of all methods that have the
/// shape `fn method(self) -> Self`. This is used by the inspector so that it
/// can use the builder methods in `Styled` and `StyledExt`.
///
/// The generated module will have the name `<snake_case_trait_name>_reflection`
/// and contain the following functions:
///
/// ```ignore
/// pub fn methods::<T: TheTrait + 'static>() -> Vec<gpuim::inspector_reflection::FunctionReflection<T>>;
///
/// pub fn find_method::<T: TheTrait + 'static>() -> Option<gpuim::inspector_reflection::FunctionReflection<T>>;
/// ```
///
/// The `invoke` method on `FunctionReflection` will run the method.
/// `FunctionReflection` also provides the method's documentation.
#[cfg(any(feature = "inspector", debug_assertions))]
#[proc_macro_attribute]
