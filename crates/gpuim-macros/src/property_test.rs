...
pub fn test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let Ok(func) = parse2::<ItemFn>(item) else {
        return quote_spanned! { item_span =>
            compile_error!("#[gpuim::property_test] must be placed on a function");
        };
    };
    ...
    let proptest_args = quote! {
        #[strategy = ::gpuim::seed_strategy()] __seed: u64,
        #proptest_args
    };
    ...
    let run_test_body = match &asyncness {
        ...
        Some(_) => quote! {
            let foreground_executor = gpuim::ForegroundExecutor::new(std::sync::Arc::new(dispatcher.clone()));
            #cx_vars
...
impl Args {
    ...
    fn render(&self) -> TokenStream {
        let user_provided_config = match &self.config {
            None => quote! { ::gpuim::proptest::prelude::ProptestConfig::default() },
            Some(config) => config.into_token_stream(),
        };

        let fixed_config = quote!(::gpuim::apply_seed_to_proptest_config(#user_provided_config));
        let remaining_args = &self.remaining_args;
        let errors = &self.errors;

        quote! {
            #errors
            #[::gpuim::proptest::property_test(
                proptest_path = "::gpuim::proptest",
                config = #fixed_config,
                #(#remaining_args,)*
            )]
        }
...
fn remove_cxs(parsed: &mut ParsedArgs, args: &mut Vec<FnArg>, test_name: &Ident) {
    ...
    args.retain_mut(|arg| {
        ...
        parsed.cx_vars.extend(quote!(
            let mut #cx_varname = gpuim::TestAppContext::build(
                dispatcher.clone(),
...
fn remove_background_executor(parsed: &mut ParsedArgs, args: &mut Vec<FnArg>) {
    args.retain_mut(|arg| {
        ...
        parsed
            .inner_fn_args
            .extend(quote!(gpuim::BackgroundExecutor::new(std::sync::Arc::new(
                dispatcher.clone()
            )),));
