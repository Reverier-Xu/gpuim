...
pub(crate) fn derive_action(input: TokenStream) -> TokenStream {
    ...
    */
    ...
    let build_fn_body = if no_json {
        ...
        quote! { Err(gpuim::private::anyhow::anyhow!(#error_msg)) }
    } else if is_unit_struct {
        quote! { Ok(Box::new(Self)) }
    } else {
        quote! { Ok(Box::new(gpuim::private::serde_json::from_value::<Self>(_value)?)) }
    };

    let json_schema_fn_body = if no_json || is_unit_struct {
        quote! { None }
    } else {
        quote! { Some(<Self as gpuim::private::schemars::JsonSchema>::json_schema(_generator)) }
    };
    ...
    TokenStream::from(quote! {
        ...
        impl gpuim::Action for #struct_name {
            ...
            fn partial_eq(&self, action: &dyn gpuim::Action) -> bool {
                action
                    .as_any()
                    .downcast_ref::<Self>()
            ...
            fn build(_value: gpuim::private::serde_json::Value) -> gpuim::Result<Box<dyn gpuim::Action>> {
                #build_fn_body
            }

            fn action_json_schema(
                _generator: &mut gpuim::private::schemars::SchemaGenerator,
            ) -> Option<gpuim::private::schemars::Schema> {
                #json_schema_fn_body
            }
