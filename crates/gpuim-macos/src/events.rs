...
pub(crate) unsafe fn platform_input_from_native(
    native_event: id, window_height: Option<Pixels>,
    ...
    unsafe {
        ...
        let key = match first_char {
            ...
            _ => {
                ...
                // * Armenian         s | ս    | cmd-s | cmd-shift-s  (layout is non-ASCII, so
                //   we use cmd layout)
                // * Dvorak+QWERTY    s | o    | cmd-s | cmd-shift-s  (layout switches on cmd)
                // * Ukrainian+QWERTY s | с    | cmd-s | cmd-shift-s  (macOS reports cmd-s
                //   instead of cmd-S)
                // * Czech            7 | ý    | cmd-ý | cmd-7        (layout has shifted
                //   numbers)
                // * Norwegian        7 | 7    | cmd-7 | cmd-/        (macOS reports cmd-shift-7
                //   instead of cmd-/)
                // * Russian          7 | 7    | cmd-7 | cmd-&        (shift-7 is . but when cmd
                //   is down, should use cmd layout)
                // * German QWERTZ    ; | ö    | cmd-ö | cmd-Ö        (Zed's shift special case
                //   only applies to a-z)
                //
                ...
                if command || always_use_cmd_layout {
                    ...
                    // Handle edge-case where cmd-shift-s reports cmd-s instead
                    // of cmd-shift-s (Ukrainian, etc.)
