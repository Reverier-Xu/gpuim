...
use gpuim::{KeybindingKeystroke, Keystroke, PlatformKeyboardLayout, PlatformKeyboardMapper};
...
// On some keyboards (e.g. German QWERTZ) it is not possible to type the full
// ASCII range without using option. This means that some of our built in
// keyboard shortcuts do not work for those users.
//
// The way macOS solves this problem is to move shortcuts around so that they
// are all reachable, even if the mnemonic changes. https://developer.apple.com/documentation/swiftui/keyboardshortcut/localization-swift.struct
//
// For example, cmd-> is the "switch window" shortcut because the > key is right
// above tab. To ensure this doesn't cause problems for shortcuts defined for a
// QWERTY layout, apple moves any shortcuts defined as cmd-> to cmd-:.
// Coincidentally this s also the same keyboard position as cmd-> on a QWERTY
// layout.
//
// Another example is cmd-[ and cmd-], as they cannot be typed without option,
// those keys are remapped to cmd-ö and cmd-ä. These shortcuts are not in the
// same position as a QWERTY keyboard, because on a QWERTZ keyboard the + key is
// in the way; and shortcuts bound to cmd-+ are still typed as cmd-+ on either
// keyboard (though the specific key moves)
//
// As far as I can tell, there's no way to query the mappings Apple uses except
// by rendering a menu with every possible key combination, and inspecting the
// UI to see what it rendered. So that's what we did...
//
// These mappings were generated by running https://github.com/ConradIrwin/keyboard-inspector, tidying up the
// output to remove languages with no mappings and other oddities, and
// converting it to a less verbose representation with:  jq -s 'map(to_entries |
// map({key: .key, value: [(.value | to_entries | map(.key) | join("")), (.value
// | to_entries | map(.value) | join(""))]}) | from_entries) | add' From there I
// used multi-cursor to produce this match statement.
