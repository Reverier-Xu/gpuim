...
impl MacPlatform {
    pub fn new(headless: bool) -> Self {
        ...
        let text_system = Arc::new(gpuim::NoopTextSystem::new());
    ...
    unsafe fn create_menu_bar(
        &self, menus: &Vec<Menu>, delegate: id, actions: &mut Vec<Box<dyn Action>>, keymap: &Keymap,
    ) -> id {
        unsafe {
    ...
    unsafe fn create_dock_menu(
        &self, menu_items: Vec<MenuItem>, delegate: id, actions: &mut Vec<Box<dyn Action>>,
        keymap: &Keymap,
    ) -> id {
        unsafe {
    ...
    unsafe fn create_menu_item(
        item: &MenuItem, delegate: id, actions: &mut Vec<Box<dyn Action>>, keymap: &Keymap,
    ) -> id {
            ...
            match item {
                ...
                MenuItem::Action {
                    ...
                    let selector = match os_action {
                        Some(gpuim::OsAction::Cut) => selector("cut:"),
                        Some(gpuim::OsAction::Copy) => selector("copy:"),
                        Some(gpuim::OsAction::Paste) => selector("paste:"),
                        Some(gpuim::OsAction::SelectAll) => selector("selectAll:"),
                        // "undo:" and "redo:" are always disabled in our case, as
                        // we don't have a NSTextView/NSTextField to enable them on.
                        Some(gpuim::OsAction::Undo) => selector("handleGPUIMenuItem:"),
                        Some(gpuim::OsAction::Redo) => selector("handleGPUIMenuItem:"),
...
impl Platform for MacPlatform {
    ...
    fn foreground_executor(&self) -> gpuim::ForegroundExecutor {
    ...
    fn quit(&self) {
        // Quitting the app causes us to close windows, which invokes `Window::on_close`
        // callbacks synchronously before this method terminates. If we call
        // `Platform::quit` while holding a borrow of the app state (which most
        // of the time we will do), we will end up double-borrowing the app
        // state in the `on_close` callbacks for our open windows. To solve
        // this, we make quitting the application asynchronous so that we aren't holding
        // borrows to the app state on the stack when we actually terminate the
        // app.
    ...
    fn screen_capture_sources(
        &self,
    ) -> oneshot::Receiver<Result<Vec<Rc<dyn gpuim::ScreenCaptureSource>>>> {
                        ...
                        if response == NSModalResponse::NSModalResponseOk {
                            ...
                            if url.isFileURL() == YES {
                                result = ns_url_to_path(panel.URL()).ok().map(|mut result| {
                                    ...
                                    // Workaround a bug in macOS Sequoia that adds an extra
                                    // file-extension sometimes.
                                    // e.g. `a.sql` becomes `a.sql.s` or `a.txtx` becomes
                                    // `a.txtx.txt`
                                    //
                                    // This is conditional on OS version because I'd like to get rid
                                    // of it, so that
                                    // you can manually create a file called `a.sql.s`. That said it
                                    // seems better
    ...
    fn write_credentials(&self, url: &str, username: &str, password: &[u8]) -> Task<Result<()>> {
        ...
        self.background_executor().spawn(async move {
            unsafe {
                ...
                // First, check if there are already credentials for the given server. If so,
...
extern "C" fn on_thermal_state_change(this: &mut Object, _: Sel, _: id) {
    // Defer to the next run loop iteration to avoid re-entrant borrows of the App
    // RefCell, as NSNotificationCenter delivers this notification synchronously
    // and it may fire while the App is already borrowed (same pattern as quit()
    // above).
    let platform = unsafe { get_mac_platform(this) };
...
unsafe extern "C" {
    ...
    pub(super) fn TISGetInputSourceProperty(
        inputSource: *mut Object, propertyKey: *const c_void,
    ) -> *mut Object;

    pub(super) fn UCKeyTranslate(
        keyLayoutPtr: *const ::std::os::raw::c_void, virtualKeyCode: u16, keyAction: u16,
        modifierKeyState: u32, keyboardType: u32, keyTranslateOptions: u32, deadKeyState: *mut u32,
        maxStringLength: usize, actualStringLength: *mut usize, unicodeString: *mut u16,
    ) -> u32;
