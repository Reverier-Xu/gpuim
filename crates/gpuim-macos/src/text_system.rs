...
impl PlatformTextSystem for MacTextSystem {
    ...
    fn all_font_names(&self) -> Vec<String> {
        ...
        // NOTE: We intentionally avoid using `collection.get_descriptors()` here
        // because it has a memory leak bug in core-text v21.0.0. The upstream
        // code uses `wrap_under_get_rule` but
        // `CTFontCollectionCreateMatchingFontDescriptors` follows the Create
        // Rule (caller owns the result), so it should use
        // `wrap_under_create_rule`. We call the function directly with correct memory
        // management.
        unsafe extern "C" {
    ...
    fn rasterize_glyph(
        &self, glyph_id: &RenderGlyphParams, raster_bounds: Bounds<DevicePixels>,
...
impl MacTextSystemState {
    ...
    fn load_family(
        &mut self, name: &str, features: &FontFeatures, fallbacks: Option<&FontFallbacks>,
    ) -> Result<SmallVec<[FontId; 4]>> {
        let name = gpuim::font_name_with_fallbacks(name, ".AppleSystemUIFont");
        ...
        for font in family.fonts() {
            ...
            // We've seen a number of panics in production caused by calling
            // font.properties() which unwraps a downcast to CFNumber. This is
            // an attempt to avoid the panic, and to try and identify the
            // incalcitrant font.
    ...
    fn rasterize_glyph(
        &self, params: &RenderGlyphParams, glyph_bounds: Bounds<DevicePixels>,
    ) -> Result<(Size<DevicePixels>, Vec<u8>)> {
        if glyph_bounds.size.width.0 == 0 || glyph_bounds.size.height.0 == 0 {
            anyhow::bail!("glyph bounds are empty");
        } else {
            // Add an extra pixel when the subpixel variant isn't zero to make room for
            // anti-aliasing.
    ...
    fn layout_line(&mut self, text: &str, font_size: Pixels, font_runs: &[FontRun]) -> LineLayout {
...

        {
            ...
            for run in font_runs {
                ...
                // note: replace_str may silently ignore codepoints it dislikes (e.g., BOM at
                // start of string)
        ...
        // Retrieve the glyphs from the shaped line, converting UTF16 offsets to UTF8
        // offsets.
        ...
        for run in glyph_runs.into_iter() {
                ...
                .zip(run.string_indices().iter())
            {
                ...
                if ix_converter.utf16_ix > glyph_utf16_ix {
                    // We cannot reuse current index converter, as it can only seek forward. Restart
                    // the search.
...
// Some fonts may have no attributes despite `core_text` requiring them (and
// panicking). This is the same version as `core_text` has without `expect`
// calls.
mod lenient_font_attributes {
    ...
    fn get_string_attribute(
        descriptor: &CTFontDescriptor, attribute: CFStringRef,
    ) -> Option<String> {
        unsafe {
...
#[cfg(test)]
mod tests {
    use gpuim::{FontRun, GlyphId, PlatformTextSystem, font, px};
