...
unsafe extern "C" {
    ...
    fn CGSSetWindowBackgroundBlurRadius(
        connection_id: id, window_id: NSInteger, radius: i64,
    ) -> i32;
}
...
struct MacWindowState {
    ...
    event_callback: Option<Box<dyn FnMut(PlatformInput) -> gpuim::DispatchEventResult>>,
...
impl MacWindowState {
    ...
    fn bounds(&self) -> Bounds<Pixels> {
        ...
        if screen == nil {
            return Bounds::new(point(px(0.), px(0.)), gpuim::DEFAULT_WINDOW_SIZE);
        }
...
impl MacWindow {
    pub fn open(
        ...
        unsafe {
            ...
            // From winit crate: On Mojave, views automatically become layer-backed shortly
            // after being added to a native_window. Changing the
            // layer-backedness of a view breaks the association between the
            // view and its associated OpenGL context. To work around this,
            // on we explicitly make the view layer-backed up front so that AppKit doesn't
                ...
                && main_window != native_window
            {
                ...
                if should_add_as_tab {
                    ...
                    if main_window_can_tab == YES && main_window_visible == YES {
                        ...
                        // Ensure the window is visible immediately after adding the tab, since the
                        // tab bar is updated with a new entry at this point.
                        // Note: Calling orderFront here can break fullscreen mode (makes fullscreen
                        // windows exit fullscreen), so only do this if the main window is not
                        // fullscreen.
            ...
            // Although we already specified the position using
            // `initWithContentRect_styleMask_backing_defer_screen_`, the window
            // position might be incorrect if the main screen (the screen that contains the
            // window that has focus)  is different from the primary screen.
...
impl PlatformWindow for MacWindow {
    ...
    fn prompt(
        &self, level: PromptLevel, msg: &str, detail: Option<&str>, answers: &[PromptButton],
    ) -> Option<oneshot::Receiver<usize>> {
        // macOs applies overrides to modal window buttons after they are added.
        // Two most important for this logic are:
        // * Buttons with "Cancel" title will be displayed as the last buttons in the
        //   modal
        // * Last button added to the modal via `addButtonWithTitle` stays focused
        // * Focused buttons react on "space"/" " keypresses
        // * Usage of `keyEquivalent`, `makeFirstResponder` or
        //   `setInitialFirstResponder` does not change the focus
        //
        ...
        // ```
        //
        // To avoid situations when the last element added is "Cancel" and it gets the
        // focus (hence stealing both ESC and Space shortcuts), we find and add
        // one non-Cancel button last, so it gets focus and a Space shortcut.
        // This way, "Save this file? Yes/No/Cancel"-ish modals will get all three
        // buttons mapped with a key.
    ...
    fn set_background_appearance(&self, background_appearance: WindowBackgroundAppearance) {
        ...
        unsafe {
            ...
            if NSAppKitVersionNumber < NSAppKitVersionNumber12_0 {
                // Whether `-[NSVisualEffectView
                // respondsToSelector:@selector(_updateProxyLayer)]`.
                ...
                // On newer macOS `NSVisualEffectView` manages the effect layer directly. Using
                // it could have a better performance (it downsamples the
                // backdrop) and more control over the effect layer.
    ...
    fn on_input(&self, callback: Box<dyn FnMut(PlatformInput) -> gpuim::DispatchEventResult>) {
    ...
    fn draw(&self, scene: &gpuim::Scene) {
        let mut this = self.0.lock();
    ...
    fn gpu_specs(&self) -> Option<gpuim::GpuSpecs> {
        None
    }
                ...
                if_window_not_closed(closed, || {
                    unsafe {
                        ...
                        match action_str.as_ref() {
                            ...
                            "Fill" => {
                                // There is no documented API for "Fill" action, so we'll just zoom
                                // the window
                                window.zoom_(nil);
                            }
                            _ => {
...
fn get_scale_factor(native_window: id) -> f32 {
    ...
    // (if it is off-screen), though we'd expect to see
    // viewDidChangeBackingProperties before it was rendered for real.
...
//  U.S. layout:
//   - The IME consumes characters like 'j' and 'k', which makes paging through
//     `less` in the terminal behave incorrectly by default. This behavior
//     should be patched by our IME integration
//   - `alt-t` should open the tasks menu
//   - In vim mode, this keybinding should work: ``` { "context": "Editor &&
//     vim_mode == insert", "bindings": {"j j": "vim::NormalBefore"} } ``` and
//     typing 'j k' in insert mode with this keybinding should insert the two
//     characters
//  Brazilian layout:
...
//   - `" cmd-down` should insert a quote, unmark it, and move to the end of the
//     file
...
//   - In vim mode with `jj` bound to `vim::NormalBefore` in insert mode, typing
//     'j i' with Japanese IME should produce "じ" (ji), not "jい"

/// Returns true if the current keyboard input source is a composition-based IME
/// (e.g. Japanese Hiragana, Korean, Chinese Pinyin) that produces non-ASCII
/// output.
///
/// This checks two properties:
/// 1. The source type is `kTISTypeKeyboardInputMode` (an IME input mode, not a
///    plain keyboard layout). This excludes non-ASCII layouts like Armenian and
///    Ukrainian that map keys directly without composition.
/// 2. The source is not ASCII-capable, which excludes modes like Japanese
///    Romaji that produce ASCII characters and should allow multi-stroke
///    keybindings like `jj`.
...
extern "C" fn handle_key_event(this: &Object, native_event: id, key_equivalent: bool) -> BOOL {
    ...
    match event {
        PlatformInput::KeyDown(key_down_event) => {
            ...
            // the "key down" event if we've already just processed its "key equivalent"
            // version.
            if key_equivalent {
            ...
            // otherwise we only send to the input handler if we don't have a matching
            // binding. The input handler may call `do_command_by_selector` if
            // it doesn't know how to handle a key. If it does so, it will
            // return YES so we won't send the key twice. We also do this for
            // non-printing keys (like arrow keys and escape) as the IME menu
            // may need them even if there is no marked text;
            // however we skip keys with control or the input handler adds
            // control-characters to the buffer. and keys with function, as the
            // input handler swallows them. and keys with platform (Cmd), so
            // that Cmd+key events (e.g. Cmd+`) are not consumed by the IME on
            // non-QWERTY / dead-key layouts. We also send printable keys to the
            // IME first when an IME input source (e.g. Japanese,
            // Korean, Chinese) is active and the input handler accepts text input. This
            // prevents multi-stroke keybindings like `jj` from intercepting
            // keys that the IME should compose (e.g. typing 'ji' should produce
            // 'じ', not 'jい'). If the IME doesn't handle the key,
            ...
            // Don't send key equivalents to the input handler if there are key modifiers
            // other than Function key, or macOS shortcuts like cmd-` will stop
            // working.
...
// Update the window scale factor and drawable size, and call the resize
// callback if any.
...
extern "C" fn window_did_change_key_status(this: &Object, selector: Sel, _: id) {
    ...
    // When opening a pop-up while the application isn't active, Cocoa sends a
    // spurious `windowDidBecomeKey` message to the previous key window even
    // though that window isn't actually key. This causes a bug if the
    // application is later activated while the pop-up is still open, making it
    // impossible to activate the previous key window even if the pop-up gets
    // closed. The only way to activate it again is to de-activate the app and
    // re-activate it, which is a pretty bad UX. The following code detects the
    // spurious event and invokes `resignKeyWindow`: in theory, we're not
    // supposed to invoke this method manually but it balances out the spurious
    // `becomeKeyWindow` event and helps us work around that bug.
    ...
    // When a window becomes active, trigger an immediate synchronous frame request
    // to prevent tab flicker when switching between windows in native tabs
    // mode.
    //
    // This is only done on subsequent activations (not the first) to ensure the
    // initial focus path is properly established. Without this guard, the focus
    // state would remain unset until the first mouse click, causing keybindings
...
extern "C" fn first_rect_for_character_range(
    this: &Object, _: Sel, range: NSRange, _: id,
) -> NSRect {
...
extern "C" fn set_marked_text(
    this: &Object, _: Sel, text: id, selected_range: NSRange, replacement_range: NSRange,
) {
    unsafe {
        let is_attributed_string: BOOL =
            msg_send![text, isKindOfClass: [class!(NSAttributedString)]];
...
extern "C" fn attributed_substring_for_proposed_range(
    this: &Object, _: Sel, range: NSRange, actual_range: *mut c_void,
) -> id {
    with_input_handler(this, |input_handler| {
...
async fn synthetic_drag(
    window_state: Weak<Mutex<MacWindowState>>, drag_id: usize, event: MouseMoveEvent,
...
/// Sends the specified FileDropEvent using `PlatformInput::FileDrop` to the
/// window state and updates the window state according to the event passed.
fn send_file_drop_event(
    window_state: Arc<Mutex<MacWindowState>>, file_drop_event: FileDropEvent,
) -> bool {
...
extern "C" fn blurred_view_init_with_frame(this: &Object, _: Sel, frame: NSRect) -> id {
    unsafe {
        ...
        // Use a colorless semantic material. The default value `AppearanceBased`,
        // though not manually set, is deprecated.
...
unsafe fn remove_layer_background(layer: id) {
    unsafe {
        ...
        if !filters.is_null() {
            ...
            // `description` reflects its name and some parameters. Currently
            // `NSVisualEffectView` uses a `CAFilter` named "colorSaturate". If
            // one day they switch to `CIFilter`, the `description` will still
            // contain "Saturat" ("... inputSaturation = ...").
