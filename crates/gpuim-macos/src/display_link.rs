...
impl DisplayLink {
    pub fn new(
        display_id: CGDirectDisplayID, data: *mut c_void, callback: extern "C" fn(*mut c_void),
    ) -> Result<DisplayLink> {
        unsafe extern "C" fn display_link_callback(
            _display_link_out: *mut sys::CVDisplayLink, _current_time: *const sys::CVTimeStamp,
            _output_time: *const sys::CVTimeStamp, _flags_in: i64, _flags_out: *mut i64,
...
impl Drop for DisplayLink {
    fn drop(&mut self) {
        ...
        //
        // It seems possible that this happens because CVDisplayLinkRelease releases the
        // CVDisplayLink on the main thread immediately, but the background
        // thread that CVDisplayLink uses for timers is still accessing it.
        //
        // We might also want to upgrade to CADisplayLink, but that requires dropping
        // old macOS support.
...
mod sys {
    ...
    pub type CVDisplayLinkOutputCallback = unsafe extern "C" fn(
        ...
        // A pointer to the current timestamp. This represents the timestamp when the callback is
        // called.
        current_time: *const CVTimeStamp,
        // A pointer to the output timestamp. This represents the timestamp for when the frame will
        // be displayed.
    ...
    unsafe extern "C" {
        ...
        pub fn CVDisplayLinkSetCurrentCGDisplay(
            display_link: &mut DisplayLinkRef, display_id: u32,
        ) -> i32;
        pub fn CVDisplayLinkSetOutputCallback(
            display_link: &mut DisplayLinkRef, callback: CVDisplayLinkOutputCallback,
    ...
    impl DisplayLink {
        ...
        pub unsafe fn new(
            display_id: CGDirectDisplayID, callback: CVDisplayLinkOutputCallback,
            user_info: *mut c_void,
        ) -> Result<Self> {
            unsafe {
