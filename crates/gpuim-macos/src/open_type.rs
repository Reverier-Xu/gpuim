...
use gpuim::{FontFallbacks, FontFeatures};

pub fn apply_features_and_fallbacks(
    font: &mut FontKitFont, features: &FontFeatures, fallbacks: Option<&FontFallbacks>,
) -> anyhow::Result<()> {
    unsafe {
...
unsafe extern "C" {
    ...
    fn CTFontCreateCopyWithAttributes(
        font: CTFontRef, size: CGFloat, matrix: *const CGAffineTransform,
        attributes: CTFontDescriptorRef,
    ) -> CTFontRef;
    fn CTFontCopyDefaultCascadeListForLanguages(
        font: CTFontRef, languagePrefList: CFArrayRef,
    ) -> CFArrayRef;
}
