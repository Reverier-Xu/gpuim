    ...
    pub fn write(&self, item: ClipboardItem) {
        unsafe {
            match item.entries.as_slice() {
                ...
                _ => {
                    // Agus NB: We're currently only writing string entries to the clipboard when we
                    // have more than one.
                    //
                    ...
                    //
                    // Note how `any_images` is always `false`. We should fix that, but that's
...
mod tests {
    ...
    use gpuim::{ClipboardEntry, ClipboardItem, ClipboardString, ImageFormat};
