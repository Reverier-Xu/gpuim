...
fn select_emoji_font(
    ch: char, fonts: &[usvg::fontdb::ID], db: &usvg::fontdb::Database, families: &[&str],
...
/// When rendering SVGs, we render them at twice the size to get a
...
impl SvgRenderer {
    ...
    pub fn render_single_frame(
        &self, bytes: &[u8], scale_factor: f32,
    ...
    pub(crate) fn render_alpha_mask(
        &self, params: &RenderSvgParams, bytes: Option<&[u8]>,
...
// fontdb defaults generic families to Microsoft fonts ("Arial", "Times New
// Roman") which aren't installed on most Linux systems. fontconfig normally
// overrides these, but when it fails the defaults remain and all generic family
// queries return None.
