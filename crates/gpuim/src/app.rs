...
use gpuim_util::{ResultExt, debug_panic};
...
/// The duration for which futures returned from [Context::on_app_quit] can run
/// before the application fully quits.
pub const SHUTDOWN_TIMEOUT: Duration = Duration::from_millis(100);

/// Temporary(?) wrapper around [`RefCell<App>`] to help us debug any double
/// borrows. Strongly consider removing after stabilization.
#[doc(hidden)]
pub struct AppCell {
    app: RefCell<App>,
}
...
/// A reference to a GPUI application, typically constructed in the `main`
/// function of your app. You won't interact with this type much outside of
...
impl Application {
    ...
    /// Register a handler to be invoked when the platform instructs the
    /// application to open one or more URLs.
    ...
    /// On macOS, this can occur when the application icon is double-clicked or
    /// the app is launched via the dock.
    ...
    /// Returns a handle to the [`BackgroundExecutor`] associated with this app,
    /// which can be used to spawn futures in the background.
    pub fn background_executor(&self) -> BackgroundExecutor {
        self.0.borrow().background_executor.clone()
    }

    /// Returns a handle to the [`ForegroundExecutor`] associated with this app,
    /// which can be used to spawn futures in the foreground.
...
pub enum QuitMode {
    /// Use [`QuitMode::Explicit`] on macOS and [`QuitMode::LastWindowClosed`]
    /// on other platforms.
    #[default]
    Default,
...
/// Contains the state of the full application, and passed as a reference to a
/// variety of callbacks. Other [Context] derefs to this type.
...
pub struct App {
    ...
    /// Per-App element arena. This isolates element allocations between
    /// different App instances (important for tests where multiple Apps run
    /// concurrently).
    ...
    // We need to ensure the leak detector drops last, after all tasks, callbacks and things have
    // been dropped. Otherwise it may report false positives.
...
impl App {
    ...
    /// Asserts that no entities created after `snapshot` still have alive
    /// handles.
    ///
    /// Entities that were already tracked at the time of the snapshot are
    /// ignored, even if they still have handles. Only *new* entities (those
    /// whose `EntityId` was not present in the snapshot) are considered
    /// leaks.
    ///
    /// # Panics
    ///
    /// Panics if any new entity handles exist. The panic message lists every
    /// leaked entity with its type name, and includes allocation-site
    /// backtraces when `LEAK_BACKTRACE` is set.
    #[cfg(any(test, feature = "leak-detection"))]
    pub fn assert_no_new_leaks(&self, snapshot: &LeakDetectorSnapshot) {
        self.entities.assert_no_new_leaks(snapshot)
    }

    /// Quit the application gracefully. Handlers registered with
    /// [`Context::on_app_quit`] will be given 100ms to complete before
    /// exiting.
    ...
    /// Schedules all windows in the application to be redrawn. This can be
    /// called multiple times in an update cycle and still result in a
    /// single redraw.
    ...
    /// Arrange a callback to be invoked when the given entity calls `notify` on
    /// its respective context.
    pub fn observe<W>(
        &mut self, entity: &Entity<W>, mut on_notify: impl FnMut(Entity<W>, &mut App) + 'static,
    ) -> Subscription
    where
        W: 'static, {
    ...
    pub(crate) fn detect_accessed_entities<R>(
        &mut self, callback: impl FnOnce(&mut App) -> R,
    ...
    pub(crate) fn record_entities_accessed(
        &mut self, window_handle: AnyWindowHandle, invalidator: WindowInvalidator,
    ...
    /// Arrange for the given callback to be invoked whenever the given entity
    /// emits an event of a given type. The callback is provided a handle to
    /// the emitting entity and a reference to the emitted event.
    pub fn subscribe<T, Event>(
        &mut self, entity: &Entity<T>,
    ...
    pub(crate) fn new_subscription(
        &mut self, key: EntityId, value: (TypeId, Listener),
    ) -> Subscription {
    ...
    /// Each handle could be downcast to a handle typed for the root view of
    /// that window. To find all windows of a given type, you could filter
    /// on
    ...
    /// Returns the window handles ordered by their appearance on screen, front
    /// to back.
    ///
    /// The first window in the returned list is the active/topmost window of
    /// the application.
    ///
    /// This method returns None if the platform doesn't implement the method
    /// yet.
    pub fn window_stack(&self) -> Option<Vec<AnyWindowHandle>> {
        self.platform.window_stack()
    }

    /// Returns a handle to the window that is currently focused at the platform
    /// level, if one exists.
    pub fn active_window(&self) -> Option<AnyWindowHandle> {
        self.platform.active_window()
    }

    /// Opens a new window with the given option and the root view returned by
    /// the given function. The function is invoked with a `Window`, which
    /// can be used to interact with window-specific functionality.
    pub fn open_window<V: 'static + Render>(
        &mut self, options: crate::WindowOptions,
        ...
        self.update(|cx| {
            ...
            match Window::new(handle.into(), options, cx) {
                Ok(mut window) => {
                    ...
                    // this didn't cause any issues on non windows platforms as it seems we always
                    // won the race to on_request_frame on windows we quite
                    // frequently lose the race and return a window that has never rendered, which
                    // leads to a crash where DispatchTree::root_node_id asserts
                    // on empty nodes
    ...
    /// Instructs the platform to activate the application by bringing it to the
    /// foreground.
    ...
    pub fn write_credentials(
        &self, url: &str, username: &str, password: &[u8],
    ) -> Task<Result<()>> {
    ...
    ///
    /// When one or more paths are selected, they'll be relayed asynchronously
    /// via the returned oneshot channel. If cancelled, a `None` will be
    /// relayed instead. May return an error on Linux if the file picker
    /// couldn't be opened.
    pub fn prompt_for_paths(
        &self, options: PathPromptOptions,
    ) -> oneshot::Receiver<Result<Option<Vec<PathBuf>>>> {
        self.platform.prompt_for_paths(options)
    }

    /// Displays a platform modal for selecting a new path where a file can be
    /// saved.
    ///
    /// The provided directory will be used to set the initial location.
    /// When a path is selected, it is relayed asynchronously via the returned
    /// oneshot channel. If cancelled, a `None` will be relayed instead.
    /// May return an error on Linux if the file picker couldn't be opened.
    pub fn prompt_for_new_path(
        &self, directory: &Path, suggested_name: Option<&str>,
    ) -> oneshot::Receiver<Result<Option<PathBuf>>> {
        self.platform.prompt_for_new_path(directory, suggested_name)
    }

    /// Reveals the specified path at the platform level, such as in Finder on
    /// macOS.
    ...
    /// Returns whether the user has configured scrollbars to auto-hide at the
    /// platform level.
    ...
    /// such as notifying observers, emitting events, etc. Effects can
    /// themselves cause effects, so we continue looping until all effects
    /// are processed.
    ...
    /// reference count has become zero. We invoke any release observers before
    /// dropping each entity.
    ...
    /// Repeatedly called during `flush_effects` to handle a focused handle
    /// being dropped.
    ...
    fn apply_entity_created_effect(
        &mut self, entity: AnyEntity, tid: TypeId, window: Option<WindowId>,
    ) {
    ...
    /// Spawns the future returned by the given function on the main thread. The
    /// closure will be invoked with [AsyncApp], which allows the
    /// application state to be accessed across await points.
    #[track_caller]
    ...
    /// Accessor for the application's asset source, which is provided when
    ...
    /// Access the global of the given type. Panics if a global for that type
    ...
    /// Access the global of the given type mutably. Panics if a global for that
    /// type has not been assigned.
    #[track_caller]
    ...
    /// Access the global of the given type mutably. A default value is assigned
    /// if a global of this type has not yet been assigned.
    ...
    /// Remove the global of the given type from the app context. Does not
    ...
    /// Register a callback to be invoked when a global of the given type is
    /// updated.
    pub fn observe_global<G: Global>(
        &mut self, mut f: impl FnMut(&mut Self) + 'static,
    ) -> Subscription {
    ...
    pub(crate) fn new_entity_observer(
        &self, key: TypeId, value: NewEntityListener,
    ) -> Subscription {
    ...
    /// Arrange for the given function to be invoked whenever a view of the
    /// specified type is created. The function will be passed a mutable
    /// reference to the view along with an appropriate context.
    pub fn observe_new<T: 'static>(
        &self, on_new: impl 'static + Fn(&mut T, Option<&mut Window>, &mut Context<T>),
    ) -> Subscription {
    ...
    /// Observe the release of a entity. The callback is invoked after the
    /// entity has no more strong references but before it has been dropped.
    pub fn observe_release<T>(
        &self, handle: &Entity<T>, on_release: impl FnOnce(&mut T, &mut App) + 'static,
    ) -> Subscription
    where
        T: 'static, {
    ...
    pub fn observe_release_in<T>(
        &self, handle: &Entity<T>, window: &Window,
    ...
    /// Register a callback to be invoked when a keystroke is received by the
    /// application in any window. Note that this fires after all other
    /// action and event mechanisms have resolved and that this API will not
    /// be invoked if the event's propagation is stopped.
    pub fn observe_keystrokes(
        &mut self, mut f: impl FnMut(&KeystrokeEvent, &mut Window, &mut App) + 'static,
    ) -> Subscription {
        fn inner(
            keystroke_observers: &SubscriberSet<(), KeystrokeObserver>, handler: KeystrokeObserver,
        ) -> Subscription {
    ...
    /// Register a callback to be invoked when a keystroke is received by the
    /// application in any window. Note that this fires _before_ all other
    /// action and event mechanisms have resolved
    /// unlike [`App::observe_keystrokes`] which fires after. This means that
    /// `cx.stop_propagation` calls within interceptors will prevent action
    /// dispatch
    ...
    /// Register a global handler for actions invoked via the keyboard. These
    /// handlers are run at the end of the bubble phase for actions, and so
    /// will only be invoked if there are no other handlers or if they
    /// called `cx.propagate()`.
    pub fn on_action<A: Action>(
        &mut self, listener: impl Fn(&A, &mut Self) + 'static,
    ) -> &mut Self {
    ...
    /// Event handlers propagate events by default. Call this method to stop
    /// dispatching to event handlers with a lower z-index (mouse) or higher
    /// in the tree (keyboard). This is the opposite of [`Self::propagate`].
    /// It's also possible to cancel a call to [`Self::propagate`] by
    /// calling this method before effects are flushed.
    pub fn stop_propagation(&mut self) {
        self.propagate_event = false;
    }

    /// Action handlers stop propagation by default during the bubble phase of
    /// action dispatch dispatching to action handlers higher in the element
    /// tree. This is the opposite of [`Self::stop_propagation`]. It's also
    /// possible to cancel a call to [`Self::stop_propagation`] by calling
    ...
    pub fn build_action(
        &self, name: &str, data: Option<serde_json::Value>,
    ) -> std::result::Result<Box<dyn Action>, ActionBuildError> {
        self.actions.build_action(name, data)
    }

    /// Get all action names that have been registered. Note that registration
    /// only allows for actions to be built dynamically, and is unrelated to
    /// binding actions in the element tree.
    pub fn all_action_names(&self) -> &[&'static str] {
        self.actions.all_action_names()
    }

    /// Returns key bindings that invoke the given action on the currently
    /// focused element, without checking context. Bindings are returned in
    /// the order they were added. For display, the last binding should take
    /// precedence.
    pub fn all_bindings_for_input(&self, input: &[Keystroke]) -> Vec<KeyBinding> {
        RefCell::borrow(&self.keymap).all_bindings_for_input(input)
    }

    /// Get all non-internal actions that have been registered, along with their
    /// schemas.
    pub fn action_schemas(
        &self, generator: &mut schemars::SchemaGenerator,
    ...
    pub fn action_schema_by_name(
        &self, name: &str, generator: &mut schemars::SchemaGenerator,
    ...
    pub fn on_app_quit<Fut>(
        &self, mut on_quit: impl FnMut(&mut App) -> Fut + 'static,
    ) -> Subscription
    where
    ...
    /// Register a callback to be invoked when the application is about to
    /// restart.
    ///
    ...
    /// The window is no longer accessible at the point this callback is
    /// invoked.
    pub fn on_window_closed(
        &self, mut on_closed: impl FnMut(&mut App, WindowId) + 'static,
    ) -> Subscription {
    ...
    /// Checks if the given action is bound in the current context, as defined
    /// by the app's current focus, the bindings in the element tree, and
    /// any global action listeners.
    ...
    /// Sets the menu bar for this application. This will replace any existing
    /// menu bar.
    ...
    /// Performs the action associated with the given dock menu item, only used
    /// on Windows for now.
    pub fn perform_dock_menu_action(&self, action: usize) {
        self.platform.perform_dock_menu_action(action);
    }

    /// Adds given path to the bottom of the list of recent paths for the
    /// application. The list is usually shown on the application icon's
    /// context menu in the dock, and allows to open the recent files via
    /// that context menu. If the path is already in the list, it will be
    /// moved to the bottom of the list.
    pub fn add_recent_document(&self, path: &Path) {
        self.platform.add_recent_document(path);
    }

    /// Updates the jump list with the updated list of recent paths for the
    /// application, only used on Windows for now. Note that this also sets
    /// the dock menu on Windows.
    pub fn update_jump_list(
        &self, menus: Vec<MenuItem>, entries: Vec<SmallVec<[PathBuf; 2]>>,
    ) -> Task<Vec<SmallVec<[PathBuf; 2]>>> {
        self.platform.update_jump_list(menus, entries)
    }

    /// Dispatch an action to the currently active window or global action
    /// handler See [`crate::Action`] for more information on how actions
    /// work
    ...
    pub fn set_active_drag_cursor_style(
        &mut self, cursor_style: CursorStyle, window: &mut Window,
    ) -> bool {
    ...
    /// Set the prompt renderer for GPUI. This will replace the default or
    /// platform specific prompts with this custom implementation.
    ...
    /// `Asset::load` call at a time, and the results of this call will be
    /// cached
    ...
    /// Obtain a new [`FocusHandle`], which allows you to track and manipulate
    /// the keyboard focus for elements rendered within this window.
    #[track_caller]
    pub fn focus_handle(&self) -> FocusHandle {
        FocusHandle::new(&self.focus_handles)
    }

    /// Tell GPUI that an entity has changed and observers of it should be
    /// notified.
    ...
    /// Returns `true` if the platform file picker supports selecting a mix of
    ...
    ///
    /// If the current window is being updated, it will be removed from
    /// `App.windows`, you can use `current_window` to specify the current
    /// window. This is a no-op if the image is not in the sprite atlas.
    ...
    pub fn register_inspector_element<T: 'static, R: crate::IntoElement>(
        &mut self, f: impl 'static + Fn(crate::InspectorElementId, &T, &mut Window, &mut App) -> R,
    ) {
...
impl AppContext for App {
    ...
    ///
    /// The given function will be invoked with a [`Context`] and must return an
    /// object representing the entity. An [`Entity`] handle will be
    /// returned, which can be used to access the entity in a context.
    ...
    fn insert_entity<T: 'static>(
        &mut self, reservation: Reservation<T>, build_entity: impl FnOnce(&mut Context<T>) -> T,
    ) -> Entity<T> {
        self.update(|cx| {
    ...
    /// Updates the entity referenced by the given handle. The function is
    /// passed a mutable reference to the entity along with a `Context` for
    /// the entity.
    fn update_entity<T: 'static, R>(
        &mut self, handle: &Entity<T>, update: impl FnOnce(&mut T, &mut Context<T>) -> R,
    ) -> R {
        self.update(|cx| {
    ...
    fn read_window<T, R>(
        &self, window: &WindowHandle<T>, read: impl FnOnce(Entity<T>, &App) -> R,
    ) -> Result<R>
    where
        T: 'static, {
        let window = self
...
/// Wraps a global variable value during `update_global` while the value has
/// been moved to the stack.
...
/// Contains state associated with an active drag operation, started by dragging
/// an element within the window or by dragging into the app from the underlying
/// platform.
pub struct AnyDrag {
...
/// Contains state associated with a tooltip. You'll only need this struct if
/// you're implementing tooltip behavior on a custom element. Otherwise, use
/// [Div::tooltip](crate::Interactivity::tooltip).
#[derive(Clone)]
pub struct AnyTooltip {
    ...
    /// Given the bounds of the tooltip, checks whether the tooltip should still
    /// be visible and updates its state accordingly. This is needed atop
    /// the hovered element's mouse move handler to handle the case where
    /// the element is not painted (e.g. via use of `visible_on_hover`).
