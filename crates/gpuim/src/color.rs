...
impl Hsla {
    ...
    /// Blends `other` on top of `self` based on `other`'s alpha value. The
    /// resulting color is a combination of `self`'s and `other`'s colors.
    ///
    /// If `other`'s alpha value is 1.0 or greater, `other` color is fully
    /// opaque, thus `other` is returned as the output color. If `other`'s
    /// alpha value is 0.0 or less, `other` color is fully transparent, thus
    /// `self` is returned as the output color. Else, the output color is
    /// calculated as a blend of `self` and `other` based on their weighted
    /// alpha values.
    ///
    /// Assumptions:
    /// - Alpha values are contained in the range [0, 1], with 1 as fully opaque
    ///   and 0 as fully transparent.
    /// - The relative contributions of `self` and `other` is based on `self`'s
    ///   alpha value (`self.a`) and `other`'s  alpha value (`other.a`), `self`
    ///   contributing `self.a * (1.0 - other.a)` and `other` contributing its
    ///   own alpha value.
    /// - RGB color components are contained in the range [0, 1].
    /// - If `self` and `other` colors are out of the valid range, the blend
    ///   operation's output and behavior is undefined.
    ...
    /// Returns a new HSLA color with the same hue, and lightness, but with no
    /// saturation.
    ...
    /// Fade out the color by a given factor. This factor should be between 0.0
    /// and 1.0. Where 0.0 will leave the color unchanged, and 1.0 will
    /// completely fade out the color.
...
///
/// The gradient line's angle of direction. A value of `0.` is equivalent to
...
pub fn linear_gradient(
    angle: f32, from: impl Into<LinearColorStop>, to: impl Into<LinearColorStop>,
) -> Background {
    Background {
...
impl LinearColorStop {
    /// Returns a new color stop with the same color, but with a modified alpha
    /// value.
    pub fn opacity(&self, factor: f32) -> Self {
        Self {
...
impl Background {
    ...
    /// Returns a new background color with the same hue, saturation, and
    /// lightness, but with a modified alpha value.
    pub fn opacity(&self, factor: f32) -> Self {
