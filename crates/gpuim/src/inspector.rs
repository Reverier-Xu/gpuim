...
mod conditional {
    ...
    /// Manages inspector state - which element is currently selected and
    /// whether the inspector is in picking mode.
    ...
    impl Inspector {
        ...
        pub(crate) fn with_active_element_state<T: 'static, R>(
            &mut self, window: &mut Window, f: impl FnOnce(&mut Option<T>, &mut Window) -> R,
        ) -> R {
        ...
        /// Starts element picking mode, allowing the user to select elements by
        /// clicking.
        ...
        /// Renders elements for all registered inspector states of the active
        /// inspector element.
    ...
    impl InspectorElementRegistry {
        pub fn register<T: 'static, R: IntoElement>(
            &mut self, f: impl 'static + Fn(InspectorElementId, &T, &mut Window, &mut App) -> R,
        ) {
...
pub mod inspector_reflection {
    ...
    /// Reification of a function that has the signature `fn some_fn(T) -> T`.
    /// Provides the name, documentation, and ability to invoke the
    /// function.
    #[derive(Clone, Copy)]
