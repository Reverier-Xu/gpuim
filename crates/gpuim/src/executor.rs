...
///
/// If you drop a task it will be cancelled immediately. Calling
/// [`Task::detach`] allows the task to continue running, but with no way to
/// return a value.
#[must_use]
#[derive(Debug)]
...
impl<T> Task<T> {
    ...
    /// Returns true if the task has completed or was created with
    /// `Task::ready`.
...
where
    ...
    E: 'static + Debug,
{
    /// Run the task to completion in the background and log any errors that
    /// occur.
    #[track_caller]
...
impl BackgroundExecutor {
    ...
    /// Enqueues the given future to be run to completion on a background
    /// thread.
    #[track_caller]
    ...
    ///
    /// When `Priority::RealtimeAudio` is used, the task runs on a dedicated
    /// thread with realtime scheduling priority, suitable for audio
    /// processing.
    #[track_caller]
    pub fn spawn_with_priority<R>(
        &self, priority: Priority, future: impl Future<Output = R> + Send + 'static,
    ) -> Task<R>
    where
        R: Send + 'static, {
    ...
    /// Enqueues the given future to be run to completion on a background thread
    /// and blocking the current task on it.
    ///
    /// This allows to spawn background work that borrows from its scope. Note
    /// that the supplied future will run to completion before the current
    /// task is resumed, even if the current task is slated for cancellation.
    ...
    /// In tests, run an arbitrary number of tasks (determined by the SEED
    ...
    /// In tests, move time forward. This does not run any tasks, but does make
    /// `timer`s ready.
    #[cfg(any(test, feature = "test-support"))]
    ...
    ///
    /// Under the scheduler-backed test dispatcher, `tick()` will not advance
    /// the clock, so a pending timer can keep `has_pending_tasks()` true
    /// even after all currently-runnable tasks have been drained. To
    /// preserve the historical semantics that tests relied on (drain all work
    /// that can make progress), we advance the clock to the next timer when
    ...
    /// In tests, prevents `run_until_parked` from panicking if there are
    /// outstanding tasks.
    #[cfg(any(test, feature = "test-support"))]
...
impl ForegroundExecutor {
    ...
    /// Enqueues the given Task to run on the main thread with the given
    /// priority.
    #[track_caller]
    pub fn spawn_with_priority<R>(
        &self, _priority: Priority, future: impl Future<Output = R> + 'static,
    ) -> Task<R>
    where
        R: 'static, {
        // Priority is ignored for foreground tasks - they run in order on the main
        // thread
    ...
    pub fn block_test<R>(&self, future: impl Future<Output = R>) -> R {
        ...
        // In async GPUI tests, we must allow foreground tasks scheduled by the test
        // itself (which are associated with the test session) to make progress
        // while we block. Otherwise, awaiting futures that depend on
        // same-session foreground work can deadlock.
    ...
    /// Block the current thread until the given future resolves or the timeout
    /// elapses.
    pub fn block_with_timeout<R, Fut: Future<Output = R>>(
        &self, duration: Duration, future: Fut,
