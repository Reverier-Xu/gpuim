...
use scheduler::{Clock, Instant, Priority, Scheduler, SessionId, TestScheduler, Timer};

use crate::{PlatformDispatcher, RunnableMeta};

/// A production implementation of [`Scheduler`] that wraps a
/// [`PlatformDispatcher`].
///
/// This allows GPUI to use the scheduler crate's executor types with the
/// platform's native dispatch mechanisms (e.g., Grand Central Dispatch on
/// macOS).
...
impl Scheduler for PlatformScheduler {
    fn block(
        &self, _session_id: Option<SessionId>,
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            let park_deadline = |deadline: Instant| {
                // Timer expirations are only delivered every ~15.6 milliseconds by default on
                // Windows. We increase the resolution during this wait so that
                // short timeouts stay reasonably short.
