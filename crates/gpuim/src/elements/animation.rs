...
impl Animation {
    ...
    /// By default the animation will only run once and will use a linear easing
    /// function.
    ...
    /// The easing function will take a time delta between 0 and 1 and return a
    /// new delta between 0 and 1
...
/// An extension trait for adding the animation wrapper to both Elements and
/// Components
pub trait AnimationExt {
    /// Render this component or element with an animation
    fn with_animation(
        self, id: impl Into<ElementId>, animation: Animation,
    ...
    fn with_animations(
        self, id: impl Into<ElementId>, animations: Vec<Animation>,
...
impl<E: IntoElement + 'static> Element for AnimationElement<E> {
    ...
    fn prepaint(
        ...
        _bounds: crate::Bounds<crate::Pixels>, element: &mut Self::RequestLayoutState,
        window: &mut Window, cx: &mut App,
    ) -> Self::PrepaintState {
...
mod easing {
    ...
    /// The quadratic ease-in-out function, which starts and ends slowly but
    ...
    /// The Quint ease-out function, which starts quickly and decelerates to a
    /// stop
    pub fn ease_out_quint() -> impl Fn(f32) -> f32 {
        move |delta| 1.0 - (1.0 - delta).powi(5)
    }

    /// Apply the given easing function, first in the forward direction and then
    ...
    /// A custom easing function for pulsating alpha that slows down as it
    /// approaches 0.1
    pub fn pulsating_between(min: f32, max: f32) -> impl Fn(f32) -> f32 {
        let range = max - min;

        move |delta| {
            // Use a combination of sine and cubic functions for a more natural breathing
            // rhythm
