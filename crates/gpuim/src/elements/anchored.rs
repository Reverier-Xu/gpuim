...
impl Anchored {
    /// Sets which corner of the anchored element should be anchored to the
    /// current position.
    ...
    /// Useful when you want to anchor to an element but offset from it, such as
    /// in PopoverMenu.
    ...
    /// While Window will have it interpret the position as relative to the
    /// window.
    ...
    /// Snap to window edge instead of switching anchor corner when an overflow
    /// would occur.
...
impl Element for Anchored {
    ...
    fn prepaint(
        ...
        // Snap the horizontal edges of the anchored element to the horizontal edges of
        // the window if its horizontal bounds overflow, aligning to the left if
        // it is wider than the limits.
        ...
        // Snap the vertical edges of the anchored element to the vertical edges of the
        // window if its vertical bounds overflow, aligning to the top if it is
        // taller than the limits.
    ...
    fn paint(
        &mut self, _id: Option<&GlobalElementId>, _inspector_id: Option<&InspectorElementId>,
        _bounds: crate::Bounds<crate::Pixels>, _request_layout: &mut Self::RequestLayoutState,
...
/// Which algorithm to use when fitting the anchored element to be inside the
/// window.
...
impl AnchoredPositionMode {
    fn get_position_and_bounds(
        &self, anchor_position: Option<Point<Pixels>>, anchor_corner: Corner, size: Size<Pixels>,
        bounds: Bounds<Pixels>, offset: Option<Point<Pixels>>,
