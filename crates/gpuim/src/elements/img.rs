...
pub trait StyledImage: Sized {
    ...
    /// Set a fallback function that will be invoked to render an error view
    /// should the image fail to load.
    fn with_fallback(mut self, fallback: impl Fn() -> AnyElement + 'static) -> Self {
        self.image_style().fallback = Some(Box::new(fallback));
        self
    }

    /// Set a fallback function that will be invoked to render a view while the
...
impl Img {
    ...
    ///
    /// If the `image_cache` is not explicitly provided, the function will
    /// determine the image cache by:
    ///
    /// 1. Checking if any ancestor node of the current node contains an
    ///    `ImageCacheElement`, If such a node exists, the image cache specified
    ///    by that ancestor will be used.
    /// 2. If no ancestor node contains an `ImageCacheElement`, the global image
    ///    cache will be used as a fallback.
    ///
    /// This mechanism provides a flexible way to manage image caching, allowing
    /// precise control when needed, while ensuring a default behavior when
    /// no cache is explicitly specified.
    #[inline]
...
impl Element for Img {
    ...
    fn paint(
        &mut self, global_id: Option<&GlobalElementId>, inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<Pixels>, layout_state: &mut Self::RequestLayoutState,
...
impl ImageSource {
    pub(crate) fn use_data(
        &self, cache: Option<AnyImageCache>, window: &mut Window, cx: &mut App,
