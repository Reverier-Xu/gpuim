//! A scrollable list of elements with uniform height, optimized for large
//! lists. Rather than use the full taffy layout system, uniform_list simply
...
/// uniform_list provides lazy rendering for a set of items that are of uniform
/// height. When rendered into a container with overflow-y: hidden and a fixed
/// (or max) height, uniform_list will only render the visible subset of items.
#[track_caller]
...
/// A list element for efficiently laying out and displaying a list of
/// uniform-height elements.
...
/// This should be stored in your view and passed to the uniform_list on each
/// frame.
...
pub enum ScrollStrategy {
    ...
    /// May not be possible if there's not enough list items above the item
    /// scrolled to: in this case, the element will be placed at the closest
    /// possible position.
    Center,
    ...
    /// - The top of the list's viewport if the target element is above
    ///   currently visible elements.
    /// - The bottom of the list's viewport if the target element is above
    ///   currently visible elements.
    Nearest,
}
...
pub struct ItemSize {
    ...
    /// The size of the item's contents, which may be larger than the item
    /// itself, if the item was bounded by a parent element.
...
impl UniformListScrollHandle {
    ...
    ///
    /// This uses non-strict scrolling: if the item is already fully visible, no
    /// scrolling occurs. If the item is out of view, it scrolls the minimum
    /// amount to bring it into view according to the strategy.
    ...
    /// Scroll the list so that the given item index is at scroll strategy
    /// position.
    ///
    /// This uses strict scrolling: the item will always be scrolled to match
    /// the strategy position, even if it's already visible. Use this when
    ...
    /// Scroll the list to the given item index with an offset in number of
    /// items.
    ///
    /// This uses non-strict scrolling: if the item is already visible within
    /// the offset region, no scrolling occurs.
    ///
    /// The offset parameter shrinks the effective viewport by the specified
    /// number of items from the corresponding edge, then applies the scroll
    /// strategy within that reduced viewport:
    /// - `ScrollStrategy::Top`: Shrinks from top, positions item at the new top
    /// - `ScrollStrategy::Center`: Shrinks from top, centers item in the
    ///   reduced viewport
    /// - `ScrollStrategy::Bottom`: Shrinks from bottom, positions item at the
    ///   new bottom
    ...
    /// Scroll the list so that the given item index is at the exact scroll
    /// strategy position with an offset.
    ///
    /// This uses strict scrolling: the item will always be scrolled to match
    /// the strategy position, even if it's already visible.
    ///
    /// The offset parameter shrinks the effective viewport by the specified
    /// number of items from the corresponding edge, then applies the scroll
    /// strategy within that reduced viewport:
    ...
    pub fn scroll_to_item_strict_with_offset(
        &self, ix: usize, strategy: ScrollStrategy, offset: usize,
    ) {
        self.0.borrow_mut().deferred_scroll_to_item = Some(DeferredScrollToItem {
            item_index: ix,
            strategy,
...
impl Element for UniformList {
    ...
    fn prepaint(
        &mut self, global_id: Option<&GlobalElementId>, inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<Pixels>, frame_state: &mut Self::RequestLayoutState, window: &mut Window,
        cx: &mut App,
    ) -> Option<Hitbox> {
        let style = self
            .interactivity
                        ...
                        if scroll_strict || is_above || is_below {
                            ...
                            match strategy {
                                ...
                                ScrollStrategy::Nearest => {
                                    // Nearest, but the item is visible -> no
                                    // scroll is required
                                }
                            }
                        }
    ...
    fn paint(
        &mut self, global_id: Option<&GlobalElementId>, inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<crate::Pixels>, request_layout: &mut Self::RequestLayoutState,
...
pub trait UniformListDecoration {
    ...
    fn compute(
        &self, visible_range: Range<usize>, bounds: Bounds<Pixels>, scroll_offset: Point<Pixels>,
        item_height: Pixels, item_count: usize, window: &mut Window, cx: &mut App,
    ) -> AnyElement;
}
...
impl UniformList {
    ...
    /// Sets the horizontal sizing behavior, controlling the way list items laid
    /// out horizontally.
    /// With [`ListHorizontalSizingBehavior::Unconstrained`] behavior, every
    /// item and the list itself will have the size of the widest item and
    /// lay out pushing the `end_slot` to the right end.
    pub fn with_horizontal_sizing_behavior(
        mut self, behavior: ListHorizontalSizingBehavior,
    ) -> Self {
    ...
    /// Track and render scroll state of this list with reference to the given
    /// scroll handle.
    ...
    /// Sets whether the list is flipped vertically, such that item 0 appears at
    /// the bottom.
...
mod test {
    ...
    fn test_scroll_strategy_nearest(cx: &mut TestAppContext) {
        ...
        impl TestView {
            pub fn select_next(
                &mut self, _: &SelectNext, window: &mut Window, _: &mut Context<Self>,
            ) {
            ...
            pub fn select_previous(
                &mut self, _: &SelectPrev, window: &mut Window, _: &mut Context<Self>,
            ) {
                if self.index == 0 {
