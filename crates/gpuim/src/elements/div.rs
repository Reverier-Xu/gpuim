//! Div is the central, reusable element that most GPUI trees will be built
//! from. It functions as a container for other elements, and provides a number
//! of useful features for laying out and styling its children as well as
//! binding mouse events and action handlers. It is meant to be similar to the
//! HTML `<div>` element, but for GPUI.
//!
//! # Build your own div
//!
//! GPUI does not directly provide APIs for stateful, multi step events like
//! `click` and `drag`. We want GPUI users to be able to build their own
//! abstractions for their own needs. However, as a UI framework, we're also
//! obliged to provide some building blocks to make the process of building your
//! own elements easier. For this we have the [`Interactivity`] and the
//! [`StyleRefinement`] structs, as well as several associated traits. Together,
//! these provide the full suite of Dom-like events and Tailwind-like styling
//! that you can use to build your own custom elements. Div is constructed by
//! combining these two systems into an all-in-one element.
...
/// An event for when a drag is moving over this element, with the given state
/// type.
...
impl Interactivity {
    ...
    /// Gets the source location of construction. Returns `None` when not in
    /// debug mode.
    ...
    /// Bind the given callback to the mouse down event for the given mouse
    /// button, during the bubble phase. The imperative API equivalent of
    /// [`InteractiveElement::on_mouse_down`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to the
    /// view state from this callback.
    ...
    /// Bind the given callback to the mouse down event for any button, during
    /// the capture phase. The imperative API equivalent of
    /// [`InteractiveElement::capture_any_mouse_down`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    pub fn capture_any_mouse_down(
        &mut self, listener: impl Fn(&MouseDownEvent, &mut Window, &mut App) + 'static,
    ) {
        self.mouse_down_listeners
            .push(Box::new(move |event, phase, hitbox, window, cx| {
    ...
    /// Bind the given callback to the mouse down event for any button, during
    /// the bubble phase. The imperative API equivalent to
    /// [`InteractiveElement::on_any_mouse_down`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Bind the given callback to the mouse pressure event, during the bubble
    /// phase the imperative API equivalent to
    /// [`InteractiveElement::on_mouse_pressure`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    pub fn on_mouse_pressure(
        &mut self, listener: impl Fn(&MousePressureEvent, &mut Window, &mut App) + 'static,
    ) {
    ...
    /// Bind the given callback to the mouse pressure event, during the capture
    /// phase the imperative API equivalent to
    /// [`InteractiveElement::on_mouse_pressure`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    ...
    /// Bind the given callback to the mouse up event for the given button,
    /// during the bubble phase. The imperative API equivalent to
    /// [`InteractiveElement::on_mouse_up`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    pub fn on_mouse_up(
    ...
    /// Bind the given callback to the mouse up event for any button, during the
    /// capture phase. The imperative API equivalent to
    /// [`InteractiveElement::capture_any_mouse_up`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    pub fn capture_any_mouse_up(
        &mut self, listener: impl Fn(&MouseUpEvent, &mut Window, &mut App) + 'static,
    ) {
        self.mouse_up_listeners
            .push(Box::new(move |event, phase, hitbox, window, cx| {
                if phase == DispatchPhase::Capture && hitbox.is_hovered(window) {
    ...
    /// Bind the given callback to the mouse up event for any button, during the
    /// bubble phase. The imperative API equivalent to
    /// [`Interactivity::on_any_mouse_up`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Bind the given callback to the mouse down event, on any button, during
    /// the capture phase, when the mouse is outside of the bounds of this
    /// element. The imperative API equivalent to
    /// [`InteractiveElement::on_mouse_down_out`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Bind the given callback to the mouse up event, for the given button,
    /// during the capture phase, when the mouse is outside of the bounds of
    /// this element. The imperative API equivalent to
    /// [`InteractiveElement::on_mouse_up_out`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Bind the given callback to the mouse move event, during the bubble
    /// phase. The imperative API equivalent to
    /// [`InteractiveElement::on_mouse_move`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    pub fn on_mouse_move(
        &mut self, listener: impl Fn(&MouseMoveEvent, &mut Window, &mut App) + 'static,
    ) {
    ...
    /// Bind the given callback to the mouse drag event of the given type. Note
    /// that this will be called for all move events, inside or outside of
    /// this element, as long as the drag was started with this element
    /// under the mouse. Useful for implementing draggable UIs that don't
    /// conform to a drag and drop style interaction, like resizing.
    /// The imperative API equivalent to [`InteractiveElement::on_drag_move`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    pub fn on_drag_move<T>(
        &mut self, listener: impl Fn(&DragMoveEvent<T>, &mut Window, &mut App) + 'static,
    ) where
        T: 'static, {
        self.mouse_move_listeners
            .push(Box::new(move |event, phase, hitbox, window, cx| {
    ...
    /// The imperative API equivalent to
    /// [`InteractiveElement::on_scroll_wheel`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    pub fn on_scroll_wheel(
        &mut self, listener: impl Fn(&ScrollWheelEvent, &mut Window, &mut App) + 'static,
    ) {
    ...
    /// Bind the given callback to pinch gesture events during the capture
    /// phase.
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    pub fn capture_pinch(
        &mut self, listener: impl Fn(&PinchEvent, &mut Window, &mut App) + 'static,
    ) {
        self.pinch_listeners
    ...
    /// Bind the given callback to an action dispatch, based on a dynamic action
    /// parameter instead of a type parameter. Useful for component
    /// libraries that want to expose action bindings to their users.
    /// The imperative API equivalent to
    /// [`InteractiveElement::on_boxed_action`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    pub fn on_boxed_action(
        &mut self, action: &dyn Action,
    ...
    /// view's state from this callback.
    pub fn on_key_down(
        &mut self, listener: impl Fn(&KeyDownEvent, &mut Window, &mut App) + 'static,
    ) {
    ...
    pub fn capture_key_up(
        &mut self, listener: impl Fn(&KeyUpEvent, &mut Window, &mut App) + 'static,
    ) {
        self.key_up_listeners
            .push(Box::new(move |event, phase, window, cx| {
                if phase == DispatchPhase::Capture {
    ...
    /// Bind the given callback to drop events of the given type, whether or not
    /// the drag started on this element. The imperative API equivalent to
    /// [`InteractiveElement::on_drop`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Use the given predicate to determine whether or not a drop event should
    /// be dispatched to this element. The imperative API equivalent to
    /// [`InteractiveElement::can_drop`].
    pub fn can_drop(
        &mut self, predicate: impl Fn(&dyn Any, &mut Window, &mut App) -> bool + 'static,
    ) {
    ...
    /// The imperative API equivalent to
    /// [`StatefulInteractiveElement::on_click`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// The imperative API equivalent to
    /// [`StatefulInteractiveElement::on_aux_click`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    pub fn on_aux_click(
        &mut self, listener: impl Fn(&ClickEvent, &mut Window, &mut App) + 'static,
    ) where
        Self: Sized, {
    ...
    /// On drag initiation, this callback will be used to create a new view to
    /// render the dragged value for a drag and drop operation. This API
    /// should also be used as the equivalent of 'on drag start' with
    ...
    /// Bind the given callback on the hover start and end events of this
    /// element. Note that the boolean passed to the callback is true when
    /// the hover starts and false when it ends. The imperative API
    /// equivalent to [`StatefulInteractiveElement::on_hover`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Use the given callback to construct a new tooltip view when the mouse
    /// hovers over this element. The imperative API equivalent to
    ...
    /// Use the given callback to construct a new tooltip view when the mouse
    /// hovers over this element. The tooltip itself is also hoverable and
    /// won't disappear when the user moves the mouse into the tooltip. The
    /// imperative API equivalent to
    /// [`StatefulInteractiveElement::hoverable_tooltip`].
    pub fn hoverable_tooltip(
        &mut self, build_tooltip: impl Fn(&mut Window, &mut App) -> AnyView + 'static,
    ) where
        Self: Sized, {
        debug_assert!(
            self.tooltip_builder.is_none(),
    ...
    /// Block the mouse from all interactions with elements behind this
    /// element's hitbox. Typically `block_mouse_except_scroll` should be
    /// preferred.
    ///
    ...
    /// Set the bounds of this element as a window control area for the platform
    /// window. The imperative API equivalent to
    ...
    /// hitbox. The imperative API equivalent to
...
/// A trait for elements that want to use the standard GPUI event handlers that
/// don't require any state.
pub trait InteractiveElement: Sized {
    ...
    ///
    /// When false, the element remains in tab-index order but cannot be reached
    /// via keyboard navigation. Useful for container elements: focus the
    /// container, then call `window.focus_next(cx)` to focus the first tab
    /// stop inside it while having the container element itself be unreachable
    /// via the keyboard. Should only be used with `tab_index`.
    ...
    /// [`Self::tab_stop`] for more information. This should only be used in
    /// conjunction with `tab_group` in order to not interfere with the tab
    ...
    /// Designate this div as a "tab group". Tab groups have their own location
    /// in the tab-index order, but for children of the tab group, the tab
    /// index is reset to 0. This can be useful for swapping the order of
    /// tab stops within the group, without having to renumber all the tab stops
    ...
    /// Apply the given style to this element when the mouse hovers over a group
    /// member
    fn group_hover(
        mut self, group_name: impl Into<SharedString>,
    ...
    /// Bind the given callback to the mouse down event for the given mouse
    /// button. The fluent API equivalent to
    /// [`Interactivity::on_mouse_down`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to the
    /// view state from this callback.
    fn on_mouse_down(
    ...
    /// Bind the given callback to the mouse down event for any button, during
    /// the capture phase. The fluent API equivalent to
    /// [`Interactivity::capture_any_mouse_down`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Bind the given callback to the mouse down event for any button, during
    /// the capture phase. The fluent API equivalent to
    /// [`Interactivity::on_any_mouse_down`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Bind the given callback to the mouse up event for the given button,
    /// during the bubble phase. The fluent API equivalent to
    /// [`Interactivity::on_mouse_up`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    fn on_mouse_up(
    ...
    /// Bind the given callback to the mouse up event for any button, during the
    /// capture phase. The fluent API equivalent to
    /// [`Interactivity::capture_any_mouse_up`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Bind the given callback to the mouse pressure event, during the bubble
    /// phase the fluent API equivalent to
    /// [`Interactivity::on_mouse_pressure`]
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Bind the given callback to the mouse down event, on any button, during
    /// the capture phase, when the mouse is outside of the bounds of this
    /// element. The fluent API equivalent to
    /// [`Interactivity::on_mouse_down_out`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Bind the given callback to the mouse up event, for the given button,
    /// during the capture phase, when the mouse is outside of the bounds of
    /// this element. The fluent API equivalent to
    /// [`Interactivity::on_mouse_up_out`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    fn on_mouse_up_out(
    ...
    /// Apply the given style when the given data type is dragged over this
    /// element
    ...
    /// Bind the given callback to drop events of the given type, whether or not
    /// the drag started on this element. The fluent API equivalent to
    /// [`Interactivity::on_drop`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    fn on_drop<T: 'static>(
        mut self, listener: impl Fn(&T, &mut Window, &mut App) + 'static,
    ) -> Self {
    ...
    /// Use the given predicate to determine whether or not a drop event should
    /// be dispatched to this element. The fluent API equivalent to
    /// [`Interactivity::can_drop`].
    fn can_drop(
        mut self, predicate: impl Fn(&dyn Any, &mut Window, &mut App) -> bool + 'static,
    ) -> Self {
    ...
    /// Block the mouse from all interactions with elements behind this
    /// element's hitbox. Typically `block_mouse_except_scroll` should be
    /// preferred. The fluent API equivalent to
    ...
    /// Set the bounds of this element as a window control area for the platform
    /// window. The fluent API equivalent to
    /// [`Interactivity::window_control_area`].
    ...
    /// Block non-scroll mouse interactions with elements behind this element's
    /// hitbox. The fluent API equivalent to
    ...
    /// Set the given styles to be applied when this element, specifically, is
    /// focused. Requires that the element is focusable. Elements can be
    /// made focusable using [`InteractiveElement::track_focus`].
    ...
    /// Set the given styles to be applied when this element is inside another
    /// element that is focused. Requires that the element is focusable.
    /// Elements can be made focusable using
    ...
    /// Set the given styles to be applied when this element is focused via
    /// keyboard navigation. This is similar to CSS's `:focus-visible`
    /// pseudo-class - it only applies when the element is focused AND the
    /// user is navigating via keyboard (not mouse clicks). Requires that
    /// the element is focusable. Elements can be made focusable using
    /// [`InteractiveElement::track_focus`].
...
/// A trait for elements that want to use the standard GPUI interactivity
/// features that require state.
pub trait StatefulInteractiveElement: InteractiveElement {
    ...
    ///
    /// This will only affect the layout of the element when overflow for this
    /// element is set to `Overflow::Scroll`.
    ...
    /// The callback also has access to the offset of triggering click from the
    /// origin of parent element. The fluent API equivalent to
    /// [`Interactivity::on_drag`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    fn on_drag<T, W>(
    ...
    /// Bind the given callback on the hover start and end events of this
    /// element. Note that the boolean passed to the callback is true when
    /// the hover starts and false when it ends. The fluent API equivalent
    /// to [`Interactivity::on_hover`].
    ///
    /// See [`Context::listener`](crate::Context::listener) to get access to a
    /// view's state from this callback.
    ...
    /// Use the given callback to construct a new tooltip view when the mouse
    /// hovers over this element. The fluent API equivalent to
    ...
    /// hovers over this element. The tooltip itself is also hoverable and
    /// won't disappear when the user moves the mouse into the tooltip. The
    /// fluent API equivalent to [`Interactivity::hoverable_tooltip`].
...
impl Div {
    /// Add a listener to be called when the children of this `Div` are
    /// prepainted. This allows you to store the [`Bounds`] of the children
    /// for later use.
    pub fn on_children_prepainted(
        mut self, listener: impl Fn(Vec<Bounds<Pixels>>, &mut Window, &mut App) + 'static,
    ) -> Self {
    ...
    /// Specify a function that determines the order in which children are
    /// prepainted.
    ///
    /// The function is called at prepaint time and should return a vector of
    /// child indices in the desired prepaint order. Each index should
    /// appear exactly once.
    ///
    /// This is useful when the prepaint of one child affects state that another
    /// child reads. For example, in split editor views, the editor with an
    /// autoscroll request should be prepainted first so its scroll position
    /// update is visible to the other editor.
    pub fn with_dynamic_prepaint_order(
        mut self, order_fn: impl Fn(&mut Window, &mut App) -> SmallVec<[usize; 8]> + 'static,
    ) -> Self {
...
/// A frame state for a `Div` element, which contains layout IDs for its
/// children.
///
/// This struct is used internally by the `Div` element to manage the layout
/// state of its children during the UI update cycle. It holds a small vector of
/// `LayoutId` values, each corresponding to a child element of the `Div`. These
/// IDs are used to query the layout engine for the computed bounds of the
/// children after the layout phase is complete.
...
pub struct DivInspectorState {
    /// The inspected element's base style. This is used for both inspecting and
    /// modifying the state. In the future it will make sense to separate
    /// the read and write, possibly tracking the modifications.
    #[cfg(any(feature = "inspector", debug_assertions))]
    pub base_style: Box<StyleRefinement>,
    /// Inspects the bounds of the element.
    pub bounds: Bounds<Pixels>,
    /// Size of the children of the element, or `bounds.size` if it has no
    /// children.
...
impl Element for Div {
    ...
    fn prepaint(
        &mut self, global_id: Option<&GlobalElementId>, inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<Pixels>, request_layout: &mut Self::RequestLayoutState, window: &mut Window,
        cx: &mut App,
    ) -> Option<Hitbox> {
        let image_cache = self
            .image_cache
    ...
    fn paint(
        ...
        hitbox: &mut Option<Hitbox>, window: &mut Window, cx: &mut App,
    ) {
        let image_cache = self
            .image_cache
            .as_mut()
...
pub struct Interactivity {
    /// The element ID of the element. In id is required to support a stateful
    /// subset of the interactivity such as on_click.
    pub element_id: Option<ElementId>,
    /// Whether the element was clicked. This will only be present after layout.
    pub active: Option<bool>,
    /// Whether the element was hovered. This will only be present after paint
    /// if an hitbox was created for the interactive element.
...
impl Interactivity {
    /// Layout this element according to this interactivity state's configured
    /// styles
    pub fn request_layout(
        &mut self, global_id: Option<&GlobalElementId>, _inspector_id: Option<&InspectorElementId>,
        window: &mut Window, cx: &mut App,
    ...
    /// Commit the bounds of this element according to this interactivity
    /// state's configured styles.
    pub fn prepaint<R>(
        &mut self, global_id: Option<&GlobalElementId>, _inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<Pixels>, content_size: Size<Pixels>, window: &mut Window, cx: &mut App,
    ...
    fn clamp_scroll_position(
        &self, bounds: Bounds<Pixels>, style: &Style, window: &mut Window, _cx: &mut App,
    ) -> Point<Pixels> {
    ...
    /// Paint this element according to this interactivity state's configured
    /// styles and bind the element's mouse and keyboard events.
    ///
    /// content_size is the size of the content of the element, which may be
    /// larger than the element's bounds if the element is scrollable.
    ///
    /// the final computed style will be passed to the provided function, along
    /// with the current scroll offset
    pub fn paint(
        &mut self, global_id: Option<&GlobalElementId>, _inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<Pixels>, hitbox: Option<&Hitbox>, window: &mut Window, cx: &mut App,
    ...
    #[cfg(debug_assertions)]
    fn paint_debug_info(
        &self, global_id: Option<&GlobalElementId>, hitbox: &Hitbox, style: &Style,
        window: &mut Window, cx: &mut App,
    ) {
    ...
    fn paint_mouse_listeners(
        &mut self, hitbox: &Hitbox, element_state: Option<&mut InteractiveElementState>,
        window: &mut Window, cx: &mut App,
    ) {
        let is_focused = self
        ...
        if let Some(element_state) = element_state {
                    ...
                    move |event: &MouseUpEvent, phase, window, cx| match phase {
                        ...
                        DispatchPhase::Capture => {
                            ...
                            if pending_mouse_down.is_some() && hitbox.is_hovered(window) {
                                ...
                                // Clear the pending mouse down event (without firing click
                                // handlers) if the hitbox is not
                                // being hovered. This avoids
                                // dragging elements that changed their position
            ...
            // We unconditionally bind both the mouse up and mouse down active state
            // handlers Because we might not get a chance to render a frame
            // before the mouse up event arrives.
    ...
    fn paint_scroll_listener(
        &self, hitbox: &Hitbox, style: &Style, window: &mut Window, _cx: &mut App,
    ) {
    ...
    /// Compute the visual style for this element, based on the current bounds
    /// and the element's state.
    pub fn compute_style(
        &self, global_id: Option<&GlobalElementId>, hitbox: Option<&Hitbox>, window: &mut Window,
        cx: &mut App,
    ) -> Style {
    ...
    /// Called from internal methods that have already called
    /// with_element_state.
    fn compute_style_internal(
        &self, hitbox: Option<&Hitbox>, element_state: Option<&mut InteractiveElementState>,
        window: &mut Window, cx: &mut App,
    ) -> Style {
...
/// Whether or not the element or a group that contains it is clicked by the
/// mouse.
...
pub(crate) enum ActiveTooltip {
    ...
    /// Tooltip is visible, element was hovered or for hoverable tooltips, the
    /// tooltip was hovered.
    Visible {
        tooltip: AnyTooltip,
        is_hoverable: bool,
    },
    /// Tooltip is visible and hoverable, but the mouse is no longer hovering.
    /// Currently delaying before hiding it.
    WaitingForHide {
        tooltip: AnyTooltip,
        _task: Task<()>,
    },
...
pub(crate) fn clear_active_tooltip(
    active_tooltip: &Rc<RefCell<Option<ActiveTooltip>>>, window: &mut Window,
) {
...
pub(crate) fn register_tooltip_mouse_handlers(
    active_tooltip: &Rc<RefCell<Option<ActiveTooltip>>>, tooltip_id: Option<TooltipId>,
    build_tooltip: Rc<dyn Fn(&mut Window, &mut App) -> Option<(AnyView, bool)>>,
    check_is_hovered: Rc<dyn Fn(&Window) -> bool>,
    check_is_hovered_during_prepaint: Rc<dyn Fn(&Window) -> bool>, window: &mut Window,
) {
    window.on_mouse_event({
...
/// where a tooltip gets displayed and then gets occluded after display, it will
/// stick around until the mouse exits the hover bounds.
fn handle_tooltip_mouse_move(
    active_tooltip: &Rc<RefCell<Option<ActiveTooltip>>>,
    build_tooltip: &Rc<dyn Fn(&mut Window, &mut App) -> Option<(AnyView, bool)>>,
    check_is_hovered: &Rc<dyn Fn(&Window) -> bool>,
    check_is_hovered_during_prepaint: &Rc<dyn Fn(&Window) -> bool>, phase: DispatchPhase,
    window: &mut Window, cx: &mut App,
) {
    // Separates logic for what mutation should occur from applying it, to avoid
...
/// Returns a callback which will be called by window prepaint to update tooltip
/// visibility. The purpose of doing this logic here instead of the mouse move
/// handler is that the mouse move handler won't get called when the element is
/// not painted (e.g. via use of `visible_on_hover`).
fn handle_tooltip_check_visible_and_update(
    active_tooltip: &Rc<RefCell<Option<ActiveTooltip>>>, tooltip_is_hoverable: bool,
    check_is_hovered: &Rc<dyn Fn(&Window) -> bool>, tooltip_bounds: Bounds<Pixels>,
    window: &mut Window, cx: &mut App,
) -> bool {
    // Separates logic for what mutation should occur from applying it, to avoid
    // overlapping RefCell borrows.
...
/// A wrapper around an element that can store state, produced after assigning
/// an ElementId.
    ...
    E: Element,
{
    ...
    fn prepaint(
        &mut self, id: Option<&GlobalElementId>, inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<Pixels>, state: &mut Self::RequestLayoutState, window: &mut Window,
        cx: &mut App,
    ) -> E::PrepaintState {
        self.element
...
impl ScrollHandle {
    ...
    /// Update [ScrollHandleState]'s active item for scrolling to in prepaint
    /// This scrolls the minimal amount to ensure that the child is the first
    /// visible element
    ...
    /// Set the offset explicitly. The offset is the distance from the top left
    /// of the parent container to the top left of the first child.
    ...
    /// Get the logical scroll bottom, based on a child index and a pixel
    /// offset.
...
mod tests {
    ...
    impl Element for TooltipCaptureElement {
        ...
        fn paint(
            &mut self, _id: Option<&GlobalElementId>, _inspector_id: Option<&InspectorElementId>,
            _bounds: Bounds<Pixels>, _request_layout: &mut Self::RequestLayoutState,
            _prepaint: &mut Self::PrepaintState, window: &mut Window, cx: &mut App,
        ) {
