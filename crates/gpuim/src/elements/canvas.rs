...
/// A canvas element, meant for accessing the low level paint API without
/// defining a whole custom element
pub struct Canvas<T> {
...
impl<T: 'static> Element for Canvas<T> {
    ...
    fn prepaint(
        &mut self, _id: Option<&GlobalElementId>, _inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<Pixels>, _request_layout: &mut Style, window: &mut Window, cx: &mut App,
    ) -> Option<T> {
    ...
    fn paint(
        &mut self, _id: Option<&GlobalElementId>, _inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<Pixels>, style: &mut Style, prepaint: &mut Self::PrepaintState,
        window: &mut Window, cx: &mut App,
    ) {
