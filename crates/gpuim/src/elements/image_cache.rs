...
/// An image cache element, all its child img elements will use the cache
/// specified by this element. Note that this could as simple as passing an
/// `Entity<T: ImageCache>`
...
impl AnyImageCache {
    ...
    /// returns the result of loading the image if it has finished loading, or
    /// None if it is still loading
    pub fn load(
        &self, resource: &Resource, window: &mut Window, cx: &mut App,
    ) -> Option<Result<Arc<RenderImage>, ImageCacheError>> {
...
mod any_image_cache {
    ...
    pub(crate) fn load<I: 'static + ImageCache>(
        image_cache: &AnyEntity, resource: &Resource, window: &mut Window, cx: &mut App,
    ) -> Option<Result<Arc<RenderImage>, ImageCacheError>> {
...
/// Implementations of this trait should ensure that images are removed from all
/// windows when they are no longer needed.
...
/// An implementation of ImageCache, that uses an LRU caching strategy to unload
...
/// Constructs a retain-all image cache that uses the element state associated
/// with the given ID.
