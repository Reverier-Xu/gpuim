//! A list element that can be used to render a large number of differently
//! sized elements efficiently. Clients of this API need to ensure that elements
//! outside of the scrolled area do not change their height for this element to
//! function correctly. If your elements do change height, notify the list
//! element via [`ListState::splice`] or [`ListState::reset`]. In order to
//! minimize re-renders, this element's state is stored intrusively on your own
//! views, so that your code can coordinate directly with the list element's
//! cached state.
//!
//! If all of your elements are the same height, see [`crate::UniformList`] for
//! a simpler API
...
pub fn list(
    state: ListState, render_item: impl FnMut(usize, &mut Window, &mut App) -> AnyElement + 'static,
) -> List {
    List {
        state,
...
pub enum FollowMode {
    ...
    #[default]
    Normal,
    /// The list should auto-scroll along with the tail, when scrolled to
    /// bottom.
    Tail,
}
...
pub struct ListScrollEvent {
    /// The range of items currently visible in the list, after applying the
    /// scroll event.
    pub visible_range: Range<usize>,

    /// The number of items that are currently visible in the list, after
    /// applying the scroll event.
    pub count: usize,

    /// Whether the list has been scrolled.
    pub is_scrolled: bool,

    /// Whether the list is currently in follow-tail mode (auto-scrolling to
    /// end).
...
pub enum ListHorizontalSizingBehavior {
    ...
    #[default]
    FitList,
    /// List items' width may go over the width of the list, if any item is
    /// wider.
    Unconstrained,
}
...
impl ListState {
    ...
    ///
    /// This is useful for ensuring that the scrollbar size is correct instead
    /// of based on only rendered elements.
    ...
    ///
    /// Note that this will cause scroll events to be dropped until the next
    /// paint.
    ...
    /// replaced by new items. As opposed to [`Self::splice`], this method
    /// allows an iterator of optional focus handles to be supplied to
    /// properly integrate with items in the list that can be focused. If a
    /// focused item is scrolled out of view, the list will continue to
    /// render it to allow keyboard interaction.
    pub fn splice_focusable(
        &self, old_range: Range<usize>,
    ...
    pub fn set_scroll_handler(
        &self, handler: impl FnMut(&ListScrollEvent, &mut Window, &mut App) + 'static,
    ) {
    ...
    ///
    /// This will prevent the height reported to the scrollbar from changing
    /// during the drag as items in the overdraw get measured, and help
    /// offset scroll position changes accordingly.
    ...
    /// Returns the maximum scroll offset according to the items we have
    /// measured. This value remains constant while dragging to prevent the
    /// scrollbar from moving away unexpectedly.
...
impl StateInner {
    ...
    fn visible_range(
        items: &SumTree<ListItem>, height: Pixels, scroll_top: &ListOffset,
    ) -> Range<usize> {
    ...
    fn scroll(
        &mut self, scroll_top: &ListOffset, height: Pixels, delta: Point<Pixels>,
        current_view: EntityId, window: &mut Window, cx: &mut App,
    ) {
    ...
    fn layout_all_items(
        &mut self, available_width: Pixels, render_item: &mut RenderItemFn, window: &mut Window,
        cx: &mut App,
    ) {
    ...
    fn layout_items(
        &mut self, available_width: Option<Pixels>, available_height: Pixels,
        padding: &Edges<Pixels>, render_item: &mut RenderItemFn, window: &mut Window, cx: &mut App,
    ) -> LayoutItemsResponse {
        ...
        for (ix, item) in cursor.by_ref().enumerate() {
            ...
            // If we're within the visible area or the height wasn't cached, render and
            // measure the item's element
        ...
        // If none of the visible items are focused, check if an off-screen item is
        // focused and include it to be rendered after the visible items so
        // keyboard interaction continues to work for it.
    ...
    fn prepaint_items(
        &mut self, bounds: Bounds<Pixels>, padding: Edges<Pixels>, autoscroll: bool,
        render_item: &mut RenderItemFn, window: &mut Window, cx: &mut App,
        ...
        window.transact(|window| {
            ...
            // Only paint the visible items, if there is actually any space for them (taking
                ...
                for item in &mut layout_response.item_layouts {
                        ...
                        && autoscroll
                    {
                        if autoscroll_bounds.top() < bounds.top() {
                            ...
                            // Account for the height of the element down until the autoscroll
                            // bottom.
...
impl Element for List {
            ...
            ListSizingBehavior::Infer => {
                ...
                window.with_text_style(style.text_style().cloned(), |window| {
                    ...
                    let available_height = if let Some(last_bounds) = state.last_layout_bounds {
                        ...
                        // we might just use the overdraw value as the available height to layout
                        // enough items.
                        state.overdraw
                    };
    ...
    fn prepaint(
        &mut self, _id: Option<&GlobalElementId>, _inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<Pixels>, _: &mut Self::RequestLayoutState, window: &mut Window,
        cx: &mut App,
    ) -> ListPrepaintState {
        let state = &mut *self.state.0.borrow_mut();
        state.reset = false;
    ...
    fn paint(
        &mut self, _id: Option<&GlobalElementId>, _inspector_id: Option<&InspectorElementId>,
        bounds: Bounds<crate::Pixels>, _: &mut Self::RequestLayoutState,
...
mod test {
    ...
    use gpuim::{ScrollDelta, ScrollWheelEvent};

    use crate::{
        self as gpuim, AppContext, Context, Element, FollowMode, IntoElement, ListState, Render,
    ...
    fn test_reset_after_paint_before_scroll(cx: &mut TestAppContext) {
        ...
        state.scroll_to(gpuim::ListOffset {
            item_ix: 0,
    ...
    /// Calling `set_follow_mode(FollowState::Normal)` or dragging the scrollbar
    /// should fully disengage follow_tail — clearing any suspended state so
