...
impl KeyContext {
    /// Initialize a new [`KeyContext`] that contains an `os` key set to either
...
pub enum KeyBindingContextPredicate {
    ...
    /// A predicate that will match a given predicate appearing below another
    /// predicate. in the element tree
    Descendant(
...
impl KeyBindingContextPredicate {
    ...
    ///
    /// `StatusBar` -> A predicate that will match a context with the identifier
    /// `StatusBar`
    ///
    /// You can also specify a key-value pair:
    ///
    /// `mode == visible` -> A predicate that will match a context with the key
    /// `mode`                      with the value `visible`
    ///
    /// And a logical operations combining these two checks:
    ///
    /// `StatusBar && mode == visible` -> A predicate that will match a context
    /// with the                                   identifier `StatusBar`
    /// and the key `mode`                                   with the value
    /// `visible`
    ///
    ///
    /// There is also a special child `>` operator that will match a predicate
    /// that is below another predicate:
    ///
    /// `StatusBar > mode == visible` -> A predicate that will match a context
    /// identifier `StatusBar`                                  and a child
    /// context that has the key `mode` with the                            
    /// value `visible`
    ///
    ...
    /// Eval a predicate against a set of contexts, arranged from lowest to
    /// highest.
    #[allow(unused)]
    ...
    /// Returns whether or not this predicate matches all possible contexts
    /// matched by the other predicate.
    pub fn is_superset(&self, other: &Self) -> bool {
        ...
        match other {
            ...
            KeyBindingContextPredicate::Equal(..) => false,
            KeyBindingContextPredicate::NotEqual(..) => false,
            KeyBindingContextPredicate::Not(_) => false,
            KeyBindingContextPredicate::Or(..) => false,
        }
    }
    ...
    fn fmt_joined(
        &self, f: &mut fmt::Formatter<'_>, separator: &str, operator: LogicalOperator,
    ...
    fn fmt_joined_inner(
        &self, f: &mut fmt::Formatter<'_>, separator: &str, operator: LogicalOperator,
        needs_parens: impl Fn(&Self) -> bool + Copy, first: &mut bool,
    ) -> fmt::Result {
        match (operator, self) {
...
mod tests {
    ...
    fn test_context_display() {
        ...
        fn and(
            a: Box<KeyBindingContextPredicate>, b: Box<KeyBindingContextPredicate>,
        ) -> Box<KeyBindingContextPredicate> {
            Box::new(And(a, b))
        }
        fn or(
