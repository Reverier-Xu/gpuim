...
/// A helper trait for building complex objects with imperative conditionals in
/// a fluent style.
pub trait FluentBuilder {
    ...
    /// Conditionally modify self with the given closure.
    fn when_else(
        self, condition: bool, then: impl FnOnce(Self) -> Self, else_fn: impl FnOnce(Self) -> Self,
    ) -> Self
    where
        Self: Sized, {
...
/// Extensions for Future types that provide additional combinators and
/// utilities.
pub trait FutureExt {
...
/// Error returned by with_timeout when the timeout duration elapsed before the
/// future resolved
pub struct Timeout;
