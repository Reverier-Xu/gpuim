...
pub trait Platform: 'static {
    ...
    fn update_jump_list(
        &self, _menus: Vec<MenuItem>, _entries: Vec<SmallVec<[PathBuf; 2]>>,
...
pub trait PlatformDisplay: Debug {
    ...
    /// Returns a stable identifier for this display that can be persisted and
    /// used across system restarts.
    ...
    /// This is the usable area where windows can be placed without being
    /// obscured. Defaults to the full display bounds if not overridden.
...
pub trait ScreenCaptureSource {
    ...
    /// with each frame.
    fn stream(
        &self, foreground_executor: &ForegroundExecutor,
...
impl WindowButtonLayout {
    ...
    /// Parses a GNOME-style `button-layout` string (e.g.
    /// `"close,minimize:maximize"`).
    pub fn parse(layout_string: &str) -> Result<Self> {
        fn parse_side(
            s: &str, seen_buttons: &mut [bool; MAX_BUTTONS_PER_SIDE],
...
pub trait PlatformWindow: HasWindowHandle + HasDisplayHandle {
    ...
    /// Renders the given scene to a texture and returns the pixel data as an
    /// RGBA image. This does not present the frame to screen - useful for
    /// visual testing where we want to capture what would be rendered
    /// without displaying it or requiring the window to be visible.
    #[cfg(any(test, feature = "test-support"))]
...
/// Previously an enum with a single variant, now simplified to a direct type
/// alias.
#[doc(hidden)]
pub type RunnableVariant = Runnable<RunnableMeta>;

#[doc(hidden)]
pub type TimerResolutionGuard = gpuim_util::Deferred<Box<dyn FnOnce() + Send>>;

/// This type is public so that our test macro can generate and use it, but it
/// should not be considered part of our public API.
#[doc(hidden)]
...
impl PlatformTextSystem for NoopTextSystem {
    ...
    fn rasterize_glyph(
        &self, _params: &RenderGlyphParams, raster_bounds: Bounds<DevicePixels>,
    ) -> Result<(Size<DevicePixels>, Vec<u8>)> {
    ...
    fn recommended_rendering_mode(
        &self, _font_id: FontId, _font_size: Pixels,
    ) -> TextRenderingMode {
...
impl PlatformInputHandler {
    ...
    pub fn text_for_range(
        &mut self, range_utf16: Range<usize>, adjusted: &mut Option<Range<usize>>,
    ) -> Option<String> {
        self.cx
            .update(|window, cx| {
                self.handler
...
pub struct UTF16Selection {
    ...
    /// Whether the head of this selection is at the start (true), or end
    /// (false) of the range
    pub reversed: bool,
}
...
pub trait InputHandler: 'static {
    ...
    ///
    /// Return value is in terms of UTF-16 characters, from 0 to the length of
    /// the document
    fn selected_text_range(
        &mut self, ignore_disabled_input: bool, window: &mut Window, cx: &mut App,
    ...
    fn replace_text_in_range(
        &mut self, replacement_range: Option<Range<usize>>, text: &str, window: &mut Window,
        cx: &mut App,
    );
    ...
    fn replace_and_mark_text_in_range(
        &mut self, range_utf16: Option<Range<usize>>, new_text: &str,
        new_selected_range: Option<Range<usize>>, window: &mut Window, cx: &mut App,
    );
    ...
    fn bounds_for_range(
        &mut self, range_utf16: Range<usize>, window: &mut Window, cx: &mut App,
    ) -> Option<Bounds<Pixels>>;

    /// Get the character offset for the given point in terms of UTF16
    /// characters
    ///
    ...
    /// Allows a given input context to opt into getting raw key repeats instead
    /// of sending these to the platform.
    /// TODO: Ideally we should be able to set ApplePressAndHoldEnabled in
    /// NSUserDefaults (which is how iTerm does it) but it doesn't seem to
    /// work for me.
    #[allow(dead_code)]
    ...
    /// Returns whether printable keys should be routed to the IME before
    /// keybinding matching when a non-ASCII input source (e.g. Japanese,
    /// Korean, Chinese IME) is active. This prevents multi-stroke
    /// keybindings like `jj` from intercepting keys that the IME should
    /// compose.
    ///
    /// Defaults to `false`. The editor overrides this based on whether it
    /// expects character input (e.g. Vim insert mode returns `true`, normal
    /// mode returns `false`). The terminal keeps the default `false` so
    /// that raw keys reach the terminal process.
...
pub struct WindowOptions {
    ...
    /// - `Some(WindowBounds)`: Open a window with corresponding state and its
    ///   restore size.
    ...
    /// Application identifier of the window. Can by used by desktop
    /// environments to group applications together.
    ...
    /// Tab group name, allows opening the window as a native tab on macOS
    /// 10.12+. Windows with the same tabbing identifier will be grouped
    /// together.
...
pub enum WindowBounds {
    /// Indicates that the window should open in a windowed state with the given
    /// bounds.
...
pub struct TitlebarOptions {
    ...
    /// Should the default system titlebar be hidden to allow for a custom-drawn
    /// titlebar? (macOS and Windows only)
...
pub enum WindowKind {
    ...
    /// A window that appears above all other windows, usually used for alerts
    /// or popups use sparingly!
    PopUp,

    /// A floating window that appears on top of its parent window
    Floating,

    /// A Wayland LayerShell window, used to draw overlays or backgrounds for
    /// applications such as docks, notifications or wallpapers.
    #[cfg(all(target_os = "linux", feature = "wayland"))]
    LayerShell(layer_shell::LayerShellOptions),

    /// A window that appears on top of its parent window and blocks interaction
    /// with it until the modal window is closed
    Dialog,
}
...
pub enum WindowAppearance {
    ...
    ///
    /// On macOS, this corresponds to the `NSAppearanceNameVibrantLight`
    /// appearance.
    VibrantLight,
    ...
    ///
    /// On macOS, this corresponds to the `NSAppearanceNameVibrantDark`
    /// appearance.
    VibrantDark,
}
...
pub enum PromptLevel {
    ...
    /// A prompt that is shown when the user needs to be warned of a potential
    /// problem
    Warning,
...
impl ClipboardItem {
    ...
    /// Create a new ClipboardItem::Image with the given image with no
    /// associated metadata
...
/// One of the editor's supported image formats (e.g. PNG, JPEG) - used when
/// dealing with images in the clipboard
...
impl Image {
    ...
    pub fn use_render_image(
        self: Arc<Self>, window: &mut Window, cx: &mut App,
    ...
    pub fn to_image_data(&self, svg_renderer: SvgRenderer) -> Result<Arc<RenderImage>> {
        fn frames_for_image(
            bytes: &[u8], format: image::ImageFormat,
...
impl ClipboardString {
    ...
    /// Return a new clipboard item with the metadata replaced by the given
    /// metadata, after serializing it as JSON.
