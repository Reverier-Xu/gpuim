...
//! ```
//!
//! The keybindings themselves are managed independently by calling
//! cx.bind_keys(). (Though mostly when developing Zed itself, you just need to
//! add a new line to  assets/keymaps/default-{platform}.json).
//!
//! ```ignore
//! cx.bind_keys([
...
//! ```
//!
//! With all of this in place, GPUI will ensure that if you have an Editor that
//! contains the focus, hitting cmd-z will Undo.
//!
//! In real apps, it is a little more complicated than this, because typically
//! you have several nested views that each register keyboard handlers. In this
//! case action matching bubbles up from the bottom. For example in Zed, the
//! Workspace is the top-level view, which contains Pane's, which contain
//! Editors. If there are conflicting keybindings defined then the Editor's
//! bindings take precedence over the Pane's bindings, which take precedence
//! over the Workspace.
//!
//! In GPUI, keybindings are not limited to just single keystrokes, you can
//! define sequences by separating the keys with a space:
//!
...
/// ID of a node within `DispatchTree`. Note that these are **not** stable
/// between frames, and so a `DispatchNodeId` should only be used with the
/// `DispatchTree` that provided it.
...
impl DispatchTree {
    ...
    pub fn reuse_subtree(
        &mut self, old_range: Range<usize>, source: &mut Self, focus: Option<FocusId>,
    ) -> ReusedSubtree {
    ...
    ///
    /// Bindings are only included if they are the highest precedence match for
    /// their keystrokes, so shadowed bindings are not included.
    pub fn bindings_for_action(
        &self, action: &dyn Action, context_stack: &[KeyContext],
    ) -> Vec<KeyBinding> {
        // Ideally this would return a `DoubleEndedIterator` to avoid
        // `highest_precedence_*` methods, but this can't be done very cleanly
        // since keymap must be borrowed.
    ...
    /// Returns the highest precedence binding for the given action and context
    /// stack. This is the same as the last result of `bindings_for_action`,
    /// but more efficient than getting all bindings.
    ...
    fn binding_matches_predicate_and_not_shadowed(
        keymap: &Keymap, binding: &KeyBinding, context_stack: &[KeyContext],
    ) -> bool {
    ...
    fn bindings_for_input(
        &self, input: &[Keystroke], dispatch_path: &SmallVec<[DispatchNodeId; 32]>,
    ...
    pub fn possible_next_bindings_for_input(
        &self, input: &[Keystroke], context_stack: &[KeyContext],
    ) -> Vec<KeyBinding> {
        self.keymap
            .borrow()
    ...
    /// input should be set to the value of `pending` from the previous call to
    /// dispatch_key. This returns three instructions to the input handler:
    /// - bindings: any bindings to execute before processing this keystroke
    /// - pending: the new set of pending keystrokes to store
    /// - to_replay: any keystroke that had been pushed to pending, but are
    ///   no-longer matched, these should be replayed first.
    pub fn dispatch_key(
        &mut self, mut input: SmallVec<[Keystroke; 1]>, keystroke: Keystroke,
    ...
    /// If the user types a matching prefix of a binding and then waits for a
    /// timeout flush_dispatch() converts any previously pending input to
    /// replay events.
    pub fn flush_dispatch(
        &mut self, input: SmallVec<[Keystroke; 1]>, dispatch_path: &SmallVec<[DispatchNodeId; 32]>,
    ) -> SmallVec<[Replay; 1]> {
        let (suffix, mut to_replay) = self.replay_prefix(input, dispatch_path);
    ...
    /// Converts the longest prefix of input to a replay event and returns the
    /// rest.
    fn replay_prefix(
        &self, mut input: SmallVec<[Keystroke; 1]>, dispatch_path: &SmallVec<[DispatchNodeId; 32]>,
    ) -> (SmallVec<[Keystroke; 1]>, SmallVec<[Replay; 1]>) {
        let mut to_replay: SmallVec<[Replay; 1]> = Default::default();
        for last in (0..input.len()).rev() {
            let (bindings, ..) = self.bindings_for_input(&input[0..=last], dispatch_path);
...
mod tests {
    ...
    use crate::{
        self as gpuim, ActionRegistry, App, AppContext, Bounds, Context, DispatchResult,
    ...
    fn test_pending_has_binding_state() {
        ...
        fn dispatch(
            tree: &mut DispatchTree, pending: SmallVec<[Keystroke; 1]>, key: &str,
            path: &DispatchPath,
        ) -> DispatchResult {
    ...
    fn test_pending_input_observers_notified_on_focus_change(cx: &mut TestAppContext) {
        ...
        impl Element for CustomElement {
            ...
            fn prepaint(
                &mut self, _: Option<&GlobalElementId>, _: Option<&InspectorElementId>,
                _: Bounds<Pixels>, _: &mut Self::RequestLayoutState, window: &mut Window,
                cx: &mut App,
            ) -> Self::PrepaintState {
        ...
        impl InputHandler for CustomElement {
            fn selected_text_range(
                &mut self, _: bool, _: &mut Window, _: &mut App,
            ...
            fn text_for_range(
                &mut self, _: Range<usize>, _: &mut Option<Range<usize>>, _: &mut Window,
                _: &mut App,
            ) -> Option<String> {
                None
            }
            ...
            fn replace_text_in_range(
                &mut self, replacement_range: Option<Range<usize>>, text: &str, _: &mut Window,
                _: &mut App,
            ) {
            ...
            fn replace_and_mark_text_in_range(
                &mut self, replacement_range: Option<Range<usize>>, new_text: &str,
                _: Option<Range<usize>>, _: &mut Window, _: &mut App,
            ) {
                if replacement_range.is_some() {
                    unimplemented!()
                }
            ...
            fn bounds_for_range(
                &mut self, _: Range<usize>, _: &mut Window, _: &mut App,
            ) -> Option<Bounds<Pixels>> {
                None
            }

            fn character_index_for_point(
                &mut self, _: Point<Pixels>, _: &mut Window, _: &mut App,
            ) -> Option<usize> {
                None
            }
        }
        ...
        // Focus-triggered pending-input notifications are deferred to the end of the
        // current effect cycle, so the observer callback should run after the
        // focus update completes.
        cx.update(|_, _| {
