    ...
    Callback: 'static,
{
    ...
    /// Inserts a new [`Subscription`] for the given `emitter_key`. By default,
    /// subscriptions are inert, meaning that they won't be listed when
    /// calling `[SubscriberSet::remove]` or `[SubscriberSet::retain]`. This
    /// method returns a tuple of a [`Subscription`] and an `impl FnOnce`, and
    /// you can use the latter to activate the [`Subscription`].
    pub fn insert(
        &self, emitter_key: EmitterKey, callback: Callback,
