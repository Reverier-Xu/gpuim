...
impl Keymap {
    ...
    /// Iterate over all bindings for the given action, in the order they were
    /// added. For display, the last binding should take precedence.
    pub fn bindings_for_action<'a>(
        &'a self, action: &'a dyn Action,
    ...
    /// Returns all bindings that might match the input without checking
    /// context. The bindings returned in precedence order (reverse of the
    /// order they were added to the keymap).
    ...
    /// Returns a list of bindings that match the given input, and a boolean
    /// indicating whether or not more bindings might match if the input was
    /// longer. Bindings are returned in precedence order (higher precedence
    /// first, reverse of the order they were added to the keymap).
    ///
    /// Precedence is defined by the depth in the tree (matches on the Editor
    /// take precedence over matches on the Pane, then the Workspace, etc.).
    /// Bindings with no context are treated as the same as the deepest
    /// context.
    ///
    /// In the case of multiple bindings at the same depth, the ones added to
    /// the keymap later take precedence. User bindings are added after
    /// built-in bindings so that they take precedence.
    ///
    /// If a user has disabled a binding with `"x": null` it will not be
    /// returned. Disabled bindings are evaluated with the same precedence
    /// rules so you can disable a rule in a given context only.
    pub fn bindings_for_input(
        &self, input: &[impl AsKeystroke], context_stack: &[KeyContext],
    ...
    /// Returns the deepest depth at which the binding matches, or None if it
    /// doesn't match.
...
mod tests {
    ...
    fn test_multiple_keystroke_binding_disabled() {
        ...
        // Ensure `space w w` results in the binding in the workspace, but not in the
        // editor
    ...
    fn test_context_precedence_with_same_source() {
        ...
        // Both bindings should be returned, but Editor binding should be first (highest
        // precedence)
        assert_eq!(result.len(), 2);
    ...
    fn test_source_precedence_sorting() {
        ...
        // Test that user keymaps take precedence over default keymaps at the same
        // context depth
        let mut keymap = Keymap::default();
