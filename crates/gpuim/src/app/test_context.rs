...
/// A TestAppContext is provided to tests created with `#[gpuim::test]`, it
/// provides an implementation of `Context` with additional methods that are
/// useful in tests.
#[derive(Clone)]
...
impl AppContext for TestAppContext {
    ...
    fn insert_entity<T: 'static>(
        &mut self, reservation: crate::Reservation<T>,
...
impl TestAppContext {
    /// Creates a new `TestAppContext`. Usually you can rely on `#[gpuim::test]`
    /// to do this for you.
    ...
    /// Checks whether there have been any new path prompts received by the
    /// platform.
    pub fn did_prompt_for_new_path(&self) -> bool {
        self.test_platform.did_prompt_for_new_path()
    }

    /// returns a new `TestAppContext` re-using the same executors to interleave
    /// tasks.
    ...
    /// Adds a new window. The Window will always be backed by a `TestWindow`
    /// which can be retrieved with `self.test_window(handle)`
    ...
    pub fn open_window<F, V>(
        &mut self, window_size: Size<Pixels>, build_window: F,
    ) -> WindowHandle<V>
    where
        F: FnOnce(&mut Window, &mut Context<V>) -> V,
        V: 'static + Render, {
    ...
    /// Adds a new window, and returns its root view and a `VisualTestContext`
    /// which can be used as a `Window` and `App` for the rest of the test.
    /// Typically you would shadow this context with the returned one. `let
    /// (view, cx) = cx.add_window_view(...);`
    pub fn add_window_view<F, V>(
        &mut self, build_root_view: F,
    ...
    pub fn simulate_new_path_selection(
        &self, select_path: impl FnOnce(&std::path::Path) -> Option<std::path::PathBuf>,
    ) {
    ...
    /// Causes the given sources to be returned if the application queries for
    ...
    /// updates the global in this context. (panics if `has_global` would return
    /// false)
    ...
    /// Returns an `AsyncApp` which can be used to run tasks that expect to be
    /// on a background thread on the current thread in tests.
    ...
    /// Simulate dispatching an action to the currently focused node in the
    /// window.
    ...
    /// in Zed, this will run backspace on the current editor through the
    /// command palette. This will also run the background executor until
    /// it's parked.
    ...
    /// dispatches a single Keystroke (see also `simulate_keystrokes` and
    /// `simulate_input`)
    ...
    /// Runs until the given condition becomes true. (Prefer `run_until_parked`
    /// if you don't need to jump in at a specific time).
    pub async fn condition<T: 'static>(
        &mut self, entity: &Entity<T>, mut predicate: impl FnMut(&mut T, &mut Context<T>) -> bool,
    ) {
...
impl<V: 'static> Entity<V> {
    ...
    pub fn next_notification(
        &self, advance_clock_by: Duration, cx: &TestAppContext,
    ) -> impl Future<Output = ()> {
...
impl<V> Entity<V> {
    ...
    pub fn condition<Evt>(
        &self, cx: &TestAppContext, mut predicate: impl FnMut(&V, &App) -> bool,
    ) -> impl Future<Output = ()>
    where
        Evt: 'static,
        V: EventEmitter<Evt>, {
...
/// A VisualTestContext is the test-equivalent of a `Window` and `App`. It
/// allows you to run window-specific test code. It can be dereferenced to a
/// `TextAppContext`.
pub struct VisualTestContext {
    #[deref]
    #[deref_mut]
    /// cx is the original TestAppContext (you can more easily access this using
    /// Deref)
...
impl VisualTestContext {
    ...
    /// Creates a new VisualTestContext. You would typically shadow the passed
    /// in TestAppContext with this, as this is typically more useful.
    ...
    /// Simulate a sequence of keystrokes `cx.simulate_keystrokes("cmd-p
    /// escape")` Automatically runs until parked.
    ...
    pub fn simulate_mouse_move(
        &mut self, position: Point<Pixels>, button: impl Into<Option<MouseButton>>,
        modifiers: Modifiers,
    ) {
    ...
    pub fn simulate_mouse_down(
        &mut self, position: Point<Pixels>, button: MouseButton, modifiers: Modifiers,
    ) {
    ...
    pub fn draw<E>(
        &mut self, origin: Point<Pixels>, space: impl Into<Size<AvailableSpace>>,
    ...
    /// Get an &mut VisualTestContext (which is mostly what you need to pass to
    /// other methods). This method internally retains the VisualTestContext
...
impl VisualContext for VisualTestContext {
    ...
    fn update_window_entity<V: 'static, R>(
        &mut self, view: &Entity<V>, update: impl FnOnce(&mut V, &mut Window, &mut Context<V>) -> R,
    ) -> R {
        self.window
            .update(&mut self.cx, |_, window, cx| {
    ...
    fn replace_root_view<V>(
        &mut self, build_view: impl FnOnce(&mut Window, &mut Context<V>) -> V,
    ) -> Entity<V>
    where
        V: 'static + Render, {
        self.window
...
impl AnyWindowHandle {
    ...
    pub fn build_entity<V: Render + 'static>(
        &self, cx: &mut TestAppContext, build_view: impl FnOnce(&mut Window, &mut Context<V>) -> V,
    ) -> Entity<V> {
