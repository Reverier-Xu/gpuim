...
use super::Context;
use crate::{
    App, AppContext, GpuiBorrow, VisualContext, Window, seal::Sealed, util::atomic_incr_if_not_zero,
};
...
impl EntityMap {
    ...
    /// Reserve a slot for an entity, which you can subsequently use with
    /// `insert`.
...
impl AnyEntity {
    fn new(
        id: EntityId, entity_type: TypeId, entity_map: Weak<RwLock<EntityRefCounts>>,
    ...
    /// Converts this entity handle into a weak variant, which does not prevent
    ...
    /// Converts this entity handle into a strongly-typed entity handle of the
    /// given type. If this entity handle is not of the specified type,
    /// returns itself as an error variant.
...
impl<T: 'static> Entity<T> {
    ...
    pub fn update<R, C: AppContext>(
        &self, cx: &mut C, update: impl FnOnce(&mut T, &mut Context<T>) -> R,
    ) -> R {
    ...
    /// the referenced entity still exists, within a visual context that has a
    /// window. Returns an error if the window has been closed.
    #[inline]
...
impl AnyWeakEntity {
    ...
    /// Check if this weak handle can be upgraded, or if the entity has already
    /// been dropped
    ...
    /// Asserts that the entity referenced by this weak handle has been fully
    /// released.
    ///
    /// # Example
    ///
    ...
    /// # Debugging Leaks
    ///
    /// If this method panics due to leaked handles, set the `LEAK_BACKTRACE`
    /// environment variable to see where the leaked handles were allocated:
    ///
    /// ```bash
    ...
    /// - Panics if the entity was recently dropped but cleanup hasn't completed
    ///   yet (resources are retained until the end of the effect cycle).
    #[cfg(any(test, feature = "leak-detection"))]
    ...
    pub fn new_invalid() -> Self {
        /// To hold the invariant that all ids are unique, and considering that
        /// slotmap increases their IDs from `0`, we can decrease ours
        /// from `u64::MAX` so these two will never conflict (u64 is way
        /// too large).
...
impl<T: 'static> WeakEntity<T> {
    ...
    /// Updates the entity referenced by this handle with the given function if
    /// the referenced entity still exists, within a visual context that has a
    /// window. Returns an error if the entity has been released.
...
///
/// Set the `LEAK_BACKTRACE` environment variable to any non-empty value to
/// enable backtrace capture. This helps identify where leaked handles were
/// allocated.
#[cfg(any(test, feature = "leak-detection"))]
...
///
/// This is distinct from `EntityId` - while multiple handles can point to the
/// same entity (same `EntityId`), each handle has its own unique `HandleId`.
#[cfg(any(test, feature = "leak-detection"))]
...
///
/// The leak detector is enabled in tests and when the `leak-detection` feature
/// is active. It tracks every `Entity<T>` and `AnyEntity` handle that is
/// created and released, allowing you to verify that all handles to an entity
/// have been properly dropped.
///
/// # How do leaks happen?
///
...
/// # Usage
///
/// You can use `WeakEntity::assert_released` or
/// `AnyWeakEntity::assert_released` to verify that an entity has been fully
/// released:
///
/// ```ignore
/// let entity = cx.new(|_| MyEntity::new());
...
/// # Debugging Leaks
///
/// When a leak is detected, the detector will panic with information about the
/// leaked handles. To see where the leaked handles were allocated, set the
/// `LEAK_BACKTRACE` environment variable:
///
/// ```bash
/// LEAK_BACKTRACE=1 cargo test my_test
/// ```
...
impl LeakDetector {
    ...
    ///
    /// Returns a unique `HandleId` that must be passed to `handle_released`
    /// when the handle is dropped. If `LEAK_BACKTRACE` is set, captures a
    /// backtrace at the allocation site.
    #[track_caller]
    pub fn handle_created(
        &mut self, entity_id: EntityId, type_name: Option<&'static str>,
    ) -> HandleId {
        let id = gpuim_util::post_inc(&mut self.next_handle_id);
    ...
    ///
    /// This removes the handle from tracking. The `handle_id` should be the
    /// same one returned by `handle_created` when the handle was allocated.
