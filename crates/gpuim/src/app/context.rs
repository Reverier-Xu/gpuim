...
impl<'a, T: 'static> Context<'a, T> {
    ...
    /// Arranges for the given function to be called whenever
    /// [`Context::notify`] is called with the given entity.
    pub fn observe<W>(
    ...
    pub fn observe_self(
        &mut self, mut on_event: impl FnMut(&mut T, &mut Context<T>) + 'static,
    ) -> Subscription
    where
        T: 'static, {
    ...
    pub fn subscribe_self<Evt>(
        &mut self, mut on_event: impl FnMut(&mut T, &Evt, &mut Context<T>) + 'static,
    ) -> Subscription
    where
    ...
    pub fn observe_global<G: 'static>(
        &mut self, mut f: impl FnMut(&mut T, &mut Context<T>) + 'static,
    ) -> Subscription
    where
        T: 'static, {
    ...
    pub fn on_app_restart(
        &self, mut on_restart: impl FnMut(&mut T, &mut App) + 'static,
    ) -> Subscription
    where
        T: 'static, {
        let handle = self.weak_entity();
    ...
    /// Arrange for the given function to be invoked whenever the application is
    /// quit. The future returned from this callback will be polled for up
    /// to [crate::SHUTDOWN_TIMEOUT] until the app fully quits.
    pub fn on_app_quit<Fut>(
        &self, mut on_quit: impl FnMut(&mut T, &mut Context<T>) -> Fut + 'static,
    ) -> Subscription
    where
    ...
    /// Tell GPUI that this entity has changed and observers of it should be
    /// notified.
    ...
    /// The function is provided a weak handle to the entity owned by this
    /// context and a context that can be held across await points.
    ...
    pub fn listener<E: ?Sized>(
        &self, f: impl Fn(&mut T, &E, &mut Window, &mut Context<T>) + 'static,
    ...
    pub fn processor<E, R>(
        &self, f: impl Fn(&mut T, E, &mut Window, &mut Context<T>) -> R + 'static,
    ...
    /// Run something using this entity and cx, when the returned struct is
    /// dropped
    pub fn on_drop(
        &self, f: impl FnOnce(&mut T, &mut Context<T>) + 'static,
    ) -> Deferred<impl FnOnce()> {
        let this = self.weak_entity();
        let mut cx = self.to_async();
        gpuim_util::defer(move || {
            this.update(&mut cx, f).ok();
        })
    }

    /// Focus the given view in the given window. View type is required to
    /// implement Focusable.
    ...
    pub fn on_next_frame(
        &self, window: &mut Window, f: impl FnOnce(&mut T, &mut Window, &mut Context<T>) + 'static,
    ) where
        T: 'static, {
        let view = self.entity();
    ...
    /// returned to the app.
    pub fn defer_in(
        &mut self, window: &Window, f: impl FnOnce(&mut T, &mut Window, &mut Context<T>) + 'static,
    ) {
        let view = self.entity();
    ...
    /// Observe another entity for changes to its state, as tracked by
    /// [`Context::notify`].
    pub fn observe_in<V2>(
        &mut self, observed: &Entity<V2>, window: &mut Window,
    ...
    /// [`EventEmitter`] trait. The callback will be invoked with a
    /// reference to the current view, a handle to the emitting `Entity`, the
    /// event, a mutable reference to the `Window`, and the context for the
    /// entity.
    pub fn subscribe_in<Emitter, Evt>(
        &mut self, emitter: &Entity<Emitter>, window: &Window,
    ...
    pub fn observe_release_in<T2>(
        &self, observed: &Entity<T2>, window: &Window,
    ...
    /// Register a callback to be invoked when the window is activated or
    /// deactivated.
    ...
    /// Register a callback to be invoked when the window's pending input
    /// changes.
    ...
    /// Register a listener to be called when the given focus handle receives
    /// focus. Returns a subscription and persists until the subscription is
    /// dropped.
    pub fn on_focus(
        &mut self, handle: &FocusHandle, window: &mut Window,
    ...
    /// Register a listener to be called when the given focus handle loses
    /// focus. Returns a subscription and persists until the subscription is
    /// dropped.
    pub fn on_blur(
        &mut self, handle: &FocusHandle, window: &mut Window,
    ...
    /// This typically happens when the node that was focused is removed from
    /// the tree, and this callback lets you chose a default place to
    /// restore the users focus. Returns a subscription and persists until
    ...
    /// The given callback is invoked with a [`WeakEntity<V>`] to avoid leaking
    /// the entity for a long-running process. It's also given an
    /// [`AsyncWindowContext`], which can be used to access the state of the
    /// entity across await points. The returned future will be polled on
    /// the main thread.
    #[track_caller]
    ...
    pub fn spawn_in_with_priority<AsyncFn, R>(
        &self, priority: Priority, window: &Window, f: AsyncFn,
    ) -> Task<R>
    where
        R: 'static,
        AsyncFn: AsyncFnOnce(WeakEntity<T>, &mut AsyncWindowContext) -> R + 'static, {
    ...
    /// Register a callback to be invoked when the given Action type is
    /// dispatched to the window.
    pub fn on_action(
        &mut self, action_type: TypeId, window: &mut Window,
...
impl<T> Context<'_, T> {
    /// Emit an event of the specified type, which can be handled by other
    /// entities that have subscribed via `subscribe` methods on their
    /// respective contexts.
...
impl<T> AppContext for Context<'_, T> {
    ...
    fn insert_entity<U: 'static>(
        &mut self, reservation: Reservation<U>, build_entity: impl FnOnce(&mut Context<U>) -> U,
    ) -> Entity<U> {
    ...
    fn update_entity<U: 'static, R>(
        &mut self, handle: &Entity<U>, update: impl FnOnce(&mut U, &mut Context<U>) -> R,
    ) -> R {
    ...
    #[inline]
    fn read_window<U, R>(
        &self, window: &WindowHandle<U>, read: impl FnOnce(Entity<U>, &App) -> R,
    ) -> Result<R>
    where
        U: 'static, {
