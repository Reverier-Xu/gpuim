...
/// An async-friendly version of [App] with a static lifetime so it can be held
/// across `await` points in async code. You're provided with an instance when
/// calling [App::spawn], and you can also create one with [App::to_async].
///
/// Internally, this holds a weak reference to an `App`. Methods will panic if
/// the app has been dropped, but this should not happen in practice when using
/// foreground tasks spawned via `cx.spawn()`, as the executor checks if the app
/// is alive before running each task.
#[derive(Clone)]
pub struct AsyncApp {
...
impl AsyncApp {
    ...
    /// Invoke the given function in the context of the app, then flush any
    /// effects produced during its invocation.
    ...
    pub fn subscribe<T, Event>(
        &mut self, entity: &Entity<T>, on_event: impl FnMut(Entity<T>, &Event, &mut App) + 'static,
    ) -> Subscription
    where
    ...
    /// Open a window with the given options based on the root view returned by
    /// the given function.
    pub fn open_window<V>(
        &self, options: crate::WindowOptions,
    ...
    /// Reads the global state of the specified type, passing it to the given
    /// callback.
    ///
    ...
    /// callback.
    ///
    /// Similar to [`AsyncApp::read_global`], but returns an error instead of
    /// panicking
    ...
    /// Reads the global state of the specified type, passing it to the given
    /// callback. A default value is assigned if a global of this type has
    /// not yet been assigned.
    pub fn read_default_global<G: Global + Default, R>(
        &self, read: impl FnOnce(&G, &App) -> R,
    ) -> R {
        let app = self.app();
        let mut app = app.borrow_mut();
        app.update(|cx| {
    ...
    /// [`App::update_global`](BorrowAppContext::update_global) for updating
    /// the global state of the specified type.
    ...
    pub fn on_drop<T: 'static, Callback: FnOnce(&mut T, &mut Context<T>) + 'static>(
        &self, entity: &WeakEntity<T>, f: Callback,
    ) -> gpuim_util::Deferred<impl FnOnce() + use<T, Callback>> {
...
impl AsyncWindowContext {
    ...
    pub fn update_root<R>(
        &mut self, update: impl FnOnce(AnyView, &mut Window, &mut App) -> R,
    ) -> Result<R> {
    ...
    pub fn read_global<G: Global, R>(
        &mut self, read: impl FnOnce(&G, &Window, &App) -> R,
    ) -> Result<R> {
        self.app
    ...
    pub fn update_global<G, R>(
        &mut self, update: impl FnOnce(&mut G, &mut Window, &mut App) -> R,
    ) -> Result<R>
    where
        G: Global, {
    ...
    /// Schedule a future to be executed on the main thread. This is used for
    /// collecting the results of background tasks and updating the UI.
    #[track_caller]
    pub fn spawn<AsyncFn, R>(&self, f: AsyncFn) -> Task<R>
    where
...
impl VisualContext for AsyncWindowContext {
    ...
    fn update_window_entity<T: 'static, R>(
        &mut self, view: &Entity<T>, update: impl FnOnce(&mut T, &mut Window, &mut Context<T>) -> R,
    ) -> Result<R> {
        self.app.update_window(self.window, |_, window, cx| {
