...
impl TestApp {
    ...
    /// Create a new test application with a custom text system for real font
    /// shaping.
    pub fn with_text_system(text_system: Arc<dyn PlatformTextSystem>) -> Self {
        Self::build(0, Some(text_system), Arc::new(()))
    }

    /// Create a new test application with a custom text system and asset
    /// source.
    pub fn with_text_system_and_assets(
        text_system: Arc<dyn PlatformTextSystem>, asset_source: Arc<dyn crate::AssetSource>,
    ) -> Self {
        Self::build(0, Some(text_system), asset_source)
    }

    fn build(
        seed: u64, platform_text_system: Option<Arc<dyn PlatformTextSystem>>,
    ...
    pub fn new_entity<T: 'static>(
        &mut self, build: impl FnOnce(&mut Context<T>) -> T,
    ) -> Entity<T> {
    ...
    pub fn update_entity<T: 'static, R>(
        &mut self, entity: &Entity<T>, f: impl FnOnce(&mut T, &mut Context<T>) -> R,
    ) -> R {
    ...
    pub fn read_entity<T: 'static, R>(
        &self, entity: &Entity<T>, f: impl FnOnce(&T, &App) -> R,
    ) -> R {
    ...
    pub fn open_window_with_options<V: Render + 'static>(
        &mut self, options: WindowOptions,
    ...
    pub fn simulate_new_path_selection(
        &self, select: impl FnOnce(&std::path::Path) -> Option<std::path::PathBuf>,
    ) {
