...
//!
//! This replaces the macOS-only `HeadlessMetalAppContext` with a
//! platform-neutral implementation backed by `TestPlatform`. Tests supply a
//! real `PlatformTextSystem` (e.g. `DirectWriteTextSystem` on Windows,
//! `MacTextSystem` on macOS) to get accurate glyph measurements while keeping
...
///
/// Unlike the old `HeadlessMetalAppContext`, this works on any platform. It
/// uses `TestPlatform` for deterministic scheduling and accepts a pluggable
/// `PlatformTextSystem` so tests get real glyph measurements.
///
/// # Usage
///
/// ```ignore
/// let text_system = Arc::new(gpuim_wgpu::CosmicTextSystem::new("fallback"));
/// let mut cx = HeadlessAppContext::with_platform(
///     text_system,
///     Arc::new(Assets),
///     || gpuim_platform::current_headless_renderer(),
/// );
/// ```
...
impl HeadlessAppContext {
    ...
    /// Creates a new headless app context with a custom text system and asset
    /// source.
    pub fn with_asset_source(
        platform_text_system: Arc<dyn PlatformTextSystem>, asset_source: Arc<dyn AssetSource>,
    ) -> Self {
        Self::with_platform(platform_text_system, asset_source, || None)
    }

    /// Creates a new headless app context with the given text system, asset
    /// source, and an optional renderer factory for screenshot support.
    ...
    /// Enables parking mode, allowing blocking on real I/O (e.g., async asset
    /// loading).
    ...
    pub fn update_window<R>(
        &mut self, window: AnyWindowHandle, f: impl FnOnce(AnyView, &mut Window, &mut App) -> R,
    ) -> Result<R> {
        let mut app = self.app.borrow_mut();
