...
/// `VisualTestAppContext` uses the real `MacPlatform` to produce actual
/// rendered output.
///
/// Windows created through this context are positioned off-screen (at
/// coordinates like -10000, -10000) so they are invisible to the user but still
/// fully rendered by the compositor.
#[derive(Clone)]
...
impl VisualTestAppContext {
    ...
    ///
    /// The window is positioned at (-10000, -10000) so it's not visible on any
    /// display, but it's still fully rendered by the compositor and can be
    /// captured via ScreenCaptureKit.
    ///
    /// # Arguments
    ...
    pub fn open_offscreen_window<V: Render + 'static>(
        &mut self, size: Size<Pixels>, build_root: impl FnOnce(&mut Window, &mut App) -> Entity<V>,
    ...
    pub fn open_offscreen_window_default<V: Render + 'static>(
        &mut self, build_root: impl FnOnce(&mut Window, &mut App) -> Entity<V>,
    ) -> Result<WindowHandle<V>> {
        use crate::{px, size};
    ...
    /// Runs all pending foreground and background tasks until there's nothing
    /// left to do. This is essential for processing async operations like
    /// tooltip timers.
    pub fn run_until_parked(&self) {
        self.dispatcher.run_until_parked();
    }

    /// Advances the simulated clock by the given duration and processes any
    /// tasks that become ready. This is essential for testing time-based
    /// behaviors like tooltip delays.
    ...
    ///
    /// Keystrokes are specified as a space-separated string, e.g., "cmd-p
    /// escape".
    ...
    pub fn simulate_mouse_move(
        &mut self, window: AnyWindowHandle, position: Point<Pixels>,
        button: impl Into<Option<MouseButton>>, modifiers: Modifiers,
    ) {
        self.simulate_event(
            window,
            MouseMoveEvent {
    ...
    pub fn simulate_mouse_down(
        &mut self, window: AnyWindowHandle, position: Point<Pixels>, button: MouseButton,
        modifiers: Modifiers,
    ) {
        self.simulate_event(
            window,
    ...
    pub fn simulate_click(
        &mut self, window: AnyWindowHandle, position: Point<Pixels>, modifiers: Modifiers,
    ) {
    ...
    pub async fn wait_for<T: 'static>(
        &mut self, entity: &Entity<T>, predicate: impl Fn(&T) -> bool, timeout: Duration,
    ) -> Result<()> {
    ...
    /// Captures a screenshot of the specified window using direct texture
    /// capture.
    ///
