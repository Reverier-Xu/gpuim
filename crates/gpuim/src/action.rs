...
pub use gpuim_macros::Action;
pub use no_action::{NoAction, Unbind, is_no_action, is_unbind};
use serde_json::json;

/// Defines and registers unit structs that can be used as actions. For more
/// complex data types, derive `Action`.
///
/// For example:
///
/// ```
...
/// ```
///
/// This will create actions with names like `editor::MoveUp`,
/// `editor::MoveDown`, etc.
///
/// The namespace argument `editor` can also be omitted, though it is required
/// for Zed actions.
#[macro_export]
macro_rules! actions {
    ($namespace:path, [ $( $(#[$attr:meta])* $name:ident),* $(,)? ]) => {
        $(
            #[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug, gpuim::Action)]
...
/// Actions are used to implement keyboard-driven UI. When you declare an
/// action, you can bind keys to the action in the keymap and listeners for that
/// action in the element tree.
///
/// To declare a list of simple actions, you can use the actions! macro, which
/// defines a simple unit struct action for each listed action name in the given
/// namespace.
///
/// ```
/// use gpuim::actions;
...
/// ```
///
/// Registering the actions with the same name will result in a panic during
/// `App` creation.
///
/// # Derive Macro
///
/// More complex data types can also be actions, by using the derive macro for
/// `Action`:
///
/// ```
/// use gpuim::Action;
...
/// ```
///
/// The derive macro for `Action` requires that the type implement `Clone` and
/// `PartialEq`. It also requires `serde::Deserialize` and
/// `schemars::JsonSchema` unless `#[action(no_json)]` is specified. In Zed
/// these trait impls are used to load keymaps from JSON.
///
...
///   `action_json_schema` to return `None`, and allows actions not implement
///   `serde::Serialize` and `schemars::JsonSchema`.
///
/// - `no_register` skips registering the action. This is useful for
///   implementing the `Action` trait while not supporting invocation by name or
///   JSON deserialization.
///
/// - `deprecated_aliases = ["editor::SomeAction"]` specifies deprecated old
///   names for the action. These action names should *not* correspond to any
///   actions that are registered. These old names can then still be used to
///   refer to invoke this action. In Zed, the keymap JSON schema will accept
///   these old names and provide warnings.
///
/// - `deprecated = "Message about why this action is deprecation"` specifies a
///   deprecation message. In Zed, the keymap JSON schema will cause this to be
///   displayed as a warning.
///
/// # Manual Implementation
///
/// If you want to control the behavior of the action trait manually, you can
/// use the lower-level `#[register_action]` macro, which only generates the
/// code needed to register your action before `main`.
///
/// ```
/// use gpuim::{SharedString, register_action};
/// #[derive(Clone, PartialEq, Eq, serde::Deserialize, schemars::JsonSchema)]
/// pub struct Paste {
///     pub content: SharedString,
/// }
///
/// impl gpuim::Action for Paste {
///     # fn boxed_clone(&self) -> Box<dyn gpuim::Action> { unimplemented!()}
///     # fn partial_eq(&self, other: &dyn gpuim::Action) -> bool { unimplemented!() }
///     # fn name(&self) -> &'static str { "Paste" }
///     # fn name_for_type() -> &'static str { "Paste" }
///     # fn build(value: serde_json::Value) -> anyhow::Result<Box<dyn gpuim::Action>> {
///     #     unimplemented!()
///     # }
/// }
///
...
pub trait Action: Any + Send {
    ...
    /// Build this action from a JSON value. This is used to construct actions
    /// from the keymap. A value of `{}` will be passed for actions that
    ...
    /// A list of alternate, deprecated names for this action. These names can
    /// still be used to invoke the action. In Zed, the keymap JSON schema
    /// will accept these old names and provide warnings.
    ...
    /// Returns the deprecation message for this action, if any. In Zed, the
    /// keymap JSON schema will cause this to be displayed as a warning.
    ...
    /// The documentation for this action, if any. When using the derive macro
    /// for actions this will be automatically generated from the doc
    /// comments on the action struct.
...
/// Error type for `Keystroke::parse`. This is used instead of `anyhow::Error`
/// so that Zed can use markdown to display it.
#[derive(Debug)]
pub enum ActionBuildError {
    ...
    /// Indicates that an error occurred while building the action, typically a
...
impl ActionRegistry {
    ...
    /// Construct an action based on its name and optional JSON parameters
    /// sourced from the keymap.
    ...
    /// sourced from the keymap.
    pub fn build_action(
        &self, name: &str, params: Option<serde_json::Value>,
...
mod no_action {
    ...
    actions!(
        zed,
        [
            /// Action with special handling which unbinds the keybinding this
            /// is associated with, if it is the highest precedence
            /// match.
            NoAction
        ]
    );

    /// Action with special handling which unbinds later bindings for the same
    /// keystrokes when they dispatch the named action, regardless of that
    /// action's context.
    ///
    /// In keymap JSON this is written as:
    ///
    /// `["zed::Unbind", "editor::NewLine"]`
    #[derive(Clone, Debug, PartialEq, Deserialize, JsonSchema, gpuim::Action)]
    #[action(namespace = zed)]
    pub struct Unbind(pub gpuim::SharedString);

    /// Returns whether or not this action represents a removed key binding.
    pub fn is_no_action(action: &dyn gpuim::Action) -> bool {
        action.as_any().is::<NoAction>()
    }

    /// Returns whether or not this action represents an unbind marker.
    pub fn is_unbind(action: &dyn gpuim::Action) -> bool {
