...
impl TaffyLayoutEngine {
    ...
    pub fn request_layout(
        &mut self, style: Style, rem_size: Pixels, scale_factor: f32, children: &[LayoutId],
    ) -> LayoutId {
    ...
    pub fn request_measured_layout(
        &mut self, style: Style, rem_size: Pixels, scale_factor: f32,
        measure: impl FnMut(
            Size<Option<Pixels>>,
            Size<AvailableSpace>,
            &mut Window,
    ...
    pub fn compute_layout(
        &mut self, id: LayoutId, available_space: Size<AvailableSpace>, window: &mut Window,
        cx: &mut App,
    ) {
...
/// A unique identifier for a layout node, generated when requesting a layout
/// from Taffy
...
impl ToTaffy<taffy::style::LengthPercentageAuto> for Length {
    fn to_taffy(
        &self, rem_size: Pixels, scale_factor: f32,
...
pub enum AvailableSpace {
    ...
    MinContent,
    /// The amount of space available is indefinite and the node should be laid
    /// out under a max-content constraint
    MaxContent,
}

impl AvailableSpace {
    /// Returns a `Size` with both width and height set to
    /// `AvailableSpace::MinContent`.
    ///
    /// This function is useful when you want to create a `Size` with the
    /// minimum content constraints for both dimensions.
    ///
    /// # Examples
    ///
    /// ```
    /// use gpuim::AvailableSpace;
