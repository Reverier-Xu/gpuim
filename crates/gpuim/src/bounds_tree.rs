...
/// A spatial tree optimized for finding maximum ordering among intersecting
/// bounds.
///
/// This is an R-tree variant specifically designed for the use case of
/// assigning z-order to overlapping UI elements. Key optimizations:
        ...
        + Default,
{
    ...
    /// Finds the maximum ordering among all bounds that intersect with the
    /// query.
    ...
    fn insert_leaf(&mut self, bounds: Bounds<U>, order: u32) -> usize {
        ...
        for &node_idx in self.insert_path.iter().rev() {
            ...
            if node.max_order < order {
                ...
                // Swap updated child to end (skip first iteration since the invariant is
                // already handled by previous cases)
