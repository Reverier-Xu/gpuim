...
impl MenuItem {
    ...
    pub fn os_action(
        name: impl Into<SharedString>, action: impl Action, os_action: OsAction,
    ) -> Self {
        Self::Action {
            name: name.into(),
            action: Box::new(action),
    ...
    ///
    /// Only for [`MenuItem::Action`] and [`MenuItem::Submenu`], otherwise,
    /// returns false
    #[inline]
...
mod tests {
    ...
    fn test_menu() {
        let menu = Menu::new("App")
            .items(vec![
                crate::MenuItem::action("Action 1", gpuim::NoAction),
    ...
    fn test_menu_item_builder() {
        ...
        let item = MenuItem::action("Test Action", gpuim::NoAction);
        assert_eq!(
            match &item {
        ...
        assert!(
            MenuItem::action("Test Action", gpuim::NoAction)
                .checked(true)
                .is_checked()
        );
        assert!(
