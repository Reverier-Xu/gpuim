...
///
/// `scap_default_target_source` should be used instead on Wayland, since
...
/// Starts screen capture for the default target, and populates the receiver
/// with a single source for it. The first frame of the screen capture is used
/// to determine the size of the stream.
///
/// On Wayland (Linux), prompts the user to select a target, and populates the
/// receiver with a single screen capture source for their selection.
#[allow(dead_code)]
...
/// Starts screen capture on the default capture target, and populates the
/// sender with the source.
fn start_default_target_screen_capture(
    sources_tx: oneshot::Sender<Result<Vec<ScapDefaultTargetCaptureSource>>>,
) {
    // Due to use of blocking APIs, a dedicated thread is used.
    std::thread::spawn(|| {
        let start_result = gpuim_util::maybe!({
    ...
    fn stream(
        ...
        match self.stream_call_tx.try_send((tx, frame_callback)) {
            ...
            | Err(std::sync::mpsc::TrySendError::Disconnected((tx, _))) => {
                // Note: support could be added for being called again after end of prior
                // stream.
                tx.send(Err(anyhow!(
...
fn run_capture(
    mut capturer: scap::capturer::Capturer, display: scap::Display,
    frame_callback: Box<dyn Fn(ScreenCaptureFrame) + Send>,
...
/// This is used by `get_screen_targets` and
/// `start_default_target_screen_capture` to turn their results into `Rc<dyn
/// ScreenCaptureSource>`. They need to `Send` their capture source, and so
/// the capture source structs are used as `Rc<dyn ScreenCaptureSource>` is not
/// `Send`.
fn to_dyn_screen_capture_sources<T: ScreenCaptureSource + 'static>(
    sources_rx: oneshot::Receiver<Result<Vec<T>>>, foreground_executor: &ForegroundExecutor,
) -> oneshot::Receiver<Result<Vec<Rc<dyn ScreenCaptureSource>>>> {
...
fn to_dyn_screen_capture_stream<T: ScreenCaptureStream + 'static>(
    sources_rx: oneshot::Receiver<Result<T>>, foreground_executor: &ForegroundExecutor,
) -> oneshot::Receiver<Result<Box<dyn ScreenCaptureStream>>> {
    let (dyn_sources_tx, dyn_sources_rx) = oneshot::channel();
    foreground_executor
        .spawn(async move {
