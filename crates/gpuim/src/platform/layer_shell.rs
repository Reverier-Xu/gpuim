...
/// The layer the surface is rendered on. Multiple surfaces can share a layer,
/// and ordering within a single layer is undefined.
...
pub enum KeyboardInteractivity {
    /// No keyboard inputs will be delivered to the surface and it won't be able
    /// to receive keyboard focus.
    None,

    /// The surface will receive exclusive keyboard focus as long as it is above
    /// the shell surface layer, and no other layer_shell surfaces are above
    /// it.
    Exclusive,
...
pub struct LayerShellOptions {
    /// The namespace for the surface, mostly used by compositors to apply
    /// rules, can not be changed after the surface is created.
    ...
    /// The anchor point of the exclusive zone, will be determined using the
    /// anchor if left unspecified.
...
/// support the required layer_shell protocol.
#[derive(Debug, Error)]
