...
/// This is a helper trait so that we can simplify the implementation of some
/// functions
...
/// Represents a keystroke that can be used in keybindings and displayed to the
/// user.
...
impl Keystroke {
    ...
    /// the key_char or the key itself. On some non-US keyboards keys we use in
    /// our bindings are behind option (for example `$` is typed `alt-ç` on
    /// a Czech keyboard), and on some keyboards the IME handler converts a
    /// sequence of keys into a specific character (for example `"` is typed
    /// as `" space` on a brazilian keyboard).
    ///
    /// This method assumes that `self` was typed and `target' is in the keymap,
    /// and checks both possibilities for self against the target.
    pub fn should_match(&self, target: &KeybindingKeystroke) -> bool {
        ...
        if let Some(key_char) = self
            ...
            .filter(|key_char| key_char != &&self.key)
        {
            // On Windows, if key_char is set, then the typed keystroke produced the
            // key_char
    ...
    pub fn parse(source: &str) -> std::result::Result<Self, InvalidKeystrokeError> {
        ...
        while let Some(component) = components.next() {
            ...
            if component.len() == 1 && component.as_bytes()[0].is_ascii_uppercase() {
                ...
                // are accepted case insensitively and stored how we expect so they are matched
                // properly
                key_str.make_ascii_lowercase()
            }
            key = Some(key_str);
...
impl KeybindingKeystroke {
    ...
    /// Create a new keybinding keystroke from the given keystroke using the
    /// given keyboard mapper.
    pub fn new_with_mapper(
        inner: Keystroke, use_key_equivalents: bool, keyboard_mapper: &dyn PlatformKeyboardMapper,
    ) -> Self {
        keyboard_mapper.map_key_equivalent(inner, use_key_equivalents)
    }

    /// Create a new keybinding keystroke from the given keystroke, without any
    ...
    /// - On macOS and Linux, this modifiers is the same as `inner.modifiers`,
    ...
    ///
    /// Platform-specific behavior:
    /// - On macOS and Linux, this key is the same as `inner.key`, which is the
    ///   GPUI representation of the keystroke.
    /// - On Windows, this key is the display key, for example, a `ctrl-@`
    ///   keystroke will have `inner.key` as `@` and `display_key` as `2`.
    pub fn key(&self) -> &str {
        #[cfg(target_os = "windows")]
        {
            &self.display_key
    ...
    /// Sets the modifiers. On Windows this modifies both `inner.modifiers` and
    /// `display_modifiers`.
    ...
    /// Sets the key. On Windows this modifies both `inner.key` and
    /// `display_key`.
