...
///
/// This implementation delegates task scheduling to the scheduler crate's
/// `TestScheduler`. Access the scheduler directly via `scheduler()` for clock,
/// rng, and parking control.
#[doc(hidden)]
...
impl TestDispatcher {
    ...
    /// Override the value returned by `BackgroundExecutor::num_cpus()` in
    /// tests. A value of 0 means no override (the default of 4 is used).
