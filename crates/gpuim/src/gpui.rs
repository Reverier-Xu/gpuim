...
/// Do not touch, here be dragons for use by gpuim_macros and such.
#[doc(hidden)]
pub mod private {
    pub use anyhow;
    pub use inventory;
...
mod seal {
    /// A mechanism for restricting implementations of a trait to only those in
    /// GPUI. See: <https://predr.ag/blog/definitive-guide-to-sealed-traits-in-rust/>
    pub trait Sealed {}
}
...
pub use gpuim_shared_string::*;
pub use gpuim_util::arc_cow::ArcCow;
pub use http_client;
pub use input::*;
pub use inspector::*;
pub use interactive::*;
...
pub trait AppContext {
    ...
    /// The returned [Reservation] allows you to obtain the [EntityId] for the
    /// future entity.
    fn reserve_entity<T: 'static>(&mut self) -> Reservation<T>;

    /// Insert a new entity in the app context based on a [Reservation]
    /// previously obtained from [`reserve_entity`].
    ///
...
/// Returned by [Context::reserve_entity] to later be passed to
/// [Context::insert_entity]. Allows you to obtain the [EntityId] for a entity
/// before it is created.
pub struct Reservation<T>(pub(crate) Slot<T>);

impl<T: 'static> Reservation<T> {
    /// Returns the [EntityId] that will be associated with the entity once it
    /// is inserted.
...
pub trait BorrowAppContext {
    ...
    where
        G: Global;
    /// Updates the global state of the given type, creating a default if it
    /// didn't exist before.
