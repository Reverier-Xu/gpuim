...
use gpuim_util::{ResultExt, measure, post_inc};
...
pub enum DispatchPhase {
    /// After the capture phase comes the bubble phase, in which mouse event
    /// listeners are invoked front to back and keyboard event listeners are
    /// invoked from the focused element to the root of the element tree.
    /// This is the phase you'll most commonly want to use when registering
    /// event listeners.
    #[default]
    Bubble,
    /// During the initial capture phase, mouse event listeners are invoked back
    /// to front, and keyboard listeners are invoked from the root of the
    /// tree downward toward the focused element. This phase is used for
    /// special purposes such as clearing the "pressed" state for click events.
    /// If you stop event propagation during this phase, you need to know
    /// what you're doing. Handlers outside of the immediate region may rely
    /// on detecting non-local events during this phase.
    Capture,
}
...
/// Allocates an element in the current arena. Uses the app-specific arena if
/// one is active (during draw), otherwise falls back to the thread-local
/// ELEMENT_ARENA.
...
/// RAII guard that sets CURRENT_ELEMENT_ARENA for the duration of a draw
/// operation. When dropped, restores the previous arena (supporting nested
/// draws).
...
/// Returned when the element arena has been used and so must be cleared before
/// the next draw.
#[must_use]
...
impl FocusId {
    /// Obtains whether the element associated with this handle is currently
    /// focused.
    pub fn is_focused(&self, window: &Window) -> bool {
        window.focus == Some(*self)
    }

    /// Obtains whether the element associated with this handle contains the
    ...
    /// Obtains whether the element associated with this handle is contained
    /// within the focused element or is itself focused.
    pub fn within_focused(&self, window: &Window, cx: &App) -> bool {
        let focused = window.focused(cx);
        focused.is_some_and(|focused| focused.id.contains(*self, window))
    }

    /// Obtains whether this handle contains the given handle in the most
...
/// A handle which can be used to track and manipulate the focused element in a
/// window.
...
impl FocusHandle {
    ...
    /// Converts this focus handle into a weak variant, which does not prevent
...
/// Emitted by implementers of [`ManagedView`] to indicate the view should be
/// dismissed, such as when a view is presented as a modal.
...
impl HitboxId {
    /// Checks if the hitbox with this ID is currently hovered. Returns `false`
    /// during keyboard input modality so that keyboard navigation
    /// suppresses hover highlights. Except when handling
    /// `ScrollWheelEvent`, this is typically what you want when determining
    ...
    /// Checks if the hitbox with this ID contains the mouse and should handle
    /// scroll events. Typically this should only be used when handling
    /// `ScrollWheelEvent`, and otherwise `is_hovered` should be used. See
    /// the documentation of `Hitbox::is_hovered` for details about
    /// this distinction.
...
impl Hitbox {
    /// Checks if the hitbox is currently hovered. Returns `false` during
    /// keyboard input modality so that keyboard navigation suppresses hover
    /// highlights. Except when handling `ScrollWheelEvent`, this is
    /// typically what you want when determining whether to handle mouse
    /// events or paint hover styles.
    ///
    /// This can return `false` even when the hitbox contains the mouse, if a
    /// hitbox in front of this sets `HitboxBehavior::BlockMouse`
    /// (`InteractiveElement::occlude`) or
    /// `HitboxBehavior::BlockMouseExceptScroll`
    /// (`InteractiveElement::block_mouse_except_scroll`), or if the current
    /// input modality is keyboard (see [`Window::last_input_was_keyboard`]).
    ///
    /// Handling of `ScrollWheelEvent` should typically use
    /// `should_handle_scroll` instead. Concretely, this is due to use-cases
    /// like overlays that cause the elements under to be non-interactive
    /// while still allowing scrolling. More abstractly, this is because
    /// `is_hovered` is about element interactions directly under the mouse -
    /// mouse moves, clicks, hover styling, etc. In contrast, scrolling is
    /// about finding the current outer scrollable container.
    pub fn is_hovered(&self, window: &Window) -> bool {
        self.id.is_hovered(window)
    }

    /// Checks if the hitbox contains the mouse and should handle scroll events.
    /// Typically this should only be used when handling `ScrollWheelEvent`,
    /// and otherwise `is_hovered` should be used. See the documentation of
    /// `Hitbox::is_hovered` for details about this distinction.
    ///
    /// This can return `false` even when the hitbox contains the mouse, if a
    /// hitbox in front of this sets `HitboxBehavior::BlockMouse`
...
pub enum HitboxBehavior {
    /// Normal hitbox mouse behavior, doesn't affect mouse handling for other
    /// hitboxes.
    #[default]
    Normal,

    /// All hitboxes behind this hitbox will be ignored and so will have
    /// `hitbox.is_hovered() == false` and `hitbox.should_handle_scroll() ==
    /// false`. Typically for elements this causes skipping of all mouse
    /// events, hover styles, and tooltips. This flag is set by
    /// [`InteractiveElement::occlude`].
    ///
    /// For mouse handlers that check those hitboxes, this behaves the same as
    /// registering a bubble-phase handler for every mouse event type:
    ///
    /// ```ignore
    ...
    /// ```
    ///
    /// This has effects beyond event handling - any use of hitbox checking,
    /// such as hover styles and tooltips. These other behaviors are the
    /// main point of this mechanism. An alternative might be to not affect
    /// mouse event handling - but this would allow inconsistent UI where
    /// clicks and moves interact with elements that are not considered to
    /// be hovered.
    BlockMouse,

    /// All hitboxes behind this hitbox will have `hitbox.is_hovered() ==
    /// false`, even when `hitbox.should_handle_scroll() == true`. Typically
    /// for elements this causes all mouse interaction except scroll events
    /// to be ignored - see the documentation of [`Hitbox::is_hovered`] for
    /// details. This flag is set by
    /// [`InteractiveElement::block_mouse_except_scroll`].
    ///
    /// For mouse handlers that check those hitboxes, this behaves the same as
    /// registering a bubble-phase handler for every mouse event type
    /// **except** `ScrollWheelEvent`:
    ///
    /// ```ignore
    ...
    /// ```
    ///
    /// See the documentation of [`Hitbox::is_hovered`] for details of why
    /// `ScrollWheelEvent` is handled differently than other mouse events.
    /// If also blocking these scroll events is desired, then a
    /// `cx.stop_propagation()` handler like the one above can be used.
    ///
    /// This has effects beyond event handling - this affects any use of
    /// `is_hovered`, such as hover styles and tooltips. These other
    /// behaviors are the main point of this mechanism. An alternative might
    /// be to not affect mouse event handling - but this would allow
    /// inconsistent UI where clicks and moves interact with elements that are
    /// not considered to be hovered.
...
pub struct Window {
    ...
    /// Tracks recent input event timestamps to determine if input is arriving
    /// at a high rate. Used to selectively enable VRR optimization only
    /// when input rate exceeds 60fps.
    ...
    /// While captured, mouse events route to this hitbox regardless of hit
    /// testing.
...
/// Tracks input event timestamps to determine if input is arriving at a high
/// rate. Used for selective VRR (Variable Refresh Rate) optimization.
#[derive(Clone, Debug)]
...
struct InputLatencyTracker {
    ...
    /// Count of input events that arrived mid-draw and were excluded from
    /// latency recording because their effects won't appear until the next
    /// frame.
...
impl InputLatencyTracker {
    ...
    /// Only the first event's timestamp per frame is retained (worst-case
    /// latency).
    ...
    /// Record that a frame was presented, flushing pending latency and
    /// coalescing samples.
...
fn default_bounds(display_id: Option<DisplayId>, cx: &mut App) -> WindowBounds {
    ...
    //
    // TODO these should be the initial window bounds not considering
...
impl Window {
    pub(crate) fn new(
        handle: AnyWindowHandle, options: WindowOptions, cx: &mut App,
    ) -> Result<Self> {
        let WindowOptions {
            window_bounds,
            titlebar,
    ...
    pub(crate) fn new_focus_listener(
        &self, value: AnyWindowFocusListener,
...
/// Indicates which region of the window is visible. Content falling outside of
/// this mask will not be rendered. Currently, only rectangular content masks
/// are supported, but we give the mask its own type to leave room to support
...
impl Window {
    fn mark_view_dirty(&mut self, view_id: EntityId) {
        // Mark ancestor views as dirty. If already in the `dirty_views` set, then all
        // its ancestors should already be dirty.
        for view_id in self
            .rendered_frame
            .dispatch_tree
    ...
    pub fn observe_window_appearance(
        &self, mut callback: impl FnMut(&mut Window, &mut App) + 'static,
    ) -> Subscription {
    ...
    /// Registers a callback to be invoked when the window button layout
    /// changes.
    ...
    pub fn replace_root<E>(
        &mut self, cx: &mut App, build_view: impl FnOnce(&mut Window, &mut Context<E>) -> E,
    ) -> Entity<E>
    where
        E: 'static + Render, {
    ...
    /// Obtain the currently focused [`FocusHandle`]. If no elements are
    /// focused, returns `None`.
    ...
    pub fn focus(&mut self, handle: &FocusHandle, cx: &mut App) {
        ...
        // Avoid re-entrant entity updates by deferring observer notifications to the
        // end of the current effect cycle, and only for this window.
    ...
    ///
    /// On some platforms (namely Windows) this is different than the bounds
    /// being the size of the display
    ...
    pub(crate) fn dispatch_keystroke_observers(
        &mut self, event: &dyn Any, action: Option<Box<dyn Action>>,
        context_stack: Vec<KeyContext>, cx: &mut App,
    ) {
    ...
    pub(crate) fn dispatch_keystroke_interceptors(
        &mut self, event: &dyn Any, context_stack: Vec<KeyContext>, cx: &mut App,
    ) {
        let Some(key_down_event) = event.downcast_ref::<KeyDownEvent>() else {
            return;
        };
    ...
    /// Schedules the given function to be run at the end of the current effect
    /// cycle, allowing entities that are currently on the stack to be
    /// returned to the app.
    ...
    /// The entity to which you're subscribing must implement the
    /// [`EventEmitter`] trait. The callback will be invoked a handle to the
    /// emitting entity, the event, and a window context for the current window.
    pub fn observe<T: 'static>(
        &mut self, observed: &Entity<T>, cx: &mut App,
    ...
    pub fn subscribe<Emitter, Evt>(
        &mut self, entity: &Entity<Emitter>, cx: &mut App,
    ...
    pub fn observe_release<T>(
        &self, entity: &Entity<T>, cx: &mut App,
    ...
    /// Creates an [`AsyncWindowContext`], which has a static lifetime and can
    /// be held across await points in async code.
    pub fn to_async(&self, cx: &App) -> AsyncWindowContext {
        AsyncWindowContext::new_context(cx.to_async(), self.handle)
    }

    /// Schedule the given closure to be run directly after the current frame is
    /// rendered.
    ...
    ///
    /// This is useful for elements that need to animate continuously, such as a
    /// video player or an animated GIF. It will cause the window to redraw
    /// on the next frame, even if no other changes have occurred.
    ///
    /// If called from within a view, it will notify that view on the next
    /// frame. Otherwise, it will refresh the entire window.
    ...
    /// pool. The closure is provided a handle to the current window and an
    /// `AsyncWindowContext` for use within your future.
    #[track_caller]
    ...
    pub fn spawn_with_priority<AsyncFn, R>(
        &self, priority: Priority, cx: &App, f: AsyncFn,
    ) -> Task<R>
    where
        R: 'static,
        AsyncFn: AsyncFnOnce(&mut AsyncWindowContext) -> R + 'static, {
    ...
    /// the platform window, then notifies observers. Normally called
    /// automatically by the platform's resize callback, but exposed
    /// publicly for test infrastructure.
    ...
    /// Returns the bounds of the current window in the global coordinate space,
    /// which could span across multiple displays.
    pub fn bounds(&self) -> Bounds<Pixels> {
        self.platform_window.bounds()
    }

    /// Renders the current frame's scene to a texture and returns the pixel
    /// data as an RGBA image. This does not present the frame to screen -
    /// useful for visual testing where we want to capture what would be
    /// rendered without displaying it or requiring the window to be visible.
    #[cfg(any(test, feature = "test-support"))]
    ...
    /// Returns whether this window is focused by the operating system
    ...
    /// Opens the native title bar context menu, useful when implementing client
    /// side decorations (Wayland and X11)
    pub fn show_window_menu(&self, position: Point<Pixels>) {
        self.platform_window.show_window_menu(position)
    }

    /// Handle window movement for Linux and macOS.
    /// Tells the compositor to take control of window movement (Wayland and
    /// X11)
    ///
    ...
    /// When using client side decorations, set this to the width of the
    /// invisible decorations (Wayland and X11)
    ...
    /// Returns whether the title bar window controls need to be rendered by the
    /// application (Wayland and X11)
    ...
    /// The scale factor of the display associated with the window. For example,
    /// it could return 2.0 for a "retina" display, indicating that each
    /// logical pixel should actually be rendered as two pixels on screen.
    pub fn scale_factor(&self) -> f32 {
        self.scale_factor
    }

    /// The size of an em for the base font of the application. Adjusting this
    /// value allows the UI to scale, just like zooming a web page.
    ...
    /// Sets the size of an em for the base font of the application. Adjusting
    /// this value allows the UI to scale, just like zooming a web page.
    ...
    pub fn with_global_id<R>(
        &mut self, element_id: ElementId, f: impl FnOnce(&GlobalElementId, &mut Self) -> R,
    ) -> R {
    ...
    #[inline]
    pub fn with_id<R>(
        &mut self, element_id: impl Into<ElementId>, f: impl FnOnce(&mut Self) -> R,
    ) -> R {
    ...
    /// Call to prevent the default action of an event. Currently only used to
    /// prevent parent elements from becoming focused on mouse down.
    pub fn prevent_default(&mut self) {
        self.default_prevented = true;
    }

    /// Obtain whether default has been prevented for the event currently being
    /// dispatched.
    pub fn default_prevented(&self) -> bool {
        self.default_prevented
    }

    /// Determine whether the given action is available along the dispatch path
    ...
    /// Captures the pointer for the given hitbox. While captured, all mouse
    /// move and mouse up events will be routed to listeners that check this
    /// hitbox's `is_hovered` status, regardless of actual hit testing. This
    /// enables drag operations that continue even when the pointer moves
    ...
    /// Returns true if the last input event was keyboard-based (key press, tab
    /// navigation, etc.) This is used for focus-visible styling to show
    ...
    /// Produces a new frame and assigns it to `rendered_frame`. To actually
    /// show the contents of the new [`Scene`], use [`Self::present`].
    #[profiling::function]
    ...
    fn prepaint_tooltip(&mut self, cx: &mut App) -> Option<AnyElement> {
        // Use indexing instead of iteration to avoid borrowing self for the duration of
        // the loop.
        for tooltip_request_index in (0..self.next_frame.tooltip_requests.len()).rev() {
            ...
            // It's possible for an element to have an active tooltip while not being
            // painted (e.g. via the `visible_on_hover` method). Since mouse
            // listeners are not active in this case, instead update the
            // tooltip's visibility here.
            let is_visible =
    ...
    fn paint_deferred_draws(&mut self, cx: &mut App) {
        ...
        // Since prepaint has already processed nested deferreds, we just paint them
        // all.
    ...
    /// Push a text style onto the stack, and call a function with that style
    /// active. Use [`Window::text_style`] to get the current, combined text
    /// style. This method should only be called as part of element drawing.
    // This function is called in a highly recursive manner in editor
    // prepainting, make sure its inlined to reduce the stack burden
    #[inline]
    ...
    /// Updates the cursor style at the platform level. This method should only
    /// be called during the paint phase of element drawing.
    ...
    /// Updates the cursor style for the entire window at the platform level. A
    /// cursor style using this method will have precedence over any cursor
    /// style set using `set_cursor_style`. This method should only be
    ...
    /// Sets a tooltip to be rendered for the upcoming frame. This method should
    ...
    /// Invoke the given function with the given content mask after intersecting
    /// it with the current mask. This method should only be called during
    /// element drawing.
    // This function is called in a highly recursive manner in editor
    // prepainting, make sure its inlined to reduce the stack burden
    #[inline]
    pub fn with_content_mask<R>(
        &mut self, mask: Option<ContentMask<Pixels>>, f: impl FnOnce(&mut Self) -> R,
    ) -> R {
        self.invalidator.debug_assert_paint_or_prepaint();
    ...
    /// Updates the global element offset relative to the current offset. This
    /// is used to implement scrolling. This method should only be called
    /// during the prepaint phase of element drawing.
    pub fn with_element_offset<R>(
        &mut self, offset: Point<Pixels>, f: impl FnOnce(&mut Self) -> R,
    ) -> R {
        self.invalidator.debug_assert_prepaint();
    ...
    /// Updates the global element offset based on the given offset. This is
    /// used to implement drag handles and other manual painting of
    /// elements. This method should only be called during the prepaint
    ...
    pub(crate) fn with_element_opacity<R>(
        &mut self, opacity: Option<f32>, f: impl FnOnce(&mut Self) -> R,
    ) -> R {
        self.invalidator.debug_assert_paint_or_prepaint();
    ...
    /// Perform prepaint on child elements in a "retryable" manner, so that any
    /// side effects of prepaints can be discarded before prepainting again.
    /// This is used to support autoscroll where we need to prepaint
    /// children to detect the autoscroll bounds, then adjust the
    /// element offset and prepaint again. See [`crate::List`] for an example.
    /// This method should only be called during the prepaint phase of
    /// element drawing.
    ...
    /// When you call this method during [`Element::prepaint`], containing
    /// elements will attempt to scroll to cause the specified bounds to
    /// become visible. When they decide to autoscroll, they will call
    /// [`Element::prepaint`] again with a new set of bounds. See
    /// [`crate::List`] for an example of an element that supports this
    /// method being called on the elements it contains. This method should only
    /// be called during the prepaint phase of element drawing.
    pub fn request_autoscroll(&mut self, bounds: Bounds<Pixels>) {
        self.invalidator.debug_assert_prepaint();
        self.requested_autoscroll = Some(bounds);
    }

    /// This method can be called from a containing element such as
    /// [`crate::List`] to support the autoscroll behavior described in
    ...
    /// Asynchronously load an asset, if the asset hasn't finished loading this
    /// will return None. Your view will be re-drawn once the asset has
    /// finished loading.
    ///
    /// Note that the multiple calls to this method will only result in one
    /// `Asset::load` call at a time.
    ...
    /// Asynchronously load an asset, if the asset hasn't finished loading or
    /// doesn't exist this will return None. Your view will not be re-drawn
    ...
    /// Obtain the current element offset. This method should only be called
    /// during the prepaint phase of element drawing.
    ...
    /// Obtain the current element opacity. This method should only be called
    /// during the prepaint phase of element drawing.
    #[inline]
    ...
    /// Obtain the current content mask. This method should only be called
    /// during element drawing.
    ...
    /// Provide elements in the called function with a new namespace in which
    /// their identifiers must be unique. This can be used within a custom
    /// element to distinguish multiple sets of child elements.
    ...
    /// Use a piece of state that exists as long this element is being rendered
    /// in consecutive frames.
    pub fn use_keyed_state<S: 'static>(
        &mut self, key: impl Into<ElementId>, cx: &mut App,
    ...
    /// Use a piece of state that exists as long this element is being rendered
    /// in consecutive frames, without needing to specify a key
    ///
    /// NOTE: This method uses the location of the caller to generate an ID for
    /// this state.       If this is not sufficient to identify your state
    /// (e.g. you're rendering a list item),       you can provide a custom
    /// ElementID using the `use_keyed_state` method.
    #[track_caller]
    pub fn use_state<S: 'static>(
        &mut self, cx: &mut App, init: impl FnOnce(&mut Self, &mut Context<S>) -> S,
    ) -> Entity<S> {
    ...
    /// Updates or initializes state for an element with the given id that lives
    /// across multiple frames. If an element with this ID existed in the
    /// rendered frame, its state will be passed to the given closure. The
    /// state returned by the closure will be stored so it can be referenced
    /// when drawing the next frame. This method should only be called as part
    /// of element drawing.
    pub fn with_element_state<S, R>(
        &mut self, global_id: &GlobalElementId, f: impl FnOnce(Option<S>, &mut Self) -> (R, S),
    ) -> R
    where
        S: 'static, {
        self.invalidator.debug_assert_paint_or_prepaint();
    ...
    /// A variant of `with_element_state` that allows the element's id to be
    /// optional. This is a convenience method for elements where the
    /// element id may or may not be assigned. Prefer using `with_element_state`
    ...
    /// Defers the drawing of the given element, scheduling it to be painted on
    /// top of the currently-drawn tree at a later time. The `priority`
    /// parameter determines the drawing order relative to other deferred
    /// elements, with higher values being drawn on top.
    ///
    /// When `content_mask` is provided, the deferred element will be clipped to
    /// that region during both prepaint and paint. When `None`, no
    /// additional clipping is applied.
    ///
    /// This method should only be called as part of the prepaint phase of
    /// element drawing.
    pub fn defer_draw(
        &mut self, element: AnyElement, absolute_offset: Point<Pixels>, priority: usize,
        content_mask: Option<ContentMask<Pixels>>,
    ) {
        self.invalidator.debug_assert_prepaint();
    ...
    /// Creates a new painting layer for the specified bounds. A "layer" is a
    /// batch of geometry that are non-overlapping and have the same draw
    /// order. This is typically used for performance reasons.
    ///
    ...
    /// Paint one or more drop shadows into the scene for the next frame at the
    /// current z-index.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    pub fn paint_shadows(
        &mut self, bounds: Bounds<Pixels>, corner_radii: Corners<Pixels>, shadows: &[BoxShadow],
    ) {
        self.invalidator.debug_assert_paint();
    ...
    /// Paint one or more quads into the scene for the next frame at the current
    /// stacking context. Quads are colored rectangular regions with an
    /// optional background, border, and corner radius. see [`fill`],
    /// [`outline`], and [`quad`] to construct this type.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    ///
    /// Note that the `quad.corner_radii` are allowed to exceed the bounds,
    /// creating sharp corners where the circular arcs meet. This will not
    /// display well when combined with dashed borders.
    /// Use `Corners::clamp_radii_for_quad_size` if the radii should fit within
    /// the bounds.
    ...
    /// Paint the given `Path` into the scene for the next frame at the current
    /// z-index.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    ...
    /// Paint an underline into the scene for the next frame at the current
    /// z-index.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    pub fn paint_underline(
        &mut self, origin: Point<Pixels>, width: Pixels, style: &UnderlineStyle,
    ) {
        self.invalidator.debug_assert_paint();
    ...
    /// Paint a strikethrough into the scene for the next frame at the current
    /// z-index.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    pub fn paint_strikethrough(
        &mut self, origin: Point<Pixels>, width: Pixels, style: &StrikethroughStyle,
    ) {
        self.invalidator.debug_assert_paint();
    ...
    /// Paints a monochrome (non-emoji) glyph into the scene for the next frame
    /// at the current z-index.
    ///
    /// The y component of the origin is the baseline of the glyph.
    /// You should generally prefer to use the
    /// [`ShapedLine::paint`](crate::ShapedLine::paint) or
    /// [`WrappedLine::paint`](crate::WrappedLine::paint) methods in the
    /// [`TextSystem`](crate::TextSystem). This method is only useful if you
    /// need to paint a single glyph that has already been shaped.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    pub fn paint_glyph(
        &mut self, origin: Point<Pixels>, font_id: FontId, glyph_id: GlyphId, font_size: Pixels,
        color: Hsla,
    ) -> Result<()> {
        self.invalidator.debug_assert_paint();
    ...
    ///
    /// This is faster than `paint_glyph` because it skips the per-glyph cache
    /// lookup. Use `ShapedLine::compute_glyph_raster_data` to batch-compute
    /// raster bounds during prepaint.
    pub fn paint_glyph_with_raster_bounds(
        &mut self, origin: Point<Pixels>, _font_id: FontId, _glyph_id: GlyphId, _font_size: Pixels,
        color: Hsla, raster_bounds: Bounds<DevicePixels>, params: &RenderGlyphParams,
    ) -> Result<()> {
        self.invalidator.debug_assert_paint();
    ...
    ///
    /// This is faster than `paint_emoji` because it skips the per-glyph cache
    /// lookup. Use `ShapedLine::compute_glyph_raster_data` to batch-compute
    /// raster bounds during prepaint.
    pub fn paint_emoji_with_raster_bounds(
        &mut self, origin: Point<Pixels>, _font_id: FontId, _glyph_id: GlyphId, _font_size: Pixels,
        raster_bounds: Bounds<DevicePixels>, params: &RenderGlyphParams,
    ) -> Result<()> {
        self.invalidator.debug_assert_paint();
    ...
    /// Paints an emoji glyph into the scene for the next frame at the current
    /// z-index.
    ///
    /// The y component of the origin is the baseline of the glyph.
    /// You should generally prefer to use the
    ...
    /// [`WrappedLine::paint`](crate::WrappedLine::paint) methods in the
    /// [`TextSystem`](crate::TextSystem). This method is only useful if you
    /// need to paint a single emoji that has already been shaped.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    pub fn paint_emoji(
    ...
    /// Paint a monochrome SVG into the scene for the next frame at the current
    /// stacking context.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    pub fn paint_svg(
        &mut self, bounds: Bounds<Pixels>, path: SharedString, mut data: Option<&[u8]>,
        transformation: TransformationMatrix, color: Hsla, cx: &App,
    ) -> Result<()> {
        self.invalidator.debug_assert_paint();
    ...
    /// drawing.
    pub fn paint_image(
        &mut self, bounds: Bounds<Pixels>, corner_radii: Corners<Pixels>, data: Arc<RenderImage>,
        frame_index: usize, grayscale: bool,
    ) -> Result<()> {
        self.invalidator.debug_assert_paint();
    ...
    /// Paint a surface into the scene for the next frame at the current
    /// z-index.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    ...
    /// Add a node to the layout tree for the current frame. Takes the `Style`
    /// of the element for which layout is being requested, along with the
    /// layout ids of any children. This method is called during
    /// calls to the [`Element::request_layout`] trait method and enables any
    /// element to participate in layout.
    ///
    /// This method should only be called as part of the request_layout or
    /// prepaint phase of element drawing.
    #[must_use]
    pub fn request_layout(
        &mut self, style: Style, children: impl IntoIterator<Item = LayoutId>, cx: &mut App,
    ) -> LayoutId {
        self.invalidator.debug_assert_prepaint();
    ...
    /// Add a node to the layout tree for the current frame. Instead of taking a
    /// `Style` and children, this variant takes a function that is invoked
    /// during layout so you can use arbitrary logic to determine the
    /// element's size. One place this is used internally is when measuring
    /// text.
    ///
    /// The given closure is invoked at layout time with the known dimensions
    /// and available space and returns a `Size`.
    ///
    /// This method should only be called as part of the request_layout or
    /// prepaint phase of element drawing.
    ...
    /// This method is called for its side effect, typically by the framework
    /// prior to painting. After calling it, you can request the bounds of
    /// the given layout node id or any descendant.
    ///
    /// This method should only be called as part of the prepaint phase of
    /// element drawing.
    pub fn compute_layout(
        &mut self, layout_id: LayoutId, available_space: Size<AvailableSpace>, cx: &mut App,
    ) {
        self.invalidator.debug_assert_prepaint();
    ...
    /// Obtain the bounds computed for the given LayoutId relative to the
    /// window. This method will usually be invoked by GPUI itself
    /// automatically in order to pass your element its `Bounds` automatically.
    ///
    ...
    /// Sets the key context for the current element. This context will be used
    /// to translate keybindings into actions.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    ...
    /// Sets the focus handle for the current element. This handle will be used
    /// to manage focus state and keyboard event dispatch for the element.
    ///
    /// This method should only be called as part of the prepaint phase of
    /// element drawing.
    ...
    /// Sets the view id for the current element, which will be used to manage
    /// view caching.
    ///
    /// This method should only be called as part of element prepaint. We plan
    /// on removing this method eventually when we solve some issues that
    /// require us to construct editor elements directly instead of always
    /// using editors via views.
    ...
    pub(crate) fn with_rendered_view<R>(
        &mut self, id: EntityId, f: impl FnOnce(&mut Self) -> R,
    ) -> R {
    ...
    /// Sets an input handler, such as
    /// [`ElementInputHandler`][element_input_handler], which interfaces with
    /// the platform to receive textual input with proper integration with
    /// concerns such as IME interactions. This handler will be active for
    /// the upcoming frame until the following frame is rendered.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    ///
    ...
    pub fn handle_input(
        &mut self, focus_handle: &FocusHandle, input_handler: impl InputHandler, cx: &App,
    ) {
        self.invalidator.debug_assert_paint();
    ...
    /// Register a mouse event listener on the window for the next frame. The
    /// type of event is determined by the first parameter of the given
    /// listener. When the next frame is rendered the listener will be
    /// cleared.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    pub fn on_mouse_event<Event: MouseEvent>(
        &mut self, mut listener: impl FnMut(&Event, DispatchPhase, &mut Window, &mut App) + 'static,
    ) {
        self.invalidator.debug_assert_paint();
    ...
    /// Register a key event listener on this node for the next frame. The type
    /// of event is determined by the first parameter of the given listener.
    /// When the next frame is rendered the listener will be cleared.
    ///
    /// This is a fairly low-level method, so prefer using event handlers on
    /// elements unless you have a specific need to register a listener
    /// yourself.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    pub fn on_key_event<Event: KeyEvent>(
        &mut self, listener: impl Fn(&Event, DispatchPhase, &mut Window, &mut App) + 'static,
    ) {
        self.invalidator.debug_assert_paint();
    ...
    /// Register a modifiers changed event listener on the window for the next
    /// frame.
    ///
    /// This is a fairly low-level method, so prefer using event handlers on
    /// elements unless you have a specific need to register a global
    /// listener.
    ///
    /// This method should only be called as part of the paint phase of element
    /// drawing.
    pub fn on_modifiers_changed(
        &mut self, listener: impl Fn(&ModifiersChangedEvent, &mut Window, &mut App) + 'static,
    ) {
        self.invalidator.debug_assert_paint();
    ...
    /// Register a listener to be called when the given focus handle or one of
    /// its descendants receives focus. This does not fire if the given
    /// focus handle - or one of its descendants - was previously focused.
    /// Returns a subscription and persists until the subscription is dropped.
    pub fn on_focus_in(
        &mut self, handle: &FocusHandle, cx: &mut App,
    ...
    /// Register a listener to be called when the given focus handle or one of
    /// its descendants loses focus. Returns a subscription and persists
    ...
    /// Return a key binding string for an action, to display in the UI. Uses
    /// the highest precedence binding for the action (last binding added to
    /// the keymap).
    ...
    fn dispatch_mouse_event(&mut self, event: &dyn Any, cx: &mut App) {
        ...
        // used for special purposes, such as detecting events outside of a
        // given Bounds.
            ...
            .map(|_| {
                self.platform_window
                    ...
                    .map_or(false, |mut input_handler| {
                        let accepts = input_handler.accepts_text_input(self, cx);
                        self.platform_window.set_input_handler(input_handler);
                        // If modifiers are not excessive (e.g. AltGr), and the input handler is
                        // accepting text input, we prefer the text input
                        // over bindings.
                        accepts
                    })
            })
    ...
    fn finish_dispatch_key_event(
        &mut self, event: &dyn Any, dispatch_path: SmallVec<[DispatchNodeId; 32]>,
        context_stack: Vec<KeyContext>, cx: &mut App,
    ) {
    ...
    fn dispatch_key_down_up_event(
        &mut self, event: &dyn Any, dispatch_path: &SmallVec<[DispatchNodeId; 32]>, cx: &mut App,
    ) {
        // Capture phase
    ...
    /// Determine whether a potential multi-stroke key binding is in progress on
    /// this window.
    ...
    /// Returns the currently pending input keystrokes that might result in a
    ...
    fn dispatch_action_on_node(
        &mut self, node_id: DispatchNodeId, action: &dyn Action, cx: &mut App,
    ) {
        let dispatch_path = self.rendered_frame.dispatch_tree.dispatch_path(node_id);
    ...
    /// Register the given handler to be invoked whenever the global of the
    /// given type is updated.
    pub fn observe_global<G: Global>(
        &mut self, cx: &mut App, f: impl Fn(&mut Window, &mut App) + 'static,
    ) -> Subscription {
        let window_handle = self.handle;
    ...
    /// Focus the current window and bring it to the foreground at the platform
    /// level.
    ...
    /// Updates the IME panel position suggestions for languages like japanese,
    /// chinese.
    ...
    /// The provided message will be presented, along with buttons for each
    /// answer. When a button is clicked, the returned Receiver will receive
    /// the index of the clicked button.
    pub fn prompt<T>(
        &mut self, level: PromptLevel, message: &str, detail: Option<&str>, answers: &[T],
        cx: &mut App,
    ...
    fn build_custom_prompt(
        &mut self, prompt_builder: &PromptBuilder, level: PromptLevel, message: &str,
        detail: Option<&str>, answers: &[PromptButton], cx: &mut App,
    ) -> oneshot::Receiver<usize> {
    ...
    /// Returns key bindings that invoke an action on the currently focused
    /// element. Bindings are returned in the order they were added. For
    /// display, the last binding should take precedence.
    ...
    /// Returns the highest precedence key binding that invokes an action on the
    /// currently focused element. This is more efficient than getting the
    /// last result of `bindings_for_action`.
    ...
    pub fn bindings_for_action_in_context(
        &self, action: &dyn Action, context: KeyContext,
    ) -> Vec<KeyBinding> {
        let dispatch_tree = &self.rendered_frame.dispatch_tree;
        dispatch_tree.bindings_for_action(action, &[context])
    }

    /// Returns the highest precedence key binding for an action in a context.
    /// This is more efficient than getting the last result of
    /// `bindings_for_action_in_context`.
    ...
    /// Returns any bindings that would invoke an action on the given focus
    /// handle if it were focused. Bindings are returned in the order they
    /// were added. For display, the last binding should take precedence.
    pub fn bindings_for_action_in(
        &self, action: &dyn Action, focus_handle: &FocusHandle,
    ) -> Vec<KeyBinding> {
        let dispatch_tree = &self.rendered_frame.dispatch_tree;
    ...
    /// Returns the highest precedence key binding that would invoke an action
    /// on the given focus handle if it were focused. This is more efficient
    /// than getting the last result of `bindings_for_action_in`.
    ...
    /// Find the bindings that can follow the current input sequence for the
    ...
    /// Returns a generic event listener that invokes the given listener with
    /// the view and context associated with the given view handle.
    pub fn listener_for<T: 'static, E>(
        &self, view: &Entity<T>, f: impl Fn(&mut T, &E, &mut Window, &mut Context<T>) + 'static,
    ...
    /// Returns a generic handler that invokes the given handler with the view
    /// and context associated with the given view handle.
    pub fn handler_for<E: 'static, Callback: Fn(&mut E, &mut Window, &mut Context<E>) + 'static>(
        &self, entity: &Entity<E>, f: Callback,
    ...
    /// Register a callback that can interrupt the closing of the current window
    /// based the returned boolean. If the callback returns false, the
    /// window won't be closed.
    pub fn on_window_should_close(
        &self, cx: &App, f: impl Fn(&mut Window, &mut App) -> bool + 'static,
    ) {
    ...
    /// Register an action listener on this node for the next frame. The type of
    /// action is determined by the first parameter of the given listener.
    /// When the next frame is rendered the listener will be cleared.
    ///
    /// This is a fairly low-level method, so prefer using action handlers on
    /// elements unless you have a specific need to register a listener
    /// yourself.
    ///
    /// This method should only be called as part of the paint phase of element
    ...
    /// Register a capturing action listener on this node for the next frame if
    /// the condition is true. The type of action is determined by the first
    /// parameter of the given listener. When the next frame is rendered the
    ...
    pub fn on_action_when(
        &mut self, condition: bool, action_type: TypeId,
        listener: impl Fn(&dyn Any, DispatchPhase, &mut Window, &mut App) + 'static,
    ) {
        self.invalidator.debug_assert_paint();
    ...
    /// Request the OS to play an alert sound. On some platforms this is
    /// associated with the window, for others it's just a simple global
    /// function call.
    ...
    /// Executes the provided function with mutable access to an inspector
    /// state.
    #[cfg(any(feature = "inspector", debug_assertions))]
    pub fn with_inspector_state<T: 'static, R>(
        &mut self, _inspector_id: Option<&crate::InspectorElementId>, cx: &mut App,
    ...
    pub(crate) fn build_inspector_element_id(
        &mut self, path: crate::InspectorElementPath,
    ...
    /// Registers a hitbox that can be used for inspector picking mode, allowing
    /// users to select and inspect UI elements by clicking on them.
    #[cfg(any(feature = "inspector", debug_assertions))]
    pub fn insert_inspector_hitbox(
        &mut self, hitbox_id: HitboxId, inspector_id: Option<&crate::InspectorElementId>, cx: &App,
    ) {
        self.invalidator.debug_assert_paint_or_prepaint();
        if !self.is_inspector_picking(cx) {
            return;
    ...
    fn hovered_inspector_hitbox(
        &self, inspector: &Inspector, frame: &Frame,
...
impl<V: 'static + Render> WindowHandle<V> {
    ...
    ///
    /// This will fail if the window is closed or if the root view's type does
    /// not match `V`.
    #[cfg(any(test, feature = "test-support"))]
    ...
    ///
    /// This will fail if the window has been closed or if the root view's type
    /// does not match
    pub fn update<C, R>(
        &self, cx: &mut C, update: impl FnOnce(&mut V, &mut Window, &mut Context<V>) -> R,
    ) -> Result<R>
    where
        C: AppContext, {
...
/// window with a specific root view type.
...
impl AnyWindowHandle {
    ...
    /// Attempt to convert this handle to a window handle with a specific root
    /// view type. If the types do not match, this will return `None`.
    ...
    pub fn update<C, R>(
        self, cx: &mut C, update: impl FnOnce(AnyView, &mut Window, &mut App) -> R,
    ) -> Result<R>
    where
        C: AppContext, {
...
pub fn quad(
    bounds: Bounds<Pixels>, corner_radii: impl Into<Corners<Pixels>>,
    background: impl Into<Background>, border_widths: impl Into<Edges<Pixels>>,
    border_color: impl Into<Hsla>, border_style: BorderStyle,
) -> PaintQuad {
    PaintQuad {
        bounds,
        corner_radii: corner_radii.into(),
...
/// Creates a rectangle outline with the given bounds, border color, and a 1px
/// border width
pub fn outline(
    bounds: impl Into<Bounds<Pixels>>, border_color: impl Into<Hsla>, border_style: BorderStyle,
) -> PaintQuad {
    PaintQuad {
        bounds: bounds.into(),
        corner_radii: (0.).into(),
