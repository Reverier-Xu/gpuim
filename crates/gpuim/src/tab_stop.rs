...
struct TabStopNode {
    ...
    /// The final node in the list is a leaf node corresponding to an actual
    /// focus handle, all other nodes are group nodes
    path: TabStopPath,
...
mod sum_tree_impl {
    ...
    impl<'a> sum_tree::Dimension<'a, TabStopOrderNodeSummary> for TabStopCount {
        ...
        fn add_summary(
            &mut self, summary: &'a TabStopOrderNodeSummary,
    ...
    impl<'a, 'b> SeekTarget<'a, TabStopOrderNodeSummary, TabStopNode> for &'b TabStopNode {
        fn cmp(
            &self, cursor_location: &TabStopNode,
            _: <TabStopOrderNodeSummary as sum_tree::Summary>::Context<'_>,
        ) -> std::cmp::Ordering {
...
mod tests {
    ...
    impl TabStopMapTest {
        ...
        fn traverse_tab_map(
            &self, traverse: impl Fn(&TabStopMap, Option<&FocusId>) -> Option<FocusHandle>,
        ) -> Vec<FocusId> {
