...
/// A dynamically-typed handle to a view, which can be downcast to a [Entity]
/// for a specific type.
#[derive(Clone, Debug)]
pub struct AnyView {
    entity: AnyEntity,
...
impl AnyView {
    ...
    /// When using this method, the view's previous layout and paint will be
    /// recycled from the previous frame if [Context::notify] has not been
    /// called since it was rendered. The one exception is when
    /// [Window::refresh] is called, in which case caching is ignored.
    ...
    /// If this handle does not contain a view of the specified type, returns
    /// itself in an `Err` variant.
...
impl Element for AnyView {
    ...
    fn prepaint(
        ...
        bounds: Bounds<Pixels>, element: &mut Self::RequestLayoutState, window: &mut Window,
        cx: &mut App,
    ) -> Option<AnyElement> {
    ...
    fn paint(
        &mut self, global_id: Option<&GlobalElementId>, _inspector_id: Option<&InspectorElementId>,
        _bounds: Bounds<Pixels>, _: &mut Self::RequestLayoutState,
        element: &mut Self::PrepaintState, window: &mut Window, cx: &mut App,
    ) {
        window.with_rendered_view(self.entity_id(), |window| {
            let caching_disabled = window.is_inspector_picking(cx);
...
/// A weak, dynamically-typed view handle that does not prevent the view from
/// being released.
...
impl AnyWeakView {
    /// Convert to a strongly-typed handle if the referenced view has not yet
    /// been released.
