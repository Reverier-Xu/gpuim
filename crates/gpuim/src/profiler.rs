...
/// Tracks which timing events have already been seen so that callers can
/// request only unseen events.
#[doc(hidden)]
...
impl ProfilingCollector {
    ...
    pub fn collect_unseen(
        &mut self, all_timings: Vec<ThreadTaskTimings>,
...
// VecDeque grows by doubling its capacity when full, so keep this a power of 2
...
impl ThreadTimings {
    ...
    /// If this task is the same as the last task, update the end time of the
    /// last task.
    ///
