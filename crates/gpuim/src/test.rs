...
//!
//! GPUI provides first-class support for testing, which includes a macro to run
//! test that rely on having a context, and a test implementation of the
//! `ForegroundExecutor` and `BackgroundExecutor` which ensure that your tests
//! run deterministically even in the face of arbitrary parallelism.
//!
//! The output of the `gpui::test` macro is understood by other rust test
//! runners, so you can use it with `cargo test` or `cargo-nextest`, or another
//! runner of your choice.
//!
//! To make it possible to test collaborative user interfaces (like Zed) you can
//! ask for as many different contexts as you need.
//!
//! ## Example
//!
//! ```
...
/// Strategy injected into `#[gpuim::property_test]` tests to control the seed
...
pub fn run_test(
    num_iterations: usize, explicit_seeds: &[u64], max_retries: usize,
    test_fn: &mut (dyn RefUnwindSafe + Fn(TestDispatcher, u64)), on_fail_fn: Option<fn()>,
) {
...
fn calculate_seeds(
    iterations: u64, explicit_seeds: &[u64],
    ...
    let iter = {
        ...
        // if `iterations` is 1 and !(`explicit_seeds` is non-empty || `SEED` is set),
        // then add     the run `0` if `iterations` is 1 and  (`explicit_seeds`
        // is non-empty || `SEED` is set), then discard the run `0`
