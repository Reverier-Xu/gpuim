...
impl LineLayout {
    ...
    /// closest_index_for_x returns the character boundary closest to the given
    /// x coordinate (e.g. to handle aligning up/down arrow keys)
    ...
    fn compute_wrap_boundaries(
        &self, text: &str, wrap_width: Pixels, max_lines: Option<usize>,
...
impl WrappedLineLayout {
    ...
    /// The index corresponding to a given position in this layout for the given
    /// line height.
    ///
    /// See also [`Self::closest_index_for_position`].
    pub fn index_for_position(
        &self, position: Point<Pixels>, line_height: Pixels,
    ) -> Result<usize, usize> {
        self._index_for_position(position, line_height, false)
    }

    /// The closest index to a given position in this layout for the given line
    /// height.
    ///
    ...
    fn _index_for_position(
        &self, mut position: Point<Pixels>, line_height: Pixels, closest: bool,
    ) -> Result<usize, usize> {
...
struct FrameCache {
    ...
    // (`HashedCacheKeyRef`) that can be computed without building a contiguous
    // `&str`/`SharedString`. On miss, we allocate once and store under an owned
    // `HashedCacheKey`.
...
impl LineLayoutCache {
    ...
    pub fn layout_wrapped_line<Text>(
        &self, text: Text, font_size: Pixels, runs: &[FontRun], wrap_width: Option<Pixels>,
    ...
    pub fn layout_line<Text>(
        &self, text: Text, font_size: Pixels, runs: &[FontRun], force_width: Option<Pixels>,
    ) -> Arc<LineLayout>
    where
        Text: AsRef<str>,
        SharedString: From<Text>, {
    ...
    /// Try to retrieve a previously-shaped line layout using a caller-provided
    /// content hash.
    ///
    /// This is a *non-allocating* cache probe: it does not materialize any
    /// text. If the layout is not already cached in either the current
    /// frame or previous frame, returns `None`.
    ///
    ...
    pub fn try_layout_line_by_hash(
        &self, text_hash: u64, text_len: usize, font_size: Pixels, runs: &[FontRun],
    ...
    /// Layout a line of text using a caller-provided content hash as the cache
    /// key.
    ///
    /// This enables cache hits without materializing a contiguous
    /// `SharedString` for `text`. If the cache misses, `materialize_text`
    ...
    pub fn layout_line_by_hash(
        ...
        // Try to reuse from previous frame without allocating; do a linear scan to find
        // a matching key. (We avoid `drain()` here because it would eagerly
        // move all entries.)
