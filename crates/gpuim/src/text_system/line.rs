...
/// Pre-computed glyph data for efficient painting without per-glyph cache
/// lookups.
///
...
impl ShapedLine {
    ...
    ///
    /// This is the glyph advance width computed by the text shaping system and
    /// is useful for incrementally advancing a "pen" when painting multiple
    /// fragments on the same row.
    ...
    pub fn paint(
        &self, origin: Point<Pixels>, line_height: Pixels, align: TextAlign,
        align_width: Option<Pixels>, window: &mut Window, cx: &mut App,
    ) -> Result<()> {
        paint_line(
            origin,
            &self.layout,
    ...
    ///
    /// - `prefix` contains glyphs for bytes `[0, byte_index)` with original
    ///   positions. Its width equals the x-advance up to the split point.
    /// - `suffix` contains glyphs for bytes `[byte_index, len)` with positions
    ///   shifted left so the first glyph starts at x=0, and byte indices
    ///   rebased to 0.
    /// - Decoration runs are partitioned at the boundary; a run that straddles
    ///   it is split into two with adjusted lengths.
...
/// A line of text that has been shaped, decorated, and wrapped by the text
/// layout system.
...
fn paint_line(
    origin: Point<Pixels>, layout: &LineLayout, line_height: Pixels, align: TextAlign,
    align_width: Option<Pixels>, decoration_runs: &[DecorationRun],
    wrap_boundaries: &[WrapBoundary], window: &mut Window, cx: &mut App,
) -> Result<()> {
...
fn aligned_origin_x(
    origin: Point<Pixels>, align_width: Pixels, last_glyph_x: Pixels, align: &TextAlign,
    layout: &LineLayout, wrap_boundary: Option<&&WrapBoundary>,
) -> Pixels {
...
mod tests {
    ...
    /// Helper: build a ShapedLine from glyph descriptors without the platform
    /// text system. Each glyph is described as (byte_index, x_position).
    fn make_shaped_line(
        text: &str, glyphs: &[(usize, f32)], width: f32, decorations: &[DecorationRun],
    ) -> ShapedLine {
    ...
    fn test_split_at_glyph_rebasing() {
        ...
        // Remainder "cdef" should have two runs: tail of A (1 glyph) + all of B (3
        // glyphs)
    ...
    fn test_split_at_decorations() {
        ...
        // Split at byte 3 — red goes entirely left, green straddles, blue goes entirely
        // right.
        let red = Hsla {
            h: 0.0,
            s: 1.0,
