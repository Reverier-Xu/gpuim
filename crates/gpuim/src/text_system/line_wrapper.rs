...
impl LineWrapper {
    ...
    /// size.
    pub fn wrap_line<'a>(
        &'a mut self, fragments: &'a [LineFragment], wrap_width: Pixels,
    ...
    pub fn should_truncate_line(
        &mut self, line: &str, truncate_width: Pixels, truncation_affix: &str,
    ...
    /// Truncate a line of text to the given width with this wrapper's font and
    /// font size.
    pub fn truncate_line<'a>(
        &mut self, line: SharedString, truncate_width: Pixels, truncation_affix: &str,
        runs: &'a [TextRun], truncate_from: TruncateFrom,
...
fn update_runs_after_truncation(
    result: &str, ellipsis: &str, runs: &mut Vec<TextRun>, truncate_from: TruncateFrom,
) {
...
impl<'a> LineFragment<'a> {
    ...
    /// Creates a new non-text element with the given width and UTF-8 encoded
    /// length.
...
mod tests {
    ...
    fn test_truncate_line_end() {
        ...
        fn perform_test(
            wrapper: &mut LineWrapper, text: &'static str, expected: &'static str, ellipsis: &str,
        ) {
    ...
    fn test_truncate_multiple_runs_end() {
        ...
        fn perform_test(
            wrapper: &mut LineWrapper, text: &'static str, expected: &str, run_lens: &[usize],
            result_run_len: &[usize], line_width: Pixels,
        ) {
