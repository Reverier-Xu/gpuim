...
/// Use this struct for interfacing with the 'debug_below' styling from your own
/// elements. If a parent element has this style set on it, then this struct
/// will be set as a global in GPUI.
...
impl ObjectFit {
    ...
    pub fn get_bounds(
        &self, bounds: Bounds<Pixels>, image_size: Size<DevicePixels>,
    ) -> Bounds<Pixels> {
        ...
        match self {
            ...
            ObjectFit::ScaleDown => {
                ...
                if image_size.width > bounds.size.width || image_size.height > bounds.size.height {
                    ...
                    // If the image is smaller than or equal to the container, display it at its
                    // original size, centered within the container.
...
pub struct Style {
    ...
    /// `Overflow::Scroll` and `Overflow::Auto` nodes.
    ...
    /// Whether scrolling should be restricted to the axis indicated by the
    /// mouse wheel.
    ///
    /// This means that:
    ...
    /// ## Motivation
    ///
    /// On the web when scrolling with the mouse wheel, scrolling up and down
    /// will always scroll the Y axis, even when the mouse is over a
    /// horizontally-scrollable element.
    ///
    /// The only way to scroll horizontally is to hold down `Shift` while
    /// scrolling, which then changes the scroll axis to the X axis.
    ///
    /// Currently, GPUI operates differently from the web in that it will scroll
    /// an element in either the X or Y axis when scrolling with just the
    /// mouse wheel. This causes problems when scrolling in a vertical list that
    /// contains horizontally-scrollable elements, as when you get to the
    /// horizontally-scrollable elements the scroll will be hijacked.
    ///
    /// Ideally we would match the web's behavior and not have a need for this,
    /// but right now we're adding this opt-in style property to limit the
    ...
    /// How should the position of this element be tweaked relative to the
    /// layout defined?
    #[refineable]
    ...
    /// Sets the preferred aspect ratio for the item. The ratio is calculated as
    ...
    /// How this node should be aligned in the cross/block axis. Falls back to
    /// the parents [`AlignItems`] if not set
    pub align_self: Option<AlignSelf>,
    /// How should content contained within this item be aligned in the
    /// cross/block axis
    ...
    /// The relative rate at which this item grows when it is expanding to fill
    /// space, 0.0 is the default value, and this value must be positive.
    pub flex_grow: f32,
    /// The relative rate at which this item shrinks when it is contracting to
    /// fit into space, 1.0 is the default value, and this value must be
    /// positive.
    pub flex_shrink: f32,
    ...
    /// Whether to draw a red debugging outline around this element and all of
    /// its conforming children
    #[cfg(debug_assertions)]
...
pub enum Visibility {
    ...
    #[default]
    Visible,
    /// The element should not be drawn, but should still take up space in the
    /// layout.
    Hidden,
}
...
pub enum TextOverflow {
    /// Truncate the text at the end when it doesn't fit, and represent this
    /// truncation by displaying the provided string (e.g., "very long
    /// te…").
    Truncate(SharedString),
    /// Truncate the text at the start when it doesn't fit, and represent this
    /// truncation by displaying the provided string at the beginning (e.g.,
    /// "…ong text here"). Typically more adequate for file paths where the
...
impl TextStyle {
    ...
    /// Convert this text style into a [`TextRun`], for the given length of the
    /// text.
...
pub struct HighlightStyle {
    ...
    /// Similar to the CSS `opacity` property, this will cause the text to be
    /// less vibrant.
...
impl HighlightStyle {
    ...
    /// Non-continuous properties, like font_weight and font_style, are
    /// overwritten.
    #[must_use]
...
pub fn combine_highlights(
    ...
    endpoints.sort_unstable_by_key(|(position, ..)| *position);
...
pub enum AlignItems {
    ...
    ///
    /// For flex containers with flex_direction RowReverse or ColumnReverse this
    /// is equivalent to End. In all other cases it is equivalent to Start.
    FlexStart,
    /// Items are packed towards the flex-relative end of the axis.
    ///
    /// For flex containers with flex_direction RowReverse or ColumnReverse this
    /// is equivalent to Start. In all other cases it is equivalent to End.
    FlexEnd,
...
pub enum AlignContent {
    ...
    /// items. The gaps are distributed evenly in proportion to these
    /// ratios.
    SpaceAround,
}
...
///
/// The default values depends on on which feature flags are enabled. The order
/// of precedence is: Flex, Grid, Block, None.
...
/// How children overflowing their container should affect layout
///
/// In CSS the primary effect of this property is to control whether contents of
/// a parent container that overflow that container should be displayed anyway,
/// be clipped, or trigger the container to become a scroll container. However
/// it also has secondary effects on layout, the main ones being:
///
///   - The automatic minimum size Flexbox/CSS Grid items with non-`Visible`
///     overflow is `0` rather than being content based
///   - `Overflow::Scroll` nodes have space in the layout reserved for a
///     scrollbar (width controlled by the `scrollbar_width` property)
///
/// In Taffy, we only implement the layout related secondary effects as we are
/// not concerned with drawing/painting. The amount of space reserved for
/// a scrollbar is controlled by the `scrollbar_width` property. If this is `0`
/// then `Scroll` behaves identically to `Hidden`.
///
...
pub enum Overflow {
    ...
    /// based on the size of its content. Content that overflows this node
    ...
    Clip,
    /// The automatic minimum size of this node as a flexbox/grid item should be
    /// `0`. Content that overflows this node should *not* contribute to the
    /// scroll region of its parent.
    Hidden,
    /// The automatic minimum size of this node as a flexbox/grid item should be
    /// `0`. Additionally, space should be reserved for a scrollbar. The
    /// amount of space reserved is controlled by the `scrollbar_width`
    /// property. Content that overflows this node should *not* contribute
...
/// field, and whether or not the item will be controlled by flexbox's layout
/// algorithm.
///
...
///
/// [`Position::Relative`] is the default value, in contrast to the default
/// behavior in CSS.
#[derive(Copy, Clone, PartialEq, Eq, Debug, Default, Serialize, Deserialize, JsonSchema)]
// Copy of taffy::style type of the same name, to derive JsonSchema.
pub enum Position {
    ...
    /// items; they are effectively a correction factor applied at the end.
    #[default]
    Relative,
    /// The offset is computed relative to this item's closest positioned
    /// ancestor, if any. Otherwise, it is placed relative to the origin.
    /// No space is created for the item in the page layout, and its size will
    /// not be altered.
    ///
    /// WARNING: to opt-out of layouting entirely, you must use
    /// [`Display::None`] instead on your [`Style`] object.
    Absolute,
}
