...
pub struct KeyDownEvent {
    ...
    /// In some cases, like AltGr on Windows, modifiers are significant for
    /// character input.
...
/// A mouse pressure event from the platform. Generated when a force-sensitive
/// trackpad is pressed hard. Currently only implemented for macOS trackpads.
...
/// A click event that was generated by a keyboard button being pressed and
/// released.
#[derive(Clone, Debug, Default)]
...
/// A click event, generated when a mouse button or keyboard button is pressed
/// and released.
#[derive(Clone, Debug)]
pub enum ClickEvent {
...
impl ClickEvent {
    ...
    pub fn modifiers(&self) -> Modifiers {
        match self {
            // Click events are only generated from keyboard events _without any modifiers_, so we
            // know the modifiers are always Default
    ...
    ///
    /// `Keyboard`: false, keyboard clicks only work if an element is already
    /// focused `Mouse`: Whether this was the first focusing click
...
pub struct PinchEvent {
    ...
    /// Positive values indicate zooming in, negative values indicate zooming
    /// out. For example, 0.1 represents a 10% zoom increase.
    pub delta: f32,
...
impl ScrollDelta {
    ...
    /// If the signs of the deltas are the same (both positive or both
    /// negative), the deltas are added together. If the signs are opposite,
    /// the second delta (other) is used, effectively overriding the first
    /// delta.
...
/// A mouse exit event from the platform, generated when the mouse leaves the
/// window.
#[derive(Clone, Debug, Default)]
...
/// A file drop event from the platform, generated when files are dragged and
...
mod test {
    ...
    use crate::{
        self as gpuim, AppContext as _, Context, FocusHandle, InteractiveElement, IntoElement,
