...
use std::{
    ...
    fmt::{self, Display},
    hash::Hash,
    ops::{Add, Div, Mul, MulAssign, Neg, Range, Sub},
};
...
pub trait Along {
    ...
    /// Applies the given function to the unit along the given axis and returns
    /// a new value.
...
///
/// It holds two public fields, `x` and `y`, which represent the coordinates in
/// the space. The type `T` for the coordinates can be any type that implements
...
impl<T: Clone + Debug + Default + PartialEq> Point<T> {
    ...
    /// Transforms the point to a `Point<U>` by applying the given function to
    /// both coordinates.
    ///
    /// This method allows for converting a `Point<T>` to a `Point<U>` by
    /// specifying a closure that defines how to convert between the two
    /// types. The closure is applied to both the `x` and `y` coordinates,
    /// resulting in a new point of the desired type.
    ///
    /// # Arguments
    ///
    /// * `f` - A closure that takes a value of type `T` and returns a value of
    ///   type `U`.
    ///
    /// # Examples
    ///
...
impl Point<Pixels> {
    /// Scales the point by a given factor, which is typically derived from the
    /// resolution of a target display to ensure proper sizing of UI
    /// elements.
    ///
    /// # Arguments
    ///
    /// * `factor` - The scaling factor to apply to both the x and y
    ///   coordinates.
    ///
    /// # Examples
    ///
    /// ```
    /// # use gpuim::{Point, Pixels, ScaledPixels};
...
{
    /// Returns a new point with the maximum values of each dimension from
    /// `self` and `other`.
    ///
    /// # Arguments
    ///
    ...
    /// Returns a new point with the minimum values of each dimension from
    /// `self` and `other`.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// Given a minimum point and a maximum point, this method constrains the
    /// current point such that its coordinates do not exceed the range
    /// defined by the minimum and maximum points. If the current point's
    /// coordinates are less than the minimum, they are set to the minimum.
    /// If they are greater than the maximum, they are set to the maximum.
    ///
    /// # Arguments
    ///
    /// * `min` - A reference to a `Point` representing the minimum allowable
    ///   coordinates.
    /// * `max` - A reference to a `Point` representing the maximum allowable
    ///   coordinates.
    ///
    /// # Examples
    ///
...
/// A structure representing a two-dimensional size with width and height in a
/// given unit.
///
/// This struct is generic over the type `T`, which can be any type that
/// implements `Clone`, `Default`, and `Debug`. It is commonly used to specify
/// dimensions for elements in a UI, such as a window or element.
#[derive(
...
where
    T: Clone + Debug + Default + PartialEq,
{
    /// Applies a function to the width and height of the size, producing a new
    /// `Size<U>`.
    ///
    /// This method allows for converting a `Size<T>` to a `Size<U>` by
    /// specifying a closure that defines how to convert between the two
    /// types. The closure is applied to both the `width` and `height`,
    /// resulting in a new size of the desired type.
    ///
    /// # Arguments
    ///
    /// * `f` - A closure that takes a value of type `T` and returns a value of
...
impl Size<Pixels> {
    ...
    ///
    /// This method multiplies both the width and height by the provided scaling
    /// factor, resulting in a new `Size<ScaledPixels>` that is
    /// proportionally larger or smaller depending on the factor.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// ```
    /// # use gpuim::{Size, Pixels, ScaledPixels};
...
where
    T: PartialOrd + Clone + Debug + Default + PartialEq,
{
    /// Returns a new `Size` with the maximum width and height from `self` and
    /// `other`.
    ///
    /// # Arguments
    ///
    ...
    /// Returns a new `Size` with the minimum width and height from `self` and
    /// `other`.
    ///
    /// # Arguments
    ///
...
impl Size<Length> {
    /// Returns a `Size` with both width and height set to fill the available
    /// space.
    ///
    /// This function creates a `Size` instance where both the width and height
    /// are set to `Length::Definite(DefiniteLength::Fraction(1.0))`,
    /// which represents 100% of the available space in both dimensions.
    ///
    /// # Returns
    ///
    /// A `Size<Length>` that will fill the available space when used in a
    /// layout.
    pub fn full() -> Self {
        Self {
...
impl Size<Length> {
    /// Returns a `Size` with both width and height set to `auto`, which allows
    /// the layout engine to determine the size.
    ///
    /// This function creates a `Size` instance where both the width and height
    /// are set to `Length::Auto`, indicating that their size should be
    /// computed based on the layout context, such as the content size or
    /// available space.
    ///
    /// # Returns
    ///
...
///
/// The `Bounds` struct is generic over a type `T` which represents the type of
/// the coordinate system. The origin is represented as a `Point<T>` which
/// defines the top left corner of the rectangle, and the size is represented as
/// a `Size<T>` which defines the width and height of the rectangle.
///
/// # Examples
///
/// ```
/// # use gpuim::{Bounds, Point, Size};
...
pub fn bounds<T: Clone + Debug + Default + PartialEq>(
    origin: Point<T>, size: Size<T>,
) -> Bounds<T> {
    Bounds { origin, size }
}

impl Bounds<Pixels> {
    /// Generate a centered bounds for the given display or primary display if
    /// none is provided
    ...
    /// Generate maximized bounds for the given display or primary display if
    /// none is provided
...
{
    /// Constructs a `Bounds` from two corner points: the top left and bottom
    /// right corners.
    ///
    /// This function calculates the origin and size of the `Bounds` based on
    /// the provided corner points. The origin is set to the top left
    /// corner, and the size is determined by the difference between
    /// the x and y coordinates of the bottom right and top left points.
    ///
    /// # Arguments
    ///
    /// * `top_left` - A `Point<T>` representing the top left corner of the
    ///   rectangle.
    /// * `bottom_right` - A `Point<T>` representing the bottom right corner of
    ///   the rectangle.
    ///
    /// # Returns
    ///
    /// Returns a `Bounds<T>` that encompasses the area defined by the two
    /// corner points.
    ///
    /// # Examples
    ///
    /// ```
    /// # use gpuim::{Bounds, Point};
    ...
    /// Constructs a `Bounds` from a corner point and size. The specified corner
    /// will be placed at the specified origin.
    ...
    T: PartialOrd + Add<T, Output = T> + Clone + Debug + Default + PartialEq,
{
    ...
    ///
    /// Two `Bounds` instances intersect if they overlap in the 2D space they
    /// occupy. This method checks if there is any overlapping area between
    /// the two bounds.
    ///
    /// # Arguments
    ///
    /// * `other` - A reference to another `Bounds` to check for intersection
    ///   with.
    ///
    /// # Returns
    ///
    /// Returns `true` if there is any intersection between the two bounds,
    /// `false` otherwise.
    ///
    /// # Examples
    ///
    ...
    T: Add<T, Output = T> + Half + Clone + Debug + Default + PartialEq,
{
    ...
    ///
    /// Calculates the center by taking the origin's x and y coordinates and
    /// adding half the width and height of the bounds, respectively. The
    /// center is represented as a `Point<T>` where `T` is the type of the
    /// coordinate system.
    ///
    /// # Returns
    ///
    ...
    T: Add<T, Output = T> + Clone + Debug + Default + PartialEq,
{
    ...
    ///
    /// The half perimeter is calculated as the sum of the width and the height
    /// of the rectangle. This method is generic over the type `T` which
    /// must implement the `Sub` trait to allow calculation of the width and
    /// height from the bounds' origin and size, as well as the `Add` trait
    ...
    T: Add<T, Output = T> + Sub<Output = T> + Clone + Debug + Default + PartialEq,
{
    ...
    ///
    /// This method expands the bounds by the given `amount`, increasing the
    /// size and adjusting the origin so that the bounds grow outwards
    /// equally in all directions. The resulting bounds will have its width
    /// and height increased by twice the `amount` (since it grows in both
    /// directions), and the origin will be moved by `-amount` in both the x
    /// and y directions.
    ///
    /// # Arguments
    ///
...
where
    T: Add<T, Output = T>
        ...
        + PartialEq,
{
    /// Inset the bounds by a specified amount. Equivalent to `dilate` with the
    /// amount negated.
    ///
    ...
    Bounds<T>
{
    ...
    ///
    /// This method computes the overlapping region of two `Bounds`. If the
    /// bounds do not intersect, the resulting `Bounds` will have a size
    /// with width and height of zero.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// Returns a `Bounds` representing the intersection area. If there is no
    /// intersection, the returned `Bounds` will have a size with width and
    /// height of zero.
    ///
    /// # Examples
    ///
    ...
    ///
    /// This method calculates the smallest `Bounds` that contains both the
    /// current `Bounds` and the `other` `Bounds`. The resulting `Bounds`
    /// will have an origin that is the minimum of the origins of the two
    /// `Bounds`, and a size that encompasses the furthest extents of both
    /// `Bounds`.
    ///
    /// # Arguments
    ///
    ...
    T: Add<T, Output = T> + Clone + Debug + Default + PartialEq,
{
    ...
    /// # Returns
    ///
    /// A value of type `T` representing the y-coordinate of the top edge of the
    /// bounds.
    pub fn top(&self) -> T {
        self.origin.y.clone()
    }
    ...
    /// # Returns
    ///
    /// A value of type `T` representing the y-coordinate of the bottom edge of
    /// the bounds.
    ...
    /// # Returns
    ///
    /// A value of type `T` representing the x-coordinate of the left edge of
    /// the bounds.
    pub fn left(&self) -> T {
        self.origin.x.clone()
    }
    ...
    /// # Returns
    ///
    /// A value of type `T` representing the x-coordinate of the right edge of
    /// the bounds.
    pub fn right(&self) -> T {
    ...
    /// # Returns
    ///
    /// A `Point<T>` representing the corner of the bounds requested by the
    /// parameter.
    ///
    /// # Examples
    ///
    /// ```
    /// use gpuim::{Bounds, Corner, Point, Size};
    /// let bounds = Bounds {
    ///     origin: Point { x: 0, y: 0 },
    ///     size: Size { width: 10, height: 20 },
    /// };
    ...
    T: Add<T, Output = T> + PartialOrd + Clone + Debug + Default + PartialEq,
{
    ...
    ///
    /// This method determines whether a point lies inside the rectangle defined
    /// by the bounds, including the edges. The point is considered inside
    /// if its x-coordinate is greater than or equal to the left edge and
    /// less than or equal to the right edge, and its y-coordinate
    /// is greater than or equal to the top edge and less than or equal to the
    /// bottom edge of the bounds.
    ///
    /// # Arguments
    ///
    /// * `point` - A reference to a `Point<T>` that represents the point to
    ///   check.
    ///
    /// # Returns
    ///
    ...
    ///
    /// This method determines whether the current bounds is entirely enclosed
    /// by the given bounds. A bounds is considered to be contained within
    /// another if its origin (top-left corner) and its bottom-right corner
    /// are both contained within the other bounds.
    ///
    /// # Arguments
    ///
    /// * `other` - A reference to another `Bounds` that might contain this
    ///   bounds.
    ///
    /// # Returns
    ///
    /// Returns `true` if this bounds is completely inside the other bounds,
    /// `false` otherwise.
    ///
    /// # Examples
    ///
    ...
    /// Applies a function to the origin and size of the bounds, producing a new
    /// `Bounds<U>`.
    ///
    /// This method allows for converting a `Bounds<T>` to a `Bounds<U>` by
    /// specifying a closure that defines how to convert between the two
    /// types. The closure is applied to the `origin` and `size` fields,
    /// resulting in new bounds of the desired type.
    ///
    /// # Arguments
    ///
    /// * `f` - A closure that takes a value of type `T` and returns a value of
    ...
    /// # Returns
    ///
    /// Returns a new `Bounds<U>` with the origin and size mapped by the
    /// provided function.
    ///
    /// # Examples
    ///
    ...
    /// Applies a function to the origin  of the bounds, producing a new
    /// `Bounds` with the new origin
    ///
    /// # Examples
    ///
    /// ```
...
/// # Returns
///
/// Returns `true` if either the width or the height of the bounds is less than
/// or equal to zero, indicating an empty area.
impl<T: PartialOrd + Clone + Debug + Default + PartialEq> Bounds<T> {
    /// Checks if the bounds represent an empty area.
    ///
    /// # Returns
    ///
    /// Returns `true` if either the width or the height of the bounds is less
    /// than or equal to zero, indicating an empty area.
    #[must_use]
...
impl Bounds<Pixels> {
    /// Scales the bounds by a given factor, typically used to adjust for
    /// display scaling.
    ///
    /// This method multiplies the origin and size of the bounds by the provided
    /// scaling factor, resulting in a new `Bounds<ScaledPixels>` that is
    /// proportionally larger or smaller depending on the scaling factor.
    /// This can be used to ensure that the bounds are properly scaled for
    /// different display densities.
    ///
    /// # Arguments
    ///
    /// * `factor` - The scaling factor to apply to the origin and size,
    ///   typically the display's scaling factor.
    ///
    /// # Returns
    ///
    ...
    ///
    /// ```
    /// # use gpuim::{Bounds, Point, Size, Pixels, ScaledPixels, DevicePixels};
    /// let bounds = Bounds {
...
///
/// Each field represents the size of the edge on one side of the box: `top`,
...
impl<T: Clone + Debug + Default + PartialEq> Edges<T> {
    ...
    ///
    /// This function creates an `Edges` instance with the `top`, `right`,
    /// `bottom`, and `left` fields all initialized to the same value
    /// provided as an argument. This is useful when you want to have uniform
    /// edges around a box, such as padding or margin with the same size on
    /// all sides.
    ///
    /// # Arguments
    ///
    ...
    /// Applies a function to each field of the `Edges`, producing a new
    /// `Edges<U>`.
    ///
    /// This method allows for converting an `Edges<T>` to an `Edges<U>` by
    /// specifying a closure that defines how to convert between the two
    /// types. The closure is applied to each field (`top`, `right`,
    /// `bottom`, `left`), resulting in new edges of the desired type.
    ///
    /// # Arguments
    ///
    /// * `f` - A closure that takes a reference to a value of type `T` and
    ///   returns a value of type `U`.
    ///
    /// # Returns
    ///
    /// Returns a new `Edges<U>` with each field mapped by the provided
    /// function.
    ///
    /// # Examples
    ///
    ...
    ///
    /// This method applies a predicate function to each field of the `Edges`
    /// and returns `true` if any field satisfies the predicate.
    ///
    /// # Arguments
    ///
    /// * `predicate` - A closure that takes a reference to a value of type `T`
    ///   and returns a `bool`.
    ///
    /// # Returns
    ///
    /// Returns `true` if the predicate returns `true` for any of the edge
...
impl Edges<Length> {
    /// Sets the edges of the `Edges` struct to `auto`, which is a special value
    /// that allows the layout engine to automatically determine the size of the
    /// edges.
    ///
    /// This is typically used in layout contexts where the exact size of the
    /// edges is not important, or when the size should be calculated based on
    /// the content or container.
    ///
    /// # Returns
    ///
    ...
    /// Sets the edges of the `Edges` struct to zero, which means no size or
    /// thickness.
    ///
    /// This is typically used when you want to specify that a box (like a
    /// padding or margin area) should have no edges, effectively making it
    /// non-existent or invisible in layout calculations.
    ///
    /// # Returns
    ///
    ...
    ///
    /// ```
    /// # use gpuim::{DefiniteLength, Edges, Length, Pixels};
...
impl Edges<DefiniteLength> {
    ...
    ///
    /// ```
    /// # use gpuim::{px, DefiniteLength, Edges};
    ...
    /// Converts the `DefiniteLength` to `Pixels` based on the parent size and
    /// the REM size.
    ///
    /// This method allows for a `DefiniteLength` value to be converted into
    /// pixels, taking into account the size of the parent element (for
    /// percentage-based lengths) and the size of a rem unit (for rem-based
    /// lengths).
    ///
    /// # Arguments
    ///
    /// * `parent_size` - `Size<AbsoluteLength>` representing the size of the
    ///   parent element.
    /// * `rem_size` - `Pixels` representing the size of one REM unit.
    ///
    /// # Returns
    ///
    /// Returns an `Edges<Pixels>` representing the edges with lengths converted
    /// to pixels.
    ///
    /// # Examples
    ///
    /// ```
    /// # use gpuim::{Edges, DefiniteLength, px, AbsoluteLength, rems, Size};
    /// let edges = Edges {
...
impl Edges<AbsoluteLength> {
    ...
    ///
    /// ```
    /// # use gpuim::{AbsoluteLength, Edges, Pixels};
    ...
    ///
    /// If the `AbsoluteLength` is already in pixels, it simply returns the
    /// corresponding `Pixels` value. If the `AbsoluteLength` is in rems, it
    /// multiplies the number of rems by the `rem_size` to convert it to pixels.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// ```
    /// # use gpuim::{Edges, AbsoluteLength, Pixels, px, rems};
    /// let edges = Edges {
...
impl Edges<Pixels> {
    /// Scales the `Edges<Pixels>` by a given factor, returning
    /// `Edges<ScaledPixels>`.
    ///
    /// This method is typically used for adjusting the edge sizes for different
    /// display densities or scaling factors.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// Returns a new `Edges<ScaledPixels>` where each edge is the result of
    /// scaling the original edge by the given factor.
    ///
    /// # Examples
    ///
    /// ```
    /// # use gpuim::{Edges, Pixels, ScaledPixels};
    /// let edges = Edges {
...
impl Corner {
    ...
    /// Returns the corner across from this corner, moving along the specified
    /// axis.
    ///
    /// # Examples
    ///
    /// ```
    /// # use gpuim::{Axis, Corner};
...
///
/// Each field represents the size of the corner on one side of the box:
...
where
    T: Clone + Debug + Default + PartialEq,
{
    /// Constructs `Corners` where all sides are set to the same specified
    /// value.
    ///
    /// This function creates a `Corners` instance with the `top_left`,
    /// `top_right`, `bottom_right`, and `bottom_left` fields all initialized
    /// to the same value provided as an argument. This is useful when you want
    /// to have uniform corners around a box, such as a uniform border
    /// radius on a rectangle.
    ///
    /// # Arguments
    ///
...
impl Corners<AbsoluteLength> {
    /// Converts the `AbsoluteLength` to `Pixels` based on the provided rem
    /// size.
    ///
    /// # Arguments
    ///
    /// * `rem_size` - The size of one REM unit in pixels, used for conversion
    ///   if the `AbsoluteLength` is in REMs.
    ///
    /// # Returns
    ///
    /// Returns a `Corners<Pixels>` instance with each corner's length converted
    /// to pixels.
    ///
    /// # Examples
    ///
    /// ```
    /// # use gpuim::{Corners, AbsoluteLength, Pixels, Rems, Size};
    /// let corners = Corners {
...
impl Corners<Pixels> {
    /// Scales the `Corners<Pixels>` by a given factor, returning
    /// `Corners<ScaledPixels>`.
    ///
    /// This method is typically used for adjusting the corner sizes for
    /// different display densities or scaling factors.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// Returns a new `Corners<ScaledPixels>` where each corner is the result of
    /// scaling the original corner by the given factor.
    ///
    /// # Examples
    ///
    /// ```
    /// # use gpuim::{Corners, Pixels, ScaledPixels};
    /// let corners = Corners {
...
impl<T: Div<f32, Output = T> + Ord + Clone + Debug + Default + PartialEq> Corners<T> {
    /// Clamps corner radii to be less than or equal to half the shortest side
    /// of a quad.
    ///
    /// # Arguments
    ///
    /// * `size` - The size of the quad which limits the size of the corner
    ///   radii.
    ///
    /// # Returns
    ///
...
impl<T: Clone + Debug + Default + PartialEq> Corners<T> {
    /// Applies a function to each field of the `Corners`, producing a new
    /// `Corners<U>`.
    ///
    /// This method allows for converting a `Corners<T>` to a `Corners<U>` by
    /// specifying a closure that defines how to convert between the two
    /// types. The closure is applied to each field (`top_left`,
    /// `top_right`, `bottom_right`, `bottom_left`), resulting in new corners of
    /// the desired type.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// Returns a new `Corners<U>` with each field mapped by the provided
    /// function.
    ///
    /// # Examples
    ///
    /// ```
    /// # use gpuim::{Corners, Pixels, Rems};
    /// let corners = Corners {
    ///     top_left: Pixels::from(10.0),
    ///     top_right: Pixels::from(20.0),
    ///     bottom_right: Pixels::from(30.0),
...
/// Represents a length in pixels, the base unit of measurement in the UI
/// framework.
///
/// `Pixels` is a value type that represents an absolute length in pixels, which
/// is used for specifying sizes, positions, and distances in the UI. It is the
/// fundamental unit of measurement for all visual elements and layout
/// calculations.
///
/// The inner value is an `f32`, allowing for sub-pixel precision which can be
/// useful for anti-aliasing and animations. However, when applied to actual
/// pixel grids, the value is typically rounded to the nearest integer.
///
/// # Examples
///
/// ```
/// use gpuim::{Pixels, ScaledPixels};
///
...
impl Pixels {
    ...
    ///
    /// This method is used when adjusting pixel values for display scaling
    /// factors, such as high DPI (dots per inch) or Retina displays, where
    /// the pixel density is higher and thus requires scaling to maintain
    /// visual consistency and readability.
    ///
    /// The resulting `ScaledPixels` represent the scaled value which can be
    /// used for rendering calculations where display scaling is considered.
    #[must_use]
    ...
    /// # Returns
    ///
    /// Returns a new `Pixels` instance with the value raised to the given
    /// exponent.
    ...
    /// # Returns
    ///
    /// A new `Pixels` instance with the absolute value of the original
    /// `Pixels`.
...
///
/// `DevicePixels` is a unit of measurement that refers to the actual pixels on
/// a device's screen. This type is used when precise pixel manipulation is
/// required, such as rendering graphics or interfacing with hardware that
/// operates on the pixel level. Unlike logical pixels that may be affected by
/// the device's scale factor, `DevicePixels` always correspond to real pixels
/// on the display.
#[derive(
    Add,
    AddAssign,
...
impl DevicePixels {
    /// Converts the `DevicePixels` value to the number of bytes needed to
    /// represent it in memory.
    ///
    /// This function is useful when working with graphical data that needs to
    /// be stored in a buffer, such as images or framebuffers, where each
    /// pixel may be represented by a specific number of bytes.
    ///
    /// # Arguments
    ///
    /// * `bytes_per_pixel` - The number of bytes used to represent a single
    ///   pixel.
    ///
    /// # Returns
    ///
    /// The number of bytes required to represent the `DevicePixels` value in
    /// memory.
    ///
    /// # Examples
    ///
    /// ```
    /// # use gpuim::DevicePixels;
...
///
/// `ScaledPixels` are used to ensure that UI elements appear at the correct
/// size on devices with different pixel densities. When a device has a higher
/// scale factor (such as Retina displays), a single logical pixel may
/// correspond to multiple physical pixels. By using `ScaledPixels`, dimensions
/// and positions can be specified in a way that scales appropriately across
...
/// which can be assigned with [`Window::set_rem_size`][set_rem_size].
///
/// Rems are used for defining lengths that are scalable and consistent across
/// different UI elements. The value of `1rem` is typically equal to the
/// font-size of the root element (often the `<html>` element in browsers),
/// making it a flexible unit that adapts to the user's text size preferences.
/// In this framework, `rems` serve a similar purpose, allowing for scalable and
/// accessible design that can adjust to different display settings or user
/// preferences.
///
/// For example, if the root element's font-size is `16px`, then `1rem` equals
/// `16px`. A length of `2rems` would then be `32px`.
///
...
///
/// `AbsoluteLength` can be either a fixed number of pixels, which is an
/// absolute measurement not affected by the current font size, or a number of
/// rems, which is relative to the font size of the root element. It is used for
/// specifying dimensions that are either independent of or related to the
/// typographic scale.
...
impl AbsoluteLength {
    ...
    ///
    /// ```
    /// # use gpuim::{AbsoluteLength, Pixels, Rems};
...
///
/// This enum represents lengths that have a specific value, as opposed to
/// lengths that are automatically determined by the context. It includes
/// absolute lengths in pixels or rems, and relative lengths as a fraction of
/// the parent's size.
#[derive(Clone, Copy, Neg, PartialEq)]
pub enum DefiniteLength {
    /// An absolute length specified in pixels or rems.
    Absolute(AbsoluteLength),
    /// A relative length specified as a fraction of the parent's size, between
    /// 0 and 1.
    Fraction(f32),
}

impl DefiniteLength {
    /// Converts the `DefiniteLength` to `Pixels` based on a given `base_size`
    /// and `rem_size`.
    ///
    /// If the `DefiniteLength` is an absolute length, it will be directly
    /// converted to `Pixels`. If it is a fraction, the fraction will be
    /// multiplied by the `base_size` to get the length in pixels.
    ///
    /// # Arguments
    ///
    /// * `base_size` - The base size in `AbsoluteLength` to which the fraction
    ///   will be applied.
    /// * `rem_size` - The size of one rem in pixels, used to convert rems to
    ///   pixels.
    ///
    /// # Returns
    ///
    ...
    ///
    /// ```
    /// # use gpuim::{DefiniteLength, AbsoluteLength, Pixels, px, rems};
...
pub enum Length {
    /// A definite length specified either in pixels, rems, or as a fraction of
    /// the parent's size.
    Definite(DefiniteLength),
    /// An automatic length that is determined by the context in which it is
    /// used.
    Auto,
}
...
/// Constructs a `DefiniteLength` representing a relative fraction of a parent
/// size.
///
/// This function creates a `DefiniteLength` that is a specified fraction of a
/// parent's dimension. The fraction should be a floating-point number between
/// 0.0 and 1.0, where 1.0 represents 100% of the parent's size.
///
/// # Arguments
///
...
/// # Returns
///
/// A `DefiniteLength` representing the relative length as a fraction of the
/// parent's size.
...
///
/// The `auto` length is often used in layout calculations where the length
/// should be determined by the layout context itself rather than being
/// explicitly set. This is commonly used in CSS for properties like `width`,
/// `height`, `margin`, `padding`, etc., where `auto` can be used to instruct
/// the layout engine to calculate the size based on other factors like the size
/// of the container or the intrinsic size of the content.
///
/// # Returns
///
...
///
/// The `Half` trait is used for types that can be evenly divided, returning a
/// new instance of the same type representing half of the original value. This
/// is commonly used for types that represent measurements or sizes,
/// such as lengths or pixels, where halving is a frequent operation during
/// layout calculations or animations.
pub trait Half {
    /// Returns half of the current value.
    ///
    /// # Returns
    ///
    /// A new instance of the implementing type, representing half of the
    /// original value.
    fn half(&self) -> Self;
}
...
///
/// This trait provides a method to determine if a value is considered to be
/// zero. It is implemented for various numeric and length-related types where
/// the concept of zero is applicable. This can be useful for comparisons,
/// optimizations, or determining if an operation has a neutral effect.
pub trait IsZero {
