...
//! Renders a div with deep children hierarchy. This example is useful to
//! exemplify that Zed can handle deep hierarchies (even though it cannot just
//! yet!).
use std::sync::LazyLock;

use gpuim::{App, Bounds, Context, Window, WindowBounds, WindowOptions, div, prelude::*, px, size};
...
impl Render for Tree {
    fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
        let mut depth = *DEPTH;
        static COLORS: [gpuim::Hsla; 4] =
            [gpuim::red(), gpuim::blue(), gpuim::green(), gpuim::yellow()];
