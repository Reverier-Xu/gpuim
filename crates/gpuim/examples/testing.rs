...
struct Counter {
    ...
    _subscription: gpuim::Subscription,
}

/// Event emitted by Counter
struct CounterEvent;

impl gpuim::EventEmitter<CounterEvent> for Counter {}
...
fn run_example() {
    application().run(|cx: &mut App| {
        cx.bind_keys([
            gpuim::KeyBinding::new("up", Increment, Some("Counter")),
            gpuim::KeyBinding::new("down", Decrement, Some("Counter")),
        ]);
...
#[cfg(test)]
mod tests {
    use gpuim::{TestAppContext, VisualTestContext};
    use rand::prelude::*;

    use super::*;

    /// Here's a basic GPUI test. Just add the macro and take a TestAppContext
    /// as an argument!
    ///
    /// Note that synchronous side effects run immediately after your "update*"
    /// calls complete.
    #[gpuim::test]
    ...
    /// Tests which involve the window require you to construct a
    /// VisualTestContext. Just like synchronous side effects, the window
    /// will be drawn after every "update*" call, so you can test
    /// render-dependent behavior.
    #[gpuim::test]
    ...
    /// GPUI tests can also be async, simply add the async keyword before the
    /// test. Note that the test executor is single thread, so async side
    /// effects (including background tasks) won't run until you explicitly
    /// yield control.
    #[gpuim::test]
    ...
    /// You should mock external systems where possible, as this feature can be
    /// used to detect potential deadlocks in your async code.
    ///
    ...
    /// Now, all of those tests are good, but GPUI also provides strong support
    /// for testing distributed systems. Let's setup a mock network and
    /// enhance the counter to send messages over it.
    mod distributed_systems {
        ...
        /// A client handle for sending/receiving messages over the mock
        /// network.
        #[derive(Clone)]
        ...
        /// You can simulate distributed systems with multiple app contexts,
        /// simply by adding additional parameters.
        #[gpuim::test]
        ...
        /// Multiple apps can run concurrently, and to capture this each test
        /// app shares a dispatcher. Whenever you call
