...
impl ImageCacheProvider for SimpleLruCacheProvider {
    fn provide(&mut self, window: &mut Window, cx: &mut App) -> gpuim::AnyImageCache {
        window
...
struct SimpleLruCache {
    max_items: usize,
    usages: Vec<u64>,
    cache: HashMap<u64, gpuim::ImageCacheItem>,
}
...
impl ImageCache for SimpleLruCache {
    fn load(
        &mut self, resource: &gpuim::Resource, window: &mut Window, cx: &mut App,
    ) -> Option<Result<Arc<gpuim::RenderImage>, gpuim::ImageCacheError>> {
        ...
        self.cache
            .insert(hash, gpuim::ImageCacheItem::Loading(task.clone()));
        self.usages.insert(0, hash);
...
fn run_example() {
    #[cfg(not(target_family = "wasm"))]
    let app = gpuim_platform::application();
    #[cfg(target_family = "wasm")]
    let app = gpuim_platform::single_threaded_web();
