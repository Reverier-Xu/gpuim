...
impl Quote {
    ...
    fn change_color(&self) -> gpuim::Hsla {
        if self.change() > 0.0 {
            gpuim::green()
        } else {
            gpuim::red()
...
impl TableRow {
    ...
    fn render_cell(&self, key: &str, width: Pixels, color: gpuim::Hsla) -> impl IntoElement {
        div()
            .whitespace_nowrap()
            .truncate()
            .w(width)
...
struct DataTable {
    /// Use `Rc` to share the same quote data across multiple items, avoid
    /// cloning.
    quotes: Vec<Rc<Quote>>,
