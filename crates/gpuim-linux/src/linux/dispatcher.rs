...
impl PlatformDispatcher for LinuxDispatcher {
    fn get_all_timings(&self) -> Vec<gpuim::ThreadTaskTimings> {
        let global_timings = GLOBAL_THREAD_TIMINGS.lock();
        ThreadTaskTimings::convert(&global_timings)
    }

    fn get_current_thread_timings(&self) -> gpuim::ThreadTaskTimings {
        gpuim::profiler::get_current_thread_task_timings()
    }
...
impl<T> PriorityQueueCalloopSender<T> {
    ...
    fn send(&self, priority: Priority, item: T) -> Result<(), gpuim::queue::SendError<T>> {
...
impl<T> calloop::EventSource for PriorityQueueCalloopReceiver<T> {
    ...
    fn process_events<F>(
        &mut self, readiness: calloop::Readiness, token: calloop::Token, mut callback: F,
...
// thread 'linux::dispatcher::tests::tomato' (478301) panicked at
// crates/gpui/src/platform/linux/dispatcher.rs:515:9: assertion failed:
// data.got_closed note: run with `RUST_BACKTRACE=1` environment variable to
// display a backtrace
