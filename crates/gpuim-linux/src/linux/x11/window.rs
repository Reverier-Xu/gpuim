...
fn query_render_extent(
    xcb: &Rc<XCBConnection>, x_window: xproto::Window,
...
pub struct Callbacks {
    ...
    input: Option<Box<dyn FnMut(PlatformInput) -> gpuim::DispatchEventResult>>,
...
/// Convert X11 connection errors to `anyhow::Error` and panic for unrecoverable
/// errors.
...
impl X11WindowState {
    pub fn new(
        handle: AnyWindowHandle, client: X11ClientStatePtr, executor: ForegroundExecutor,
        gpu_context: gpuim_wgpu::GpuContext, compositor_gpu: Option<CompositorGpuHint>,
        params: WindowParams, xcb: &Rc<XCBConnection>, client_side_decorations_supported: bool,
        x_main_screen_index: usize, x_window: xproto::Window, atoms: &XcbAtoms, scale_factor: f32,
        appearance: WindowAppearance, parent_window: Option<X11WindowStatePtr>,
...
impl X11Window {
    ...
    fn set_wm_hints<C: Display + Send + Sync + 'static, F: FnOnce() -> C>(
        &self, failure_context: F, wm_hint_property_state: WmHintPropertyState, prop1: u32,
        prop2: u32,
    ) -> anyhow::Result<()> {
        let state = self.0.state.borrow();
...
impl X11WindowStatePtr {
    ...
    fn set_edge_constraints(
        &self, mut state: std::cell::RefMut<X11WindowState>,
    ) -> anyhow::Result<()> {
        let reply = get_reply(
...
impl PlatformWindow for X11Window {
    ...
    fn content_size(&self) -> Size<Pixels> {
        // After the wgpu migration, X11WindowState::content_size() returns logical
        // pixels (bounds.size is already divided by scale_factor in
        // set_bounds), so no further division is needed here. This matches the
    ...
    fn capslock(&self) -> gpuim::Capslock {
        self.0
            .state
            .borrow()
            .client
    ...
    fn window_decorations(&self) -> gpuim::Decorations {
        let state = self.0.state.borrow();
    ...
    fn request_decorations(&self, mut decorations: gpuim::WindowDecorations) {
        let mut state = self.0.state.borrow_mut();

        if matches!(decorations, gpuim::WindowDecorations::Client)
            && !state.client_side_decorations_supported
        {
            log::info!(
                "x11: no compositor present, falling back to server-side window decorations"
            );
            decorations = gpuim::WindowDecorations::Server;
        }
