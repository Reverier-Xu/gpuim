...
use gpuim::{
    AnyWindowHandle, Bounds, Capslock, ClipboardItem, CursorStyle, DisplayId, FileDropEvent,
    ...
    Size, TaskTiming, TouchPhase, WindowButtonLayout, WindowParams, point, profiler, px,
};
...
use crate::linux::{
    ...
    keystroke_from_xkb, keystroke_underlying_dead_key, log_cursor_icon_warning,
    modifiers_from_xinput_info, modifiers_from_xkb, open_uri_internal,
...
struct ScrollAxisState {
    ...
    ///
    /// This gets set to `None` whenever it might be invalid - when devices
    /// change or when window focus changes. The logic errs on the side of
    /// invalidating this, since the consequence is just skipping the delta of
    /// one scroll event. The consequence of not invalidating it can be
    /// large invalid deltas, which are much more user visible.
...
impl X11Client {
            ...
            .insert_source(main_receiver, {
                ...
                move |event, _, _: &mut X11Client| {
                    if let calloop::channel::Event::Msg(runnable) = event {
                        // Insert the runnables as idle callbacks, so we make sure that user-input
                        // and X11 events have higher priority and runnables
                        // are only worked off after the event callbacks.
    ...
    pub fn process_x11_events(
        ...
        loop {
            ...
            // event handlers for new keyboard / remapping refresh the state without using
            // event details, this deduplicates them.
                    ...
                    Ok(Some(event)) => {
                        match event {
                            ...
                            Event::KeyPress(key_press) => {
                                ...
                                {
                                    // We ignore that last KeyRelease if it's too close to this
                                    // KeyPress, suggesting that
                                    // it's auto-generated by X11 as a key-repeat event.
            ...
            for event in events.into_iter() {
                ...
                match xim_filtered {
                    ...
                    Err(err) => {
                        ...
                        // we do lose 1-2 keys when crash happens since there is no reliable way to
                        // get that info luckily, x11 sends us window not
                        // found error when xim server crashes upon further key press
    ...
    fn handle_event(&self, event: Event) -> Option<()> {
        match event {
            ...
            Event::FocusOut(event) => {
                ...
                let mut state = self.0.borrow_mut();
                // Set last scroll values to `None` so that a large delta isn't created if
                // scrolling is done outside the window (the valuator is global)
            ...
            Event::KeyPress(event) => {
                ...
                drop(state);
                window.handle_input(PlatformInput::KeyDown(gpuim::KeyDownEvent {
                    keystroke,
                    is_held: false,
            ...
            Event::KeyRelease(event) => {
                ...
                drop(state);
                window.handle_input(PlatformInput::KeyUp(gpuim::KeyUpEvent { keystroke }));
            }
            Event::XinputButtonPress(event) => {
                ...
                match button_or_scroll_from_event_detail(event.detail) {
                    Some(ButtonOrScroll::Button(button)) => {
                        ...
                        drop(state);
                        window.handle_input(PlatformInput::MouseDown(gpuim::MouseDownEvent {
                            button,
                            position,
                            modifiers,
                    ...
                    Some(ButtonOrScroll::Scroll(direction)) => {
                        drop(state);
                        // Emulated scroll button presses are sent simultaneously with smooth
                        // scrolling XinputMotion events. Since handling
                        // those events does the scrolling, they are skipped here.
                        if !event
                            .flags
            ...
            Event::XinputButtonRelease(event) => {
                ...
                match button_or_scroll_from_event_detail(event.detail) {
                    Some(ButtonOrScroll::Button(button)) => {
                        ...
                        drop(state);
                        window.handle_input(PlatformInput::MouseUp(gpuim::MouseUpEvent {
                            button,
                            position,
                            modifiers,
                            click_count,
            ...
            Event::XinputMotion(event) => {
                ...
                if event.valuator_mask[0] & 3 != 0 {
                    window.handle_input(PlatformInput::MouseMove(gpuim::MouseMoveEvent {
                        position,
                        pressed_button,
                        modifiers,
                    }));
            ...
            Event::XinputLeave(event) if event.mode == xinput::NotifyMode::NORMAL => {
                ...
                let window = self.get_window(event.event)?;
                window.handle_input(PlatformInput::MouseExited(gpuim::MouseExitEvent {
                    pressed_button,
                    position,
                    modifiers,
                }));
            ...
            Event::XinputHierarchy(event) => {
                let mut state = self.0.borrow_mut();
                // Temporarily use `state.pointer_device_states` to only store pointers that
                // still have valid scroll values. Any change to a device
                // invalidates its scroll values.
            ...
            Event::XinputGesturePinchBegin(event) => {
                ...
                drop(state);
                window.handle_input(PlatformInput::Pinch(gpuim::PinchEvent {
                    position,
                    delta: 0.0,
                    modifiers,
                    phase: gpuim::TouchPhase::Started,
                }));
            }
...
impl LinuxClient for X11Client {
    ...
    {
        gpuim::scap_screen_capture::scap_screen_sources(&self.0.borrow().common.foreground_executor)
    }
...
impl X11ClientState {
    ...
    fn update_refresh_loop(&mut self, x_window: xproto::Window) {
        ...
        match (is_visible, window_ref.refresh_state.take()) {
            ...
            (true, None) => {
                ...
                // doesn't seem to be an efficient / straightforward way to do this. Should also
                // be updated when screen configurations change.
    ...
    fn start_refresh_loop(
        &self, x_window: xproto::Window, refresh_rate: Duration,
    ) -> RegistrationToken {
        self.loop_handle
...
fn check_gtk_frame_extents_supported(
    xcb_connection: &XCBConnection, atoms: &XcbAtoms, root: xproto::Window,
) -> bool {
...
fn xdnd_get_supported_atom(
    xcb_connection: &XCBConnection, supported_atoms: &XcbAtoms, target: xproto::Window,
) -> u32 {
...
fn xdnd_send_finished(
    xcb_connection: &XCBConnection, atoms: &XcbAtoms, source: xproto::Window,
...
fn xdnd_send_status(
    xcb_connection: &XCBConnection, atoms: &XcbAtoms, source: xproto::Window,
    target: xproto::Window, action: u32,
) {
    let message = ClientMessageEvent {
        format: 32,
        window: target,
...
/// When a device is present in `scroll_values_to_preserve`, its value for
/// `ScrollAxisState.scroll_value` is used.
...
/// Returns true if the device is a pointer device. Does not include pointer
/// device groups.
...
/// Returns the scroll delta for a smooth scrolling motion event, or `None` if
/// no scroll data is present.
fn get_scroll_delta_and_update_state(
    pointer: &mut PointerDeviceState, event: &xinput::MotionEvent,
) -> Option<Point<f32>> {
...
fn get_axis_scroll_delta_and_update_state(
    event: &xinput::MotionEvent, axis: &mut ScrollAxisState,
) -> Option<f32> {
...
fn make_scroll_wheel_event(
    position: Point<Pixels>, scroll_delta: Point<f32>, modifiers: Modifiers,
) -> gpuim::ScrollWheelEvent {
...
fn get_scale_factor(
    connection: &XCBConnection, resource_database: &Database, screen_index: usize,
) -> f32 {
