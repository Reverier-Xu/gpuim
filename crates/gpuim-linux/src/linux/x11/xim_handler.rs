...
impl<C: Client<XEvent = xproto::KeyPressEvent>> ClientHandler<C> for XimHandler {
    ...
    fn handle_get_im_values(
        &mut self, client: &mut C, input_method_id: u16,
    ...
    fn handle_create_ic(
        &mut self, _client: &mut C, _input_method_id: u16, input_context_id: u16,
    ) -> Result<(), ClientError> {
        self.connected = true;
    ...
    fn handle_commit(
        &mut self, _client: &mut C, _input_method_id: u16, _input_context_id: u16, text: &str,
    ) -> Result<(), ClientError> {
    ...
    fn handle_forward_event(
        &mut self, _client: &mut C, _input_method_id: u16, _input_context_id: u16,
        _flag: xim::ForwardEventFlag, xev: C::XEvent,
    ) -> Result<(), ClientError> {
    ...
    fn handle_preedit_draw(
        &mut self, _client: &mut C, _input_method_id: u16, _input_context_id: u16, _caret: i32,
        _chg_first: i32, _chg_len: i32, _status: xim::PreeditDrawStatus, preedit_string: &str,
        ...
        // XIMVisibleToForward: 64, XIMVisibleToBackward: 128, XIMVisibleCenter: 256:
        // text align position XIMPrimary, XIMHighlight, XIMSecondary,
        // XIMTertiary are not specified, but interchangeable as above
