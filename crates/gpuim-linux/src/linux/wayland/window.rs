...
use gpuim_wgpu::{CompositorGpuHint, WgpuRenderer, WgpuSurfaceConfig};
...
use wayland_protocols::{
    wp::{fractional_scale::v1::client::wp_fractional_scale_v1, viewporter::client::wp_viewport},
    xdg::{
...
pub(crate) struct Callbacks {
    ...
    input: Option<Box<dyn FnMut(gpuim::PlatformInput) -> gpuim::DispatchEventResult>>,
...
impl WaylandSurfaceState {
    fn new(
        surface: &wl_surface::WlSurface, globals: &Globals, params: &WindowParams,
        parent: Option<WaylandWindowStatePtr>, target_output: Option<wl_output::WlOutput>,
    ) -> anyhow::Result<Self> {
    ...
    fn destroy(&mut self) {
        match self {
            WaylandSurfaceState::Xdg(WaylandXdgSurfaceState {
                ...
                // drop the dialog before toplevel so compositor can explicitly unapply it's
                // effects
...
impl WaylandWindowState {
    pub(crate) fn new(
        handle: AnyWindowHandle, surface: wl_surface::WlSurface,
        surface_state: WaylandSurfaceState, appearance: WindowAppearance,
        viewport: Option<wp_viewport::WpViewport>, client: WaylandClientStatePtr, globals: Globals,
        gpu_context: gpuim_wgpu::GpuContext, compositor_gpu: Option<CompositorGpuHint>,
        options: WindowParams, parent: Option<WaylandWindowStatePtr>,
    ) -> anyhow::Result<Self> {
        let renderer = {
        ...
        if let WaylandSurfaceState::Xdg(ref xdg_state) = surface_state {
            ...
            // This prevents the window from being resized larger than what the GPU can
            // render.
...
impl Drop for WaylandWindow {
    fn drop(&mut self) {
        ...
        // Surface state might contain xdg_toplevel/xdg_surface which can be destroyed
        // now that decorations are gone. layer_surface has no dependencies.
...
impl WaylandWindow {
    ...
    pub fn new(
        handle: AnyWindowHandle, globals: Globals, gpu_context: gpuim_wgpu::GpuContext,
        compositor_gpu: Option<CompositorGpuHint>, client: WaylandClientStatePtr,
        params: WindowParams, appearance: WindowAppearance, parent: Option<WaylandWindowStatePtr>,
...
impl WaylandWindowStatePtr {
    ...
    pub fn handle_surface_event(
        &self, event: wl_surface::Event, outputs: HashMap<ObjectId, Output>,
    ) {
        let mut state = self.state.borrow_mut();
...
impl PlatformWindow for WaylandWindow {
    ...
    fn resize(&mut self, size: Size<Pixels>) {
        ...
        // Keep window geometry consistent with configure handling. On Wayland, window
        // geometry is surface-local: resizing should not attempt to translate
        // the window; the compositor controls placement. We also account for
        // client-side decoration insets and tiling.
        let window_geometry = inset_by_tiling(
            Bounds {
                origin: Point::default(),
                size,
    ...
    fn prompt(
        &self, _level: PromptLevel, _msg: &str, _detail: Option<&str>, _answers: &[PromptButton],
    ) -> Option<Receiver<usize>> {
        None
    }

    fn activate(&self) {
        // Try to request an activation token. Even though the activation is likely
        // going to be rejected, KWin and Mutter can use the app_id to visually
        // indicate we're requesting attention.
        let state = self.borrow();
        if let (Some(activation), Some(app_id)) = (&state.globals.activation, state.app_id.clone())
        {
            ...
            // The serial isn't exactly important here, since the activation is probably
    ...
    fn draw(&self, scene: &Scene) {
        ...
        if state.renderer.device_lost() {
            ...
            // The current scene references atlas textures that were cleared during
            // recovery. Skip this frame and let the next frame rebuild the
            // scene with fresh textures.
    ...
    fn start_window_resize(&self, edge: gpuim::ResizeEdge) {
        let state = self.borrow();
        if let Some(toplevel) = state.surface_state.toplevel() {
            toplevel.resize(
                &state.globals.seat,
...
/// The configuration event is in terms of the window geometry, which we are
/// constantly updating to account for the client decorations. But that's not
/// the area we want to render to, due to our intrusize CSD. So, here we
/// calculate the 'actual' size, by adding back in the insets
fn compute_outer_size(
    inset: Pixels, new_size: Option<Size<Pixels>>, tiling: Tiling,
) -> Option<Size<Pixels>> {
