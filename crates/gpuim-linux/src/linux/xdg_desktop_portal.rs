...
use calloop::{EventSource, Poll, PostAction, Readiness, Token, TokenFactory, channel::Channel};
use gpuim::{BackgroundExecutor, WindowAppearance};
...
impl EventSource for XDPEventSource {
    ...
    fn register(
        &mut self, poll: &mut Poll, token_factory: &mut TokenFactory,
    ) -> calloop::Result<()> {
