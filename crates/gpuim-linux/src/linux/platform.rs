    ...
    fn screen_capture_sources(
        ...
        sources_tx
            .send(Err(anyhow::anyhow!(
                "gpuim_linux was compiled without the screen-capture feature"
            )))
            .ok();
        sources_rx
    }
...
impl<P: LinuxClient + 'static> Platform for LinuxPlatform<P> {
    ...
    fn keyboard_mapper(&self) -> Rc<dyn PlatformKeyboardMapper> {
        Rc::new(gpuim::DummyKeyboardMapper)
    }
    ...
    fn restart(&self, binary_path: Option<PathBuf>) {
        ...
        // Pass dynamic values as positional parameters to avoid shell interpolation
        // issues.
        let script = r#"
        ...
        self.foreground_executor()
            .spawn(async move {
                ...
                let request = match ashpd::desktop::file_chooser::OpenFileRequest::default()
                    ...
                    .modal(true)
                    .title(title)
                    .accept_label(options.prompt.as_ref().map(gpuim::SharedString::as_str))
    ...
    fn can_select_mixed_files_and_dirs(&self) -> bool {
        // org.freedesktop.portal.FileChooser only supports "pick files" and "pick
        // directories".
        false
    }
...
pub(super) fn open_uri_internal(
    executor: BackgroundExecutor, uri: &str, activation_token: Option<String>,
) {
...
pub(super) fn reveal_path_internal(
    executor: BackgroundExecutor, path: PathBuf, activation_token: Option<String>,
) {
    executor
        .spawn(async move {
...
pub(super) fn keystroke_from_xkb(
    state: &State, mut modifiers: gpuim::Modifiers, keycode: Keycode,
) -> gpuim::Keystroke {
...
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(super) fn modifiers_from_xkb(keymap_state: &State) -> gpuim::Modifiers {
...
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(super) fn capslock_from_xkb(keymap_state: &State) -> gpuim::Capslock {
...
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(super) fn compositor_gpu_hint_from_dev_t(dev: u64) -> Option<gpuim_wgpu::CompositorGpuHint> {
    ...
    Some(gpuim_wgpu::CompositorGpuHint {
        vendor_id,
        device_id,
    })
}
