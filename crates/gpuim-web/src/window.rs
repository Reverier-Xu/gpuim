...
use gpuim_wgpu::{WgpuContext, WgpuRenderer, WgpuSurfaceConfig};
use wasm_bindgen::prelude::*;

use crate::{
    display::WebDisplay,
    events::{ClickState, WebEventListeners, is_mac_platform},
};
...
impl WebWindow {
    pub fn new(
        handle: AnyWindowHandle, _params: WindowParams, context: &WgpuContext,
...
impl WebWindowInner {
    ...
    pub(crate) fn with_input_handler<R>(
        &self, f: impl FnOnce(&mut PlatformInputHandler) -> R,
    ) -> Option<R> {
...
// Safari does not support `devicePixelContentBoxSize`, so detect whether it's
// available.
