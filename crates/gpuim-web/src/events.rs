...
impl WebWindowInner {
    ...
    fn listen(
        self: &Rc<Self>, event_name: &str, handler: impl FnMut(JsValue) + 'static,
    ...
    /// Registers a listener with `{passive: false}` so that `preventDefault()`
    /// works. Needed for events like `wheel` which are passive by default
    /// in modern browsers.
...
fn compute_key_char(
    event: &web_sys::KeyboardEvent, gpui_key: &str, modifiers: &Modifiers,
) -> Option<String> {
