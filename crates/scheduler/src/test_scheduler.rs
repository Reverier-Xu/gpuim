...
impl TestScheduler {
    ...
    pub fn many<R>(
        default_iterations: usize, mut f: impl AsyncFnMut(Arc<TestScheduler>) -> R,
    ) -> Vec<R> {
    ...
    /// This is used by GPUI's TestDispatcher to map dispatcher instances to
    /// sessions.
    ...
    ///
    /// This is the public interface for GPUI's TestDispatcher to drive task
    /// execution.
    pub fn tick(&self) -> bool {
        self.step_filtered(false)
    }

    /// Execute one tick, but only run background tasks (no foreground/session
    /// tasks). Returns true if any work was done.
    ...
    /// Foreground tasks are those with a session_id, background tasks have
    /// none.
    ...
    fn step_filtered(&self, background_only: bool) -> bool {
        ...
        let runnable = {
            ...
            // - For foreground tasks (with session_id), only the first task from each
            //   session is a candidate (to preserve intra-session ordering)
    ...
    ///
    /// This is used by the leak detector to ensure that all tasks have been
    /// dropped as tasks may keep entities alive otherwise. Why do we even
    /// have tasks left when tests finish you may ask. The reason for that is
    /// simple, the scheduler itself is the executor and it retains the
    /// scheduled runnables. A lot of tasks, including every foreground task
    /// contain an executor handle that keeps the test scheduler alive, causing
    /// a reference cycle, thus the need for this function right now.
...
impl Scheduler for TestScheduler {
    ...
    fn block(
        &self, session_id: Option<SessionId>, mut future: Pin<&mut dyn Future<Output = ()>>,
    ...
    fn schedule_background_with_priority(
        &self, runnable: Runnable<RunnableMeta>, priority: Priority,
    ) {
        assert_correct_thread(&self.thread, &self.state);
        let mut state = self.state.lock();
        let ix = if state.randomize_order {
