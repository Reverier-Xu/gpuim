...
///
/// Higher priority tasks are more likely to be scheduled before lower priority
/// tasks, but this is not a strict guarantee - the scheduler may interleave
/// tasks of different priorities to prevent starvation.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Hash)]
#[repr(u8)]
pub enum Priority {
    /// Realtime priority
    ///
    /// Spawning a task with this priority will spin it off on a separate thread
    /// dedicated just to that task. Only use for audio.
    RealtimeAudio,
    /// High priority - use for tasks critical to user
...
pub trait Scheduler: Send + Sync {
    ...
    fn block(
        &self, session_id: Option<SessionId>, future: Pin<&mut dyn Future<Output = ()>>,
