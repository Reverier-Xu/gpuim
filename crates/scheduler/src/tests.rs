...
fn test_block_with_timeout() {
    ...
    TestScheduler::once(async |scheduler| {
        // Make timeout behavior deterministic by forcing the timeout tick budget to be
        // exactly 0. This prevents `block_with_timeout` from making progress
        // via extra scheduler stepping and accidentally completing work that we
        // expect to time out.
    ...
    TestScheduler::many(100, async |scheduler| {
        // Keep the existing probabilistic behavior here (do not force 0 ticks), since
        // this subtest is explicitly checking that some seeds/timeouts can
        // complete while others can time out.
    ...
    // pollable to completion later. We also want to ensure time only advances when
    // we explicitly advance it (not by yielding).
    TestScheduler::once(async |scheduler| {
...
// When calling block, we shouldn't make progress on foreground-spawned futures
