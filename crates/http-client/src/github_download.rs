...
pub async fn download_server_binary(
    http_client: &dyn HttpClient, url: &str, digest: Option<&str>, destination_path: &Path,
...
async fn extract_to_staging(
    body: impl AsyncRead + Unpin, digest: Option<&str>, url: &str, staging_path: &Path,
    asset_kind: AssetKind,
) -> Result<()> {
    match digest {
...
async fn stream_response_archive(
    response: impl AsyncRead + Unpin, url: &str, destination_path: &Path, asset_kind: AssetKind,
) -> Result<()> {
    match asset_kind {
...
async fn stream_file_archive(
    file_archive: impl AsyncRead + AsyncSeek + Unpin, url: &str, destination_path: &Path,
    asset_kind: AssetKind,
) -> Result<()> {
    match asset_kind {
...
async fn extract_tar_gz(
    destination_path: &Path, url: &str, from: impl AsyncRead + Unpin,
) -> Result<(), anyhow::Error> {
...
async fn unpack_tar_archive(
    destination_path: &Path, url: &str, archive_bytes: impl AsyncRead + Unpin,
) -> Result<(), anyhow::Error> {
...
impl<W: AsyncWrite + Unpin> AsyncWrite for HashingWriter<W> {
    fn poll_write(
        mut self: Pin<&mut Self>, cx: &mut std::task::Context<'_>, buf: &[u8],
